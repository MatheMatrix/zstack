...
- ❌ 双写阶段推翻 — PhysicalServerCapacityVO 是容量唯一数据源（source of truth），HostCapacityVO 降级为只读 VIEW
...
#### FR-001: PhysicalServerAO/VO 统一实体

**Priority:** Must Have

**Description:**
定义 PhysicalServerAO/VO 统一物理服务器实体，独立于 HostVO 继承链。以 serialNumber 作为硬件唯一主键。核心字段包括 zoneUuid、poolUuid、name、managementIp、architecture、serialNumber、manufacturer、model、state、status，以及 OOB 带外管理凭据。
...
**Description:**
容量的唯一数据源（source of truth）。PhysicalServerCapacityVO 是真表，HostCapacityVO 降级为只读 MySQL JOIN VIEW 映射到此表。所有容量写入通过 PhysicalServerCapacityUpdater 统一入口，HostCapacityUpdater 改造为其内部包装器（59 个调用方零改动）。available 字段扣除所有角色消耗（包括 EXTERNAL_READONLY），不允许超配。

字段：totalPhysicalCpu、totalPhysicalMemory、cpuOverprovisioningRatio、memoryOverprovisioningRatio、availableCpu、availableMemory、reservedMemory、totalDisk、availableDisk、capacityState（枚举：Initialized/Ready/Allocated/Recalculating/Stale）。

**Acceptance Criteria:**
- [ ] PhysicalServerCapacityVO 是真表，HostCapacityVO 是只读 JOIN VIEW
- [ ] 所有容量写入通过 PhysicalServerCapacityUpdater，HostCapacityUpdater 是包装器
- [ ] HostCapacityVO VIEW 对现有 47 个读取方完全透明（Hibernate @Entity 映射、@OneToOne EAGER、Query API）
- [ ] available = 总容量 - 所有角色消耗（包括 Container），不超配
...
- [ ] 迁移：ALTER TABLE HostCapacityVO RENAME TO PhysicalServerCapacityVO + CREATE VIEW HostCapacityVO
...
- [ ] EXTERNAL_READONLY 的容量消耗计入 available（所有角色消耗都扣减，不超配）
...
**Description:**
统一分配消息，核心字段：requiredRoleType、requiredCpu、requiredMemory、clusterUuid、zoneUuid、serverUuid（指定分配）、poolUuid（池内分配）、schedulingMode。采用两阶段薄适配模式：ServerAllocatorChain 只做通用过滤（不含 originalMessage），角色特有参数由阶段2的现有 HostAllocatorChain 处理。
...
**Description:**
KVM 角色适配器。在 KVM Host PostConnect 时创建 PhysicalServerRoleVO，SchedulingMode = INTERNAL_SHARED。getCapacityConsumption 从 PhysicalServerCapacityVO 读取已用容量。
...
> **BM2 优先策略**：BM2 RoleProvider（FR-025）优先实现。BM1 RoleProvider 延后，但 SPI 接口设计已保证 BM1 可无修改接入（ServerRoleType 枚举保留 BAREMETAL_V1、SPI 不含 BM2 特有假设、ProvisionNetworkVO 保留 STANDALONE_PXE 类型、RoleMatchContext 包含 oobAddress 字段）。
...
| EPIC-6 | 角色 SPI + 四角色适配 | FR-022~027 | Must | 6-8 stories（BM1 RoleProvider 延后，不计入当前迭代）|
| EPIC-7 | 兼容层 + 统一 API | FR-028~033 | Must | 7-9 stories |

**总计**: 7 Epics, 33 FRs (28 Must / 5 Should), 10 NFRs (10 Must), 预估 36-47 stories（BM1 RoleProvider FR-024 降为 Could，不计入当前迭代估算）
...
- 不改变现有 API 行为
- 不做 UI 层变更
- v1.0 物理服务器管理限 admin 角色操作，不设计细粒度权限模型（zone-admin、租户可见性等留后续版本）
- L2 Network 挂载不从 Cluster 下放（L2 是调度边界，ServerPool 是运维标签）
- 不做双写/数据同步阶段（PhysicalServerCapacityVO 是唯一真表，HostCapacityVO 是只读 VIEW）
- 不包含跨角色实时切换能力
- HostCapacityUpdater 改造为 PhysicalServerCapacityUpdater 的包装器（6 个写入路径改为直接写真表，47 个读取方通过 VIEW 零改动）

---

## 11. Open Questions（已澄清）

### Q1: 角色互斥规则（FAQ-04）✅ 已解决

BM（INTERNAL_EXCLUSIVE）与 KVM（INTERNAL_SHARED）**在模型层强制互斥**，由 `PhysicalServerManagerImpl.registerRole()` 统一检查：

| 已有角色 \ 新角色 | KVM (SHARED) | BM2 (EXCLUSIVE) | Container (READONLY) |
|----------------|:-:|:-:|:-:|
| 无 | ✅ | ✅ | ✅ |
| KVM (SHARED) | ❌ 同类型 | ❌ 互斥 | ✅ |
| BM2 (EXCLUSIVE) | ❌ 互斥 | ❌ 同类型 | ✅ |
| Container (READONLY) | ✅ | ✅ | ❌ 同类型 |

各 RoleProvider 不自行做互斥检查，统一由 registerRole 保证。

### Q2: serialNumber 可靠性（FAQ-15）✅ 已解决

三级降级匹配策略：
1. **serialNumber**（优先）— KVM 通过现有 `SYSTEM_SERIAL_NUMBER` SystemTag 采集（`collect-kvm-host-facts`），BM2 通过 IPMI FRU，Container 通过 K8s Node `systemUUID`
2. **oobAddress + zoneUuid**（降级）— BM 场景的 IPMI 地址匹配
3. **managementIp + zoneUuid**（最终降级）— 无 serialNumber 和 OOB 时兜底

已知限制：虚拟化环境和白牌服务器 serialNumber 可能无效，此时自动降级到方案 2/3。无效值黑名单（如 "Not Specified"、"To Be Filled"）在匹配时过滤。

### Q3: 混部 IP 匹配问题（FAQ-16）✅ 已解决

KVM + Container 混部时 managementIp 可能不同（多网卡），但 **serialNumber 是主匹配标识，不依赖 IP**。Container 通过 K8s Node `status.nodeInfo.systemUUID` 获取 serialNumber（归一化后与 KVM 的 `/sys/class/dmi/id/product_serial` 一致），确保两个角色关联到同一个 PhysicalServerVO。

### Q4: ProvisionNetwork 迁移字段映射（FAQ-17）✅ 已解决

BM2 映射已完成（BM2 是参考模型）：

| BM2 字段 | 统一字段 | 说明 |
|---------|---------|------|
| dhcpInterface | dhcpInterface | 直接映射 |
...
| dhcpRangeNetworkCidr | — | 不加入统一模型（冗余，可从 startIp+netmask 计算） |

BM1 的 PxeServerVO 迁移**延后**（BM1 整体延后为 Could）。BM2 所有 ProvisionNetwork 映射为 type=GATEWAY_PXE，BM1 映射为 type=STANDALONE_PXE。

### Q5: ServerPool 层级结构（FAQ-18）✅ 已确认

**v1.0 采用扁平结构**，不引入层级。理由：
- ServerPool 定位为运维标签，扁平结构满足"按机房/机架分组"的需求
- 未来如需层级（机房 → 机柜 → 机位），可通过增加 `parentUuid` 自引用字段扩展，不需要改数据模型
