...
- PhysicalServerAO/VO 统一物理服务器实体（独立于 HostVO 继承链）
- PhysicalServerRoleVO 角色映射表（1:N，指向 HostVO/ChassisVO 等）
...
- KVM Host RoleProvider 实现（优先）
- Baremetal V2 RoleProvider 实现（优先）
- Container/K8s RoleProvider 实现（优先）
- Baremetal V1 RoleProvider 实现（延后，Could — SPI 接口设计保证 BM1 可无修改接入）
- 角色自动关联机制（serialNumber 优先匹配，managementIp + zoneUuid 降级）

> **BM2 优先，BM1 不关门**：优先实现 KVM / BM2 / Container 三个 RoleProvider。BM1 RoleProvider 延后实现，但 SPI 接口设计保证 BM1 兼容性（ServerRoleType 枚举保留 BAREMETAL_V1、ProvisionNetworkVO 保留 STANDALONE_PXE 类型、RoleMatchContext 包含 oobAddress 字段、CapacityState 包含 Initialized 状态）。
...
- **Risk:** 此风险已消除——PhysicalServerCapacityVO 是唯一真表，HostCapacityVO 是只读 VIEW，数据天然一致
  - **Likelihood:** N/A
  - **Mitigation:** 无需额外措施
