    ...
    private void handle(APIScanVmInstanceMetadataFromPrimaryStorageMsg msg) {
        ...
        bus.send(gmsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply r) {
                ...
                List<VmMetadataScanEntry> result = re.getVmInstanceMetadata();
                result.stream().filter(e -> !VmMetadataCategory.TEMPLATE_CACHE.toString().equals(e.getVmCategory())).forEach(result::add);
