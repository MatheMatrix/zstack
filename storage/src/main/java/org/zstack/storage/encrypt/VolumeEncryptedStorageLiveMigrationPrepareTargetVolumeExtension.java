...
public class VolumeEncryptedStorageLiveMigrationPrepareTargetVolumeExtension implements KvmStorageLiveMigrationExtensionPoint {
    ...
    @Override
    public List<Flow> getBeforeCreateTemporaryVolumeFlows(String vmUuid,
                                                          String srcHostUuid,
    ...
    @Override
    public ErrorCode beforeCreateTemporaryVolume(CreateVolumeMsg msg,
    ...
    public ErrorCode beforeInstantiateTemporaryVolume(String hostUuid,
        ...
        if (!Boolean.TRUE.equals(temporaryVolume.getEncrypted())) {
            return null;
        }
        ...
        try {
            ...
            spec.setEncryptLuksSecretMaterialFilePath(
                    volumeEncryptedSecretHelper.prepareLuksSecretMaterialFileOnHost(hostUuid, temporaryVolume.getUuid()));
            msg.setVolumeLuksAgentSpec(spec);
            return null;
        } catch (OperationFailureException e) {
            return e.getErrorCode();
        } catch (Exception e) {
            return operr("failed to prepare LUKS secret material for live migration temporary volume[uuid:%s] on host[uuid:%s]: %s",
                    temporaryVolume.getUuid(), hostUuid, e.getMessage());
        }
    }
}
