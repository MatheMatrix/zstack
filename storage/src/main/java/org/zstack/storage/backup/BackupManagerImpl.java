...
public class BackupManagerImpl extends AbstractService implements BackupManager {
    @Autowired
    private CloudBus bus;
    @Autowired
    private ThreadFacade thdf;
    @Autowired
    private BackupOperationConflictManager conflictManager;

    @Override
    public boolean routeToQueue(NeedReplyMessage msg, String vmUuid, String backendServiceId,
        ...
        sendToQueue(msg, vmUuid, backendServiceId, operation, resourceUuid, new CloudBusCallBack(msg) {
            @Override
    ...
    private void sendToQueue(NeedReplyMessage msg, String vmUuid, String backendServiceId,
                             BackupOperationConflictChecker.Operation operation, String resourceUuid,
                             CloudBusCallBack callback) {
        BackupMessage bmsg = BackupMessage.valueOf(vmUuid, backendServiceId, operation, resourceUuid, msg);
        ...
        thdf.chainSubmit(new ChainTask(msg) {
            ...
            public void run(SyncTaskChain chain) {
                if (msg.getMessageDeadline() != -1 &&
                    ...
                    replyAndContinue(msg, reply, chain);
                    return;
                }

                ErrorCode error = conflictManager.check(msg.getOperation(), msg.getVmInstanceUuid(),
                        msg.getResourceUuid());
                if (error != null) {
                    MessageReply reply = new MessageReply();
                ...
                bus.makeLocalServiceId(inner, msg.getBackendServiceId());
    ...
    @Deferred
    private void replyAndContinue(BackupMessage msg, MessageReply reply, SyncTaskChain chain) {
        Defer.defer(chain::next);
        bus.reply(msg, reply);
    }
