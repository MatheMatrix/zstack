...
public abstract class BackupStorageBase extends AbstractBackupStorage {
    ...
    private long getContentLength(String url) {
        ...
        try {
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_BACKUP_10015, "failed to get header of image url %s: %s", url, e.toString()));
        }

        if (header == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_BACKUP_10016, "failed to get header of image url %s", url));
        }
    ...
    protected void exceptionIfImageSizeGreaterThanAvailableCapacity(String url) {
        ...
        if (size == -1) {
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_BACKUP_10017, "the image size get from url %s is %d bytes, " +
                    "it's too small for an image, please check the url again.", url, size));
        } else if (size > available) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_BACKUP_10018, "the backup storage[uuid:%s, name:%s] has not enough capacity to download the image[%s]." +
    ...
    protected void checkStatus(Message msg) {
        if (!statusChecker.isOperationAllowed(msg.getClass().getName(), self.getStatus().toString())) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_BACKUP_10019, "backup storage cannot proceed message[%s] because its status is %s", msg.getClass().getName(), self.getStatus()));
        }
    }

    protected void checkState(Message msg) {
        if (!stateChecker.isOperationAllowed(msg.getClass().getName(), self.getState().toString())) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_BACKUP_10020, "backup storage cannot proceed message[%s] because its state is %s", msg.getClass().getName(), self.getState()));
        }
    }
    ...
    private void cleanTrash(Long trashId, final ReturnValueCompletion<TrashCleanupResult> completion) {
        ...
        if (details != null) {
            completion.success(new TrashCleanupResult(inv.getResourceUuid(), inv.getTrashId(), operr(ORG_ZSTACK_STORAGE_BACKUP_10021, details)));
            return;
        }
    ...
    private void cleanUpTrash(Long trashId, final ReturnValueCompletion<List<TrashCleanupResult>> completion) {
        ...
        new While<>(trashs).all((inv, coml) -> {
            String details = trash.makeSureInstallPathNotUsed(inv);
            if (details != null) {
                results.add(new TrashCleanupResult(inv.getResourceUuid(), inv.getTrashId(), operr(ORG_ZSTACK_STORAGE_BACKUP_10022, details)));
                coml.done();
                return;
            }
    ...
    protected void handle(final APIDetachBackupStorageFromZoneMsg msg) {
        ...
        try {
            ...
            evt.setError(err(ORG_ZSTACK_STORAGE_BACKUP_10023, BackupStorageErrors.DETACH_ERROR, e.getMessage()));
            bus.publish(evt);
            return;
        }
        ...
        detachHook(new Completion(msg) {
            ...
            public void fail(ErrorCode errorCode) {
                ...
                evt.setError(err(ORG_ZSTACK_STORAGE_BACKUP_10024, BackupStorageErrors.DETACH_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
            }
        });
    }
    ...
    protected void handle(final APIAttachBackupStorageToZoneMsg msg) {
        ...
        if (errStr != null) {
            evt.setError(err(ORG_ZSTACK_STORAGE_BACKUP_10025, BackupStorageErrors.ATTACH_ERROR, errStr));
            bus.publish(evt);
            return;
        }
        ...
        attachHook(msg.getZoneUuid(), new Completion(msg) {
            ...
            public void fail(ErrorCode errorCode) {
                ...
                evt.setError(err(ORG_ZSTACK_STORAGE_BACKUP_10026, BackupStorageErrors.ATTACH_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
            }
        });
    }
    ...
    protected void handle(APIChangeBackupStorageStateMsg msg) {
        ...
        try {
            ...
            evt.setError(err(ORG_ZSTACK_STORAGE_BACKUP_10027, SysErrors.CHANGE_RESOURCE_STATE_ERROR, e.getMessage()));
            bus.publish(evt);
            return;
        }
    ...
    protected void handle(APIDeleteBackupStorageMsg msg) {
        ...
        chain.done(new FlowDoneHandler(msg) {
            ...
            public void handle(ErrorCode errCode, Map data) {
                evt.setError(err(ORG_ZSTACK_STORAGE_BACKUP_10028, SysErrors.DELETE_RESOURCE_ERROR, errCode, errCode.getDetails()));
                bus.publish(evt);
            }
        }).start();
    }
