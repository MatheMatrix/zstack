...
public class VolumeApiInterceptor implements ApiMessageInterceptor, Component, GlobalApiMessageInterceptor {
    ...
    private void validate(VolumeCreateMessage msg) {
        ...
        if (!config.getAllocate().getAllPrimaryStorages().isEmpty()) {
            ...
            if (msg.getPrimaryStorageUuid() != null && !requiredPrimaryStorageUuids.contains(msg.getPrimaryStorageUuid())) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10051, "primary storage uuid conflict, the primary storage specified by the disk offering are %s, and the primary storage specified in the creation parameter is %s",
    ...
    private void validate(APICreateVolumeSnapshotMsg msg) {
        ...
        if (status != VolumeStatus.Ready) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10052, "volume[uuid:%s] is not in status Ready, current is %s, can't create snapshot", msg.getVolumeUuid(), status));
        }

        VolumeType type = (VolumeType) tuple.get(1);
        if (type != VolumeType.Root && type != VolumeType.Data) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10053, "volume[uuid:%s, type:%s], can't create snapshot", msg.getVolumeUuid(), type));
        }

        VolumeState state = (VolumeState) tuple.get(2);
        if (state != VolumeState.Enabled) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10054, "volume[uuid:%s] is not in state Enabled, " +
    ...
    private void validate(APICreateVolumeSnapshotGroupMsg msg) {
        ...
        if (vmvo == null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10055, "volume[uuid:%s] is not root volume", msg.getRootVolumeUuid()));
        }

        if (msg.isWithMemory() && !(vmvo.getState().equals(VmInstanceState.Running) || (vmvo.getState().equals(VmInstanceState.Paused)))) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10056, "Can not take memory snapshot, vm current state[%s], but expect state are [%s, %s]",
        ...
        for (VolumeVO vol : vmvo.getAllVolumes()) {
            if (vol.getStatus() != VolumeStatus.Ready) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10057, "volume[uuid:%s] is not in status Ready, " +
    ...
    private void validate(APIRecoverDataVolumeMsg msg) {
        ...
        if (!q.isExists()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10058, "the volume[uuid:%s] is not in status of deleted. This is operation is to recover a deleted data volume",
                    msg.getVolumeUuid()));
        }
    }
    ...
    private void exceptionIsVolumeIsDeleted(String volumeUuid) {
        ...
        q.add(VolumeVO_.status, Op.EQ, VolumeStatus.Deleted);
        if (q.isExists()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10059, "the volume[uuid:%s] is in status of deleted, cannot do the operation", volumeUuid));
        }
    }
    ...
    private void validate(APICreateDataVolumeFromVolumeTemplateMsg msg) {
        ...
        if (ImageMediaType.DataVolumeTemplate != type) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10060, "image[uuid:%s] is not %s, it's %s", msg.getImageUuid(), ImageMediaType.DataVolumeTemplate, type));
        }

        if (ImageState.Enabled != img.getState()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10061, "image[uuid:%s] is not Enabled, it's %s", img.getUuid(), img.getState()));
        }

        if (ImageStatus.Ready != img.getStatus()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10062, "image[uuid:%s] is not Ready, it's %s", img.getUuid(), img.getStatus()));
        }
    }
    ...
    private void validate(APIGetDataVolumeAttachableVmMsg msg) {
        ...
        if (type == VolumeType.Root) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10063, "volume[uuid:%s] is Root volume, can not be attach to vm", msg.getVolumeUuid()));
        }
        ...
        if (state != VolumeState.Enabled) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10064, "volume[uuid:%s] is in state[%s], data volume can only be attached when state is %s", msg.getVolumeUuid(), state, VolumeState.Enabled));
        }

        VolumeStatus status = t.get(2, VolumeStatus.class);
        if (status != VolumeStatus.Ready && status != VolumeStatus.NotInstantiated) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10065, "volume[uuid:%s] is in status[%s], data volume can only be attached when status is %s or %S", msg.getVolumeUuid(), status, VolumeStatus.Ready, VolumeStatus.NotInstantiated));
        }
    }
    ...
    private void validate(APIDetachDataVolumeFromVmMsg msg) {
        ...
        if (!vol.isShareable() && vol.getVmInstanceUuid() == null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10066, "data volume[uuid:%s] is not attached to any vm, can't detach", msg.getVolumeUuid()));
        }

        if (vol.isShareable() && msg.getVmUuid() == null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10067, "to detach shareable data volume[uuid:%s], vm uuid is needed.", msg.getVolumeUuid()));
        }


        if (vol.getType() != VolumeType.Data) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10068, "the volume[uuid:%s, name:%s, type:%s] can't detach it",
    ...
    private void validate(APIAttachDataVolumeToVmMsg msg) {
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (count > 0) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10069, "the vm[uuid:%s] doesn't support to online attach volume[%s] on the basis of that the image platform type of the vm is other ", msg.getVmInstanceUuid(), msg.getVolumeUuid()));
                }
    ...
    private ErrorCode checkDataVolume(VolumeVO volumeVO, String hvType, long attachedDataVolumeNum) {
        if (volumeVO.getType() == VolumeType.Root) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10070, "the volume[uuid:%s, name:%s] is Root Volume, can't attach it", volumeVO.getUuid(), volumeVO.getName());
        }

        if (volumeVO.getState() == VolumeState.Disabled) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10071, "data volume[uuid:%s] is Disabled, can't attach", volumeVO.getUuid());
        }

        if (volumeVO.getStatus() == VolumeStatus.Deleted) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10072, "the volume[uuid:%s] is in status of deleted, cannot do the operation", volumeVO.getUuid());
        }

        if (volumeVO.isAttached() && !volumeVO.isShareable()) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10073, "data volume[uuid:%s] has been attached to some vm, can't attach again", volumeVO.getUuid());
        }

        if (VolumeStatus.Ready != volumeVO.getStatus() && VolumeStatus.NotInstantiated != volumeVO.getStatus()) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10074, "data volume can only be attached when status is [%s, %s], current is %s",
        ...
        if (volumeVO.getFormat() != null && hvType != null) {
            ...
            if (hvTypes.isEmpty()) {
                return operr(ORG_ZSTACK_STORAGE_VOLUME_10075, "data volume[uuid:%s] of format[%s] is not supported for attach to any hypervisor.", volumeVO.getUuid(), volumeVO.getFormat());
            }

            if (!hvTypes.contains(hvType)) {
                return operr(ORG_ZSTACK_STORAGE_VOLUME_10076, "data volume[uuid:%s] has format[%s] that can only be attached to hypervisor[%s], " +
    ...
    private ErrorCode checkClusterAccessible(VolumeVO volumeVO, List<String> vmInstanceClusterUuids) {
        ...
        if (vmInstanceClusterUuids.isEmpty() && !volumeClusterUuids.isEmpty()) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10077, "Can't attach volume to VM, no qualified cluster");
        }
    ...
    private ErrorCode checkHostAccessible(VolumeVO volumeVO, String hostUuid) {
        ...
        if (primaryStorageHostStatus == PrimaryStorageHostStatus.Disconnected) {
            return operr(ORG_ZSTACK_STORAGE_VOLUME_10078, "Can not attach volume to vm runs on host[uuid: %s] which is disconnected " +
    ...
    private void validate(APIBackupDataVolumeMsg msg) {
        if (isRootVolume(msg.getUuid())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10079, "it's not allowed to backup root volume, uuid:%s", msg.getUuid()));
        }
    ...
    private void validate(APICreateDataVolumeMsg msg) {
        if (msg.getDiskOfferingUuid() == null) {
            if (msg.getDiskSize() < 0) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10080, "unexpected disk size settings"));
            }
        } else {
    ...
    private void validate(APIDeleteDataVolumeMsg msg) {
        ...
        if (type != VolumeType.Data) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10081, "volume[uuid:%s, type:%s] can't be deleted", msg.getVolumeUuid(), type));
        }

        VolumeStatus status = t.get(1, VolumeStatus.class);
        if (status == VolumeStatus.Deleted) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10082, "volume[uuid:%s] is already in status of deleted", msg.getVolumeUuid()));
        }
        ...
        if (hostUuid != null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10083, "can not delete volume[%s], " +
    ...
    private void validate(APIChangeVolumeStateMsg msg) {
        if (isRootVolume(msg.getUuid())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10084, "it's not allowed to change state of root volume, uuid:%s", msg.getUuid()));
        }
        ...
        String hostUuid = Q.New(VolumeHostRefVO.class).select(VolumeHostRefVO_.hostUuid)
                .eq(VolumeHostRefVO_.volumeUuid, msg.getUuid()).findValue();
        if (hostUuid != null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10085, "can not change volume[%s] state, " +
                    "because volume attach to host[%s]", msg.getVolumeUuid(), hostUuid));
        }
    }
    ...
    private void validate(APIAttachDataVolumeToHostMsg msg) {
        ...
        if (hostStatus != HostStatus.Connected) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10086, "can not attach volume[%s] to host[%s], because host[status:%s] is not connected",
                    msg.getVolumeUuid(), msg.getHostUuid(), hostStatus));
        }

        if (!msg.getMountPath().startsWith("/")) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10087, "mount path must be absolute path"));
        }
    ...
    private void doValidateAttachedVolume(Tuple hostAndMountPath, APIAttachDataVolumeToHostMsg msg) {
        ...
        if (!hostUuid.equals(msg.getHostUuid())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10088, "can not attach volume[%s] to host[%s], because volume is attaching to host[%s]",
                    msg.getVolumeUuid(), msg.getHostUuid(), hostUuid));
        }
        if (!mountPath.equals(msg.getMountPath())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10089, "can not attach volume[%s] to host[%s], because the volume[%s] occupies the mount path[%s] on host[%s]",
    ...
    private void checkMountPathOnHost(APIAttachDataVolumeToHostMsg msg) {
        ...
        if (mountPaths.contains(msg.getMountPath())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10090, "can not attach volume[%s] to host[%s], because the another volume occupies the mount path[%s]",
    ...
    private void validate(APIDetachDataVolumeFromHostMsg msg) {
        if (!Q.New(VolumeHostRefVO.class).eq(VolumeHostRefVO_.volumeUuid, msg.getVolumeUuid()).isExists()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10091, "can not detach volume[%s] from host. " +
    ...
    private void validate(APIFlattenVolumeMsg msg) {
        ...
        if (isShareable) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10092, "cannot flatten a shareable volume[uuid:%s]", msg.getVolumeUuid()));
        }
    }
    ...
    private void validate(APIUndoSnapshotCreationMsg msg) {
        ...
        if (currentTreeUuid == null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10093, "can not found in used snapshot tree of volume[uuid: %s]", msg.getUuid()));
        }
        ...
        if (!isLatest) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10094, "cannot undo not latest snapshot"));
        }
    }
    ...
    protected void validate(APICreateVmInstanceMsg msg) {
        ...
        if (!CollectionUtils.isEmpty(duplicateVolumeUuids)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_VOLUME_10095, "duplicate volume uuids: %s", duplicateVolumeUuids.toString()));
        }
        ...
        if (!errors.isEmpty()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_VOLUME_10096, errors.toString()));
        }
    }
