    ...
    private boolean getShareableCapabilityFromMsg(CreateDataVolumeFromVolumeTemplateMsg msg) {
        ...
        if (isShareable && StringUtils.isNotEmpty(msg.getPrimaryStorageUuid())) {
            ...
            if (StringUtils.isEmpty(psType)) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10000, "get primaryStorage %s type failed", msg.getPrimaryStorageUuid()));
            }

            if (!PrimaryStorageType.getSupportFeaturesTypes(PrimaryStorageType::isSupportSharedVolume).contains(psType)) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10001, "primaryStorage type [%s] not support shared volume yet", psType));
            }
        }
            ...
            public void setup() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        List<String> bsUuids = new SQLBatchWithReturn<List<String>>() {
                            ...
                            protected List<String> scripts() {
                                ...
                                if (bsUuids.isEmpty()) {
                                    throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10002, "the image[uuid:%s, name:%s] has been deleted on all backup storage", template.getUuid(), template.getName()));
                                }
                        ...
                        if (bsUuids.isEmpty()) {
                            trigger.fail(operr(ORG_ZSTACK_STORAGE_VOLUME_10003, "cannot find a backup storage on which the image[uuid:%s] is that satisfies all conditions of: 1. has state Enabled 2. has status Connected. 3 has attached to zone in which primary storage[uuid:%s] is",
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (vo == null) {
                            trigger.fail(operr(ORG_ZSTACK_STORAGE_VOLUME_10004, "target volume is expunged during volume creation"));
                            return;
                        }
                    ...
                    public void handle(Map data) {
                        VolumeVO vo = dbf.reload(vol);
                        if (vo == null) {
                            reply.setError(operr(ORG_ZSTACK_STORAGE_VOLUME_10005, "target volume is expunged during volume creation"));
                            bus.reply(msg, reply);
                            return;
                        }
    ...
    public static void preCheckPrimaryStorage(VolumeCreateMessage msg) {
        ...
        if (!config.getAllocate().getAllPrimaryStorages().isEmpty()) {
            ...
            if (!requiredPrimaryStorageUuids.contains(msg.getPrimaryStorageUuid())) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10006, "primary storage uuid conflict, the primary storage specified by the disk offering are %s, and the primary storage specified in the creation parameter is %s",
    ...
    private void instantiateDataVolumeFromSnapshot(VolumeVO vo, String snapshotUuid, List<String> systemTags, ReturnValueCompletion<VolumeInventory> completion) {
        ...
        bus.send(cmsg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (vvo == null) {
                    reply.setError(operr(ORG_ZSTACK_STORAGE_VOLUME_10007, "target volume is expunged during volume creation"));
                }
    ...
    public boolean start() {
...

        {
            List<VolumeFactory> exts = pluginRgty.getExtensionList(
                    VolumeFactory.class);
            if (exts.size() > 1) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10008, "there should not be more than one %s implementation.",
