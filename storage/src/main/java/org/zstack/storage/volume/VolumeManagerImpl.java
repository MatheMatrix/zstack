    ...
    private boolean getShareableCapabilityFromMsg(CreateDataVolumeFromVolumeTemplateMsg msg) {
        ...
        if (isShareable && StringUtils.isNotEmpty(msg.getPrimaryStorageUuid())) {
            ...
            if (!PrimaryStorageType.getSupportFeaturesTypes(PrimaryStorageType::isSupportSharedVolume).contains(psType)) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10079, "primaryStorage type [%s] not support shared volume yet", psType));
            }
        }
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        List<String> bsUuids = new SQLBatchWithReturn<List<String>>() {
                            ...
                            protected List<String> scripts() {
                                ...
                                if (bsUuids.isEmpty()) {
                                    throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10080, "the image[uuid:%s, name:%s] has been deleted on all backup storage", template.getUuid(), template.getName()));
                                }
                        ...
                        if (bsUuids.isEmpty()) {
                            trigger.fail(operr(ORG_ZSTACK_STORAGE_VOLUME_10081, "cannot find a backup storage on which the image[uuid:%s] is that satisfies all conditions of: 1. has state Enabled 2. has status Connected. 3 has attached to zone in which primary storage[uuid:%s] is",
    ...
    public static void preCheckPrimaryStorage(VolumeCreateMessage msg) {
        ...
        if (!config.getAllocate().getAllPrimaryStorages().isEmpty()) {
            ...
            if (!requiredPrimaryStorageUuids.contains(msg.getPrimaryStorageUuid())) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10084, "primary storage uuid conflict, the primary storage specified by the disk offering are %s, and the primary storage specified in the creation parameter is %s",
    ...
    public boolean start() {
...

        {
            List<VolumeFactory> exts = pluginRgty.getExtensionList(
                    VolumeFactory.class);
            if (exts.size() > 1) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10086, "there should not be more than one %s implementation.",
