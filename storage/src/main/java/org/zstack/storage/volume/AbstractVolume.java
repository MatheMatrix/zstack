...
public abstract class AbstractVolume {
    ...
    private ErrorCode validateOperationByState(OperationChecker checker, Message msg, VolumeStatus currentState, Enum errorCode) {
        ...
        ErrorCode cause = Platform.err(ORG_ZSTACK_STORAGE_VOLUME_10055, VmErrors.NOT_IN_CORRECT_STATE, "current volume state[%s] doesn't allow to proceed message[%s]", currentState,
                msg.getMessageName());
        if (errorCode != null) {
            return Platform.err(ORG_ZSTACK_STORAGE_VOLUME_10056, errorCode, cause, cause.getDetails());
        }
        return cause;
    }
