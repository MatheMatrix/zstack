...
public class VolumeBase extends AbstractVolume implements Volume {
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (!cacheExists && !imageExists) {
                            trigger.fail(operr(ORG_ZSTACK_STORAGE_VOLUME_10012, "cannot find image cache[imageUuid: %s] for reinit volume", self.getRootImageUuid()));
                            return;
                        }
    ...
    private void expunge(final Completion completion) {
        ...
        if (self.getStatus() != VolumeStatus.Deleted) {
            completion.fail(operr(ORG_ZSTACK_STORAGE_VOLUME_10013, "the volume[uuid:%s, name:%s] is not deleted yet, can't expunge it",
                            ...
                            protected void scripts() {
                                ...
                                VolumeVO newRootVolumeVO = vm.getAllVolumes().stream().filter(it -> it.getUuid().equals(msg.getVolumeUuid()))
                                        .findFirst().orElseThrow(() -> new OperationFailureException(
                                                operr(ORG_ZSTACK_STORAGE_VOLUME_10014, "volume[uuid%s] should be attached.")
                                        ));
    ...
    private void handle(ChangeVolumeTypeMsg msg) {
        if (self.isAttached()) {
            ...
            reply.setError(operr(ORG_ZSTACK_STORAGE_VOLUME_10015, "only support detached volume, use SetVmBootVolumeMsg instead."));
            bus.reply(msg, reply);
            return;
        }
    ...
    private void handle(APIDeleteDataVolumeMsg msg) {
        ...
        delete(msg.getDeletionMode() == DeletionMode.Enforcing, new Completion(msg) {
            ...
            @Override
            public void fail(ErrorCode errorCode) {
                evt.setError(err(ORG_ZSTACK_STORAGE_VOLUME_10016, SysErrors.DELETE_RESOURCE_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
            }
        });
    }
        ...
        thdf.chainSubmit(new ChainTask(msg) {
            ...
            public void run(SyncTaskChain chain) {
                ...
                flattenVolume(new Completion(chain) {
                    ...
                    @Override
                    public void fail(ErrorCode errorCode) {
                        reply.setError(err(ORG_ZSTACK_STORAGE_VOLUME_10017, VolumeErrors.FLATTEN_ERROR, errorCode, "failed to flatten volume[uuid:%s]", self.getUuid()));
                        bus.reply(msg, reply);
                        chain.next();
                    }
                });
    ...
    static void vmAttachVolumeValidator(VmInstanceInventory vmInv, String volumeUuid) {
        ...
        if (vmDataVolumeUsage + 1 > maxDataVolumeNum) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_VOLUME_10018, "hypervisor[%s] only allows max %s data volumes to be attached to a single vm; there have been %s data volumes attached to vm[uuid:%s]",
