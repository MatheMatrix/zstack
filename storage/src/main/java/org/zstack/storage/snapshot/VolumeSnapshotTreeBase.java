                            ...
                            flow(new NoRollbackFlow() {
                                ...
                                public void run(FlowTrigger trigger, Map data) {
                                    ...
                                    bus.send(smsg, new CloudBusCallBack(completion) {
                                        ...
                                        public void run(MessageReply reply) {
                                            if (!reply.isSuccess()) {
                                                trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10012, String.format("failed to get volume[uuid:%s, installPath:%s] size on primary storage[uuid:%s], because %s",
    ...
    private void changeStatusOfSnapshots(final StatusEvent evt, final List<VolumeSnapshotInventory> snapshots, final Completion completion) {
        ...
        bus.send(msgs, new CloudBusListCallBack(completion) {
            ...
            public void run(List<MessageReply> replies) {
                ...
                if (err != null) {
                    completion.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10013, "failed to change status of volume snapshot[uuid:%s, name:%s] by status event[%s]",
...
*/
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        for (String vmUuid : volumeInventory.getAttachedVmUuids()) {
                            ...
                            if (state != VmInstanceState.Stopped) {
                                trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10014, "unable to reset volume[uuid:%s] to snapshot[uuid:%s]," +
    ...
    private void deleteVolumeSnapshot(final DeleteVolumeSnapshotMessage msg, Completion completion) {
        ...
        chain.done(new FlowDoneHandler(completion) {
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                completion.fail(err(ORG_ZSTACK_STORAGE_SNAPSHOT_10015, SysErrors.DELETE_RESOURCE_ERROR, errCode, errCode.getDetails()));
            }
        }).start();
    }
