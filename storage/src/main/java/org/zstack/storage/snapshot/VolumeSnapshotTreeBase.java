...
public class VolumeSnapshotTreeBase {
    ...
    private void deleteOnlySelf(FlowChain chain, final VolumeSnapshotDeletionMsg msg, NoErrorCompletion completion) {
        ...
        chain.then(new ShareFlow() {
            ...
            List<String> srcSnapshotChildrenInstallPaths = new ArrayList<>();
            ...
            private void buildDeleteSnapshotFlows() {
                ...
                if (childrenSize == 1) {
                    if (currentParentSnapshotLeaf != null && currentParentSnapshotLeaf.getChildren().size() == 1 &&
                            Objects.equals(currentParentSnapshotLeaf.getInventory().getStatus(), VolumeSnapshotStatus.Deleted.toString())) {
                        dstSnapshotInv = currentParentSnapshotLeaf.getInventory();
                        requiredExtraSize = currentSnapshotVO.getSize() + srcSnapshotInv.getSize();
                    } else {
                        dstSnapshotInv = currentSnapshotLeaf.getInventory();
                        requiredExtraSize = srcSnapshotInv.getSize();
                    }
                    VolumeTree.VolumeSnapshotLeaf srcSnapshotLeaf = currentSnapshotLeaf.getChildren().get(0);
                    srcSnapshotInv = srcSnapshotLeaf.getInventory();
                    newLatestSnapshotUuid = Objects.equals(srcSnapshotInv.getStatus(), VolumeSnapshotStatus.Ready.toString()) &&
                            currentSnapshotLeaf.getInventory().isLatest() ? srcSnapshotInv.getUuid() : null;
                    srcSnapshotInvIsVolume = srcSnapshotInv.getUuid().equals(volume.getUuid());
                    srcSnapshotChildrenInstallPaths = srcSnapshotLeaf.getChildren().stream()
            ...
            private void buildDeleteSnapshotFromParentFlows() {
                ...
                if (currentParentSnapshotLeaf.getChildren().size() == 1) {
                    setupDirectDeleteSnapshotBitsFlows(VolumeSnapshotInventory.valueOf(currentSnapshotVO));
                    if (currentParentSnapshotLeaf.getInventory().isLatest()) {
                        newLatestSnapshotUuid = currentParentSnapshotLeaf.getParent().getInventory().getUuid();
                    }
                    return;
                }

                if (currentParentSnapshotLeaf.getChildren().size() == 2) {
                    VolumeTree.VolumeSnapshotLeaf srcSnapshotLeaf = currentVolumeTree.getSiblingLeaves(currentSnapshotLeaf).get(0);
                    srcSnapshotInv = srcSnapshotLeaf.getInventory();
                    dstSnapshotInv = currentParentSnapshotLeaf.getInventory();
            ...
            private void setupDeleteSnapshotOnlySelfFlows() {
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        bmsg.setAliveChainInstallPathInDb(currentVolumeTree.getAliveChainSnapshotInstallPaths());
                        bmsg.setSrcChildrenInstallPath(srcSnapshotChildrenInstallPaths);
            ...
            private void updateDistance(VolumeSnapshotInventory volumeSnapshotVO, int distanceDifferenceValue) {
                final Map<String, Integer> distanceMap = new HashMap<>();

                buildDistanceMap(getSnapLeaf(volumeSnapshotVO.getUuid()).getChildren(), distanceMap, distanceDifferenceValue);
            ...
            private void buildDistanceMap(List<SnapshotLeaf> snapshotLeaves, Map<String, Integer> distanceMap, int distanceDifferenceValue) {
