...
public class VolumeSnapshotTreeBase {
    ...
    private void deletion(final VolumeSnapshotDeletionMsg msg, final NoErrorCompletion completion) {
        ...
        if (msg.isOnlySelf()) {
            deleteOnlySelf(chain, msg, completion);
            return;
        }
    ...
    private void deleteOnlySelf(FlowChain chain, final VolumeSnapshotDeletionMsg msg, NoErrorCompletion completion) {
        ...
        VolumeTree.VolumeSnapshotDeletionContext context = currentVolumeTree.createDeletionContext(currentSnapshotVO);
        ...
        Map<String, VolumeSnapshotInventory> snapshotsToDeleteByUuid = context.getSnapshotsToDeleteByUuid();

        if (context.isDeleteDbOnly()) {
            logger.debug(String.format("mark-snapshot-%s-status-as-Deleted", currentSnapshotVO.getUuid()));
        ...
        chain.then(new ShareFlow() {
            ...
            public void setup() {
                ...
                snapshotsToDeleteByUuid.values().forEach(this::setupDeleteBitsFlows);
            ...
            private void setupDeleteSnapshotOnlySelfFlows() {
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        List<String> aliveChainInstallPaths = currentVolumeTree.getAliveChainInstallPaths();
                        if (volume.getVmInstanceUuid() != null && aliveChainInstallPaths.contains(bmsg.getSrcPath())
                                && aliveChainInstallPaths.contains(bmsg.getDstPath())) {
                            bmsg.setVmUuid(volume.getVmInstanceUuid());
                        }
                        bmsg.setAliveChainInstallPathInDb(aliveChainInstallPaths);
                        ...
                        bus.send(bmsg, new CloudBusCallBack(trigger) {
                            ...
                            public void run(MessageReply reply) {
                                ...
                                if (Objects.equals(srcSnapshotInv.getPrimaryStorageInstallPath(), volume.getInstallPath())) {
                                    markSnapshotAsVolume(volume, dstSnapshotInv, newInstallPath, newInstallPathSize, trigger);
                                    return;
                                }
                            ...
                            private void markSnapshotAsVolume(VolumeVO volume, VolumeSnapshotInventory snapshotInv,
                                                              String newVolumeInstallPath, long newInstallPathSize, FlowTrigger trigger) {
                                logger.debug(String.format("mark snapshot[uuid:%s] as volume", snapshotInv));

                                new SQLBatch() {
                                    @Override
                                    protected void scripts() {
                                        sql(VolumeVO.class).eq(VolumeVO_.uuid, volume.getUuid())
                                                .set(VolumeVO_.installPath, newVolumeInstallPath).update();
                                        sql(VolumeSnapshotVO.class).eq(VolumeSnapshotVO_.uuid, snapshotInv.getUuid())
                                                .set(VolumeSnapshotVO_.primaryStorageInstallPath, volume.getInstallPath()).update();
                                        if (snapshotInv.getParentUuid() != null) {
                                            sql(VolumeSnapshotVO.class).eq(VolumeSnapshotVO_.uuid, snapshotInv.getParentUuid())
                                                    .set(VolumeSnapshotVO_.latest, true).update();
                                            logger.debug(String.format("reset latest snapshot of tree[uuid:%s] to snapshot[uuid:%s]",
                                                    snapshotInv.getTreeUuid(), snapshotInv.getParentUuid()));
                                        }
                                        VolumeSnapshotReferenceUtils.updateReferenceAfterMarkSnapshotAsVolume(snapshotInv);
                                    }
                                }.execute();
                                ...
                                cleanupAfterCommitInQueue(snapshotInv, new NoErrorCompletion() {
                                    @Override
                                    public void done() {
                                        List<MarkSnapshotAsVolumeExtension> extensions = pluginRgty.getExtensionList(MarkSnapshotAsVolumeExtension.class);
                                        for (MarkSnapshotAsVolumeExtension extension : extensions) {
                                            extension.afterMarkSnapshotAsVolume(snapshotInv);
                                        }
                                        ...
                                        long volumeNewSize = newInstallPathSize +
                                                allSnapshotSizes.stream().mapToLong(Long::longValue).sum() - snapshotInv.getSize();
                                        if (volumeNewSize < 0) {
                                            volumeNewSize = 0;
                                        }
                            ...
                            private void updateVolumeSnapshotPrimaryStorageInstallPath(String snapshotUuid,
                                                                                       String primaryStorageInstallPath, long size) {
                            ...
                            private void updateDistance() {
                                int distanceDifferenceValue = srcSnapshotInv.getDistance() - dstSnapshotInv.getDistance();
            ...
            private void setupDeleteBitsFlows(VolumeSnapshotInventory snapshotInv) {
                if (srcSnapshotInv != null && dstSnapshotInv != null &&
                        Objects.equals(srcSnapshotInv.getUuid(), volume.getUuid()) &&
                        Objects.equals(dstSnapshotInv.getUuid(), snapshotInv.getUuid())) {
...
//            handle((APIBackupVolumeSnapshotMsg) msg);
    ...
    private void cleanupAfterCommitInQueue(VolumeSnapshotInventory volumeSnapshotInv, NoErrorCompletion completion) {
        if (volumeSnapshotInv.getGroupUuid() == null) {
            cleanupAfterCommit(volumeSnapshotInv, new NoErrorCompletion() {
                @Override
                public void done() {
                    completion.done();
                }
        ...
        queue.name("cleanup-after-commit-in-queue").asyncBackup(null)
                .run(chain -> cleanupAfterCommit(volumeSnapshotInv, new NoErrorCompletion(chain) {
                    @Override
                    public void done() {
                        completion.done();
                        chain.next();
    ...
    private void cleanupAfterCommit(VolumeSnapshotInventory volumeSnapshotInv, final NoErrorCompletion completion) {
        ...
        ungroupAfterCommit(volumeSnapshotInv);
        completion.done();
    }

    private void ungroupAfterCommit(VolumeSnapshotInventory volumeSnapshotInv) {
...
*/
    ...
    private void deleteVolumeSnapshot(final DeleteVolumeSnapshotMessage msg, Completion completion) {
        ...
        VolumeSnapshotDeletionStructs ctx = new VolumeSnapshotDeletionStructs(Collections.singletonList(getSelfInventory()), msg.isOnlySelf());
        FlowChain chain = FlowChainBuilder.newSimpleFlowChain();
