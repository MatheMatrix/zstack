...
public class VolumeSnapshotTreeBase {
    ...
    private ErrorCode isOperationAllowed(Message msg) {
        if (allowedStatus.isOperationAllowed(msg.getClass().getName(), currentRoot.getStatus().toString())) {
            ...
            return err(ORG_ZSTACK_STORAGE_SNAPSHOT_10010, VolumeSnapshotErrors.NOT_IN_CORRECT_STATE,
    ...
    private void refreshVO() {
        ...
        if (vo == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10011, "cannot find volume snapshot[uuid:%s, name:%s], it may have been deleted by previous operation",
    ...
    chain.then(new ShareFlow() {
        ...
        flow(new NoRollbackFlow() {
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10012, "snapshot or its desendant has reference volume[uuids:%s]", refVolUuids));
            }
        });
        flow(new NoRollbackFlow() {
    ...
    private void deleteSingleFlows() {
        flow(new NoRollbackFlow() {
            ...
            public void run(final FlowTrigger trigger, Map data) {
                if (volume == null) {
                    trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10013, "volume not found for volumeUuid: %s", msg.getVolumeUuid()));
                    return;
                }
                ...
                if (volume.getVmInstanceUuid() != null) {
                    ...
                    if (vmState != VmInstanceState.Running && vmState != VmInstanceState.Paused
                        ...
                        trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10014, "vm[uuid:%s] is not Running, Paused or Destroyed, Stopped, Destroying, " +
                            ...
                            flow(new NoRollbackFlow() {
                                ...
                                public void run(FlowTrigger trigger, Map data) {
                                    ...
                                    bus.send(smsg, new CloudBusCallBack(completion) {
                                        ...
                                        public void run(MessageReply reply) {
                                            if (!reply.isSuccess()) {
                                                trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10015, String.format("failed to get volume[uuid:%s, installPath:%s] size " +
                            ...
                            flow(new NoRollbackFlow() {
                                ...
                                public void run(FlowTrigger trigger, Map data) {
                                    ...
                                    bus.send(smsg, new CloudBusCallBack(completion) {
                                        ...
                                        public void run(MessageReply reply) {
                                            if (!reply.isSuccess()) {
                                                trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10016, String.format("failed to get volume[uuid:%s, installPath:%s] size on primary storage[uuid:%s], because %s",
    ...
    private void changeStatusOfSnapshots(final StatusEvent evt, final List<VolumeSnapshotInventory> snapshots, final Completion completion) {
        ...
        bus.send(msgs, new CloudBusListCallBack(completion) {
            ...
            public void run(List<MessageReply> replies) {
                ...
                if (err != null) {
                    completion.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10017, "failed to change status of volume snapshot[uuid:%s, name:%s] by status event[%s]",
...
//            handle((APIBackupVolumeSnapshotMsg) msg);
    ...
    // The logic for cleaning up snapshots when deleting a snapshot chain
...
*/
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        for (String vmUuid : volumeInventory.getAttachedVmUuids()) {
                            ...
                            if (state != VmInstanceState.Stopped) {
                                trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10018, "unable to reset volume[uuid:%s] to snapshot[uuid:%s]," +
    ...
    private void deleteVolumeSnapshot(final DeleteVolumeSnapshotMessage msg, Completion completion) {
        ...
        chain.done(new FlowDoneHandler(completion) {
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                completion.fail(err(ORG_ZSTACK_STORAGE_SNAPSHOT_10019, SysErrors.DELETE_RESOURCE_ERROR, errCode, errCode.getDetails()));
            }
        }).start();
    }
    ...
    private void markSnapshotAsVolume(VolumeInventory volume,long volumeInstallPathSize, String newInstallPath, long newInstallPathSize) {
                ...
                new SQLBatch() {
                    ...
                    protected void scripts() {
                        ...
                        sql(VolumeSnapshotVO.class).eq(VolumeSnapshotVO_.uuid, currentRoot.getUuid())
                                .set(VolumeSnapshotVO_.primaryStorageInstallPath, volume.getInstallPath())
                                .set(VolumeSnapshotVO_.size, volumeInstallPathSize)
                                .set(VolumeSnapshotVO_.latest, false)
                                .update();
                        ...
                        if (currentRoot.getParentUuid() == null) {
                            sql(VolumeSnapshotTreeVO.class).eq(VolumeSnapshotTreeVO_.uuid, currentRoot.getTreeUuid())
