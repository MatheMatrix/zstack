                        ...
                        bus.send(bmsg, new CloudBusCallBack(trigger) {
                            ...
                            private void markSnapshotAsVolume(VolumeVO volume, VolumeSnapshotInventory snapshotInv,
                                ...
                                new SQLBatch() {
                                    @Override
                                    protected void scripts() {
                                        sql(VolumeVO.class).eq(VolumeVO_.uuid, volume.getUuid()).set(VolumeVO_.installPath, newVolumeInstallPath).update();
                                        UpdateQuery sql = sql(VolumeSnapshotVO.class).eq(VolumeSnapshotVO_.uuid, snapshotInv.getUuid())
                                                .set(VolumeSnapshotVO_.primaryStorageInstallPath, volume.getInstallPath());
                                        if (snapshotInv.getParentUuid() != null) {
                                            sql.set(VolumeSnapshotVO_.latest, true);
                                        }
                                        sql.update();
                                        logger.debug(String.format("reset latest snapshot of tree[uuid:%s] to snapshot[uuid:%s]",
