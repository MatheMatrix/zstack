...
public class VolumeSnapshotTreeBase {
    ...
    private void deleteSingle(FlowChain chain, final VolumeSnapshotDeletionMsg msg, NoErrorCompletion completion) {
        ...
        if (volume.getVmInstanceUuid() != null) {
            ...
            if (vmInstanceState != VmInstanceState.Running && vmInstanceState != VmInstanceState.Paused &&
                ...
                reply.setError(operr("vm[uuid:%s] is not Running, Paused or Stopped, Destroyed, Destroying, " +
                        "current state[%s]", volume.getVmInstanceUuid(), vmInstanceState));
                bus.reply(msg, reply);
                return;
            }
        }
        ...
        VolumeTree.VolumeSnapshotDeletionContext context = currentVolumeTree.
                createDeletionContext(VolumeSnapshotInventory.valueOf(currentSnapshotVO), vmInstanceState, msg.getDirection());
                            ...
                            public void run(MessageReply reply) {
                                ...
                                if (Objects.equals(srcSnapshotInv.getUuid(), volume.getUuid())) {
                                    if (srcSnapshotInv.getParentUuid() != null) {
                                        SQL.New(VolumeSnapshotVO.class).eq(VolumeSnapshotVO_.uuid, srcSnapshotInv.getParentUuid())
                                                .set(VolumeSnapshotVO_.latest, true).update();
                                        logger.debug(String.format("reset latest snapshot of tree[uuid:%s] to snapshot[uuid:%s]",
                                ...
                                if (srcSnapshotInv.getParentUuid() != null) {
                                    ...
                                    logger.debug(String.format("update the parent of snapshot[uuid:%s] to %s",
                                            dstSnapshotInv.getUuid(), srcSnapshotInv.getParentUuid()));
                                }

                                SQL.New(VolumeSnapshotVO.class).eq(VolumeSnapshotVO_.uuid, dstSnapshotInv.getUuid())
                                        .set(VolumeSnapshotVO_.size, newInstallPathSize).update();
                                logger.debug(String.format("update the size of snapshot[uuid:%s] to %s",
                                        pullMsg.getSrcSnapshot().getUuid(), newInstallPathSize));
