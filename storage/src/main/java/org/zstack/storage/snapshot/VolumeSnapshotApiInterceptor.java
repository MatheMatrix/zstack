...
//            validate((APIBackupVolumeSnapshotMsg) msg);
    ...
    private void validate(APIRevertVmFromSnapshotGroupMsg msg) {
        ...
        if (!disabledSnapshotUuids.isEmpty()) {
            ...
            ORG_ZSTACK_STORAGE_SNAPSHOT_10000,         "volume snapshot[uuids:%s] is in state Disabled, cannot revert volume to it", disabledSnapshotUuids)
            );
        }
        ...
        if (isWithMemoryForSnapshotGroup(group)
                ...
                .isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_SNAPSHOT_10001, "Can not take memory snapshot, expected vm states are [%s, %s]",
...
*/
    ...
    private void validate(APIRevertVolumeFromSnapshotMsg msg) {
        ...
        if (state != VolumeSnapshotState.Enabled) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10002, "volume snapshot[uuid:%s] is in state %s, cannot revert volume to it", msg.getUuid(), state));
        }

        String volUuid = t.get(1, String.class);
        if (volUuid == null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10003, "original volume for snapshot[uuid:%s] has been deleted, cannot revert volume to it", msg.getUuid()));
        }
    }
    ...
    private void validate(APIBatchDeleteVolumeSnapshotMsg msg) {
        ...
        for (VolumeSnapshotVO snapshotVO : snapshotVOS) {
            if (msg.getVolumeUuid() == null) {
                ...
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10004, "not support delete snapshots on different volumes[uuid: %s, %s]", msg.getVolumeUuid(), snapshotVO.getVolumeUuid()));
            }
        }
        if (msg.getVolumeUuid() == null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10005, "can not find volume uuid for snapshosts[uuid: %s]", msg.getUuids()));
        }
    }
}
