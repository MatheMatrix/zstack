...
public class DeleteVolumeSnapshotLongJob implements LongJob {
    ...
    private ErrorCode checkIfVolumeSnapshotSupportCancel(String volumeUuid) {
        ...
        if (volume == null) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10006, "failed to cancel deletion job. Volume[uuid:%s] not exists.");
        }

        if (volume.getVmInstanceUuid() == null) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10007, "failed to cancel deletion job. Volume[uuid:%s] not attached to any vm," +
        ...
        if (vmInstance == null) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10008, "failed to cancel deletion job. Volume[uuid:%s] attached vm not exists," +
                    " offline snapshot deletion do not support cancel.");
        }

        if (!vmInstance.getState().equals(VmInstanceState.Running)) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10009, "failed to cancel deletion job. Volume[uuid:%s] attached vm not in state %s" +
