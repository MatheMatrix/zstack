...
public class DeleteVolumeSnapshotLongJob implements LongJob {
    ...
    private ErrorCode checkIfVolumeSnapshotSupportCancel(String volumeUuid) {
        ...
        if (volume == null) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10016, "failed to cancel deletion job. Volume[uuid:%s] not exists.");
        }

        if (volume.getVmInstanceUuid() == null) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10017, "failed to cancel deletion job. Volume[uuid:%s] not attached to any vm," +
        ...
        if (vmInstance == null) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10018, "failed to cancel deletion job. Volume[uuid:%s] attached vm not exists," +
                    " offline snapshot deletion do not support cancel.");
        }

        if (!vmInstance.getState().equals(VmInstanceState.Running)) {
            return Platform.operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10019, "failed to cancel deletion job. Volume[uuid:%s] attached vm not in state %s" +
