...
public class VolumeSnapshotManagerImpl extends AbstractService implements
    ...
    private void passThrough(VolumeSnapshotMessage msg) {
        ...
        if (vo == null) {
            throw new OperationFailureException(err(ORG_ZSTACK_STORAGE_SNAPSHOT_10020, SysErrors.RESOURCE_NOT_FOUND,
    ...
    private void passThrough(VolumeSnapshotGroupMessage msg) {
        ...
        if (vo == null) {
            throw new OperationFailureException(err(ORG_ZSTACK_STORAGE_SNAPSHOT_10021, SysErrors.RESOURCE_NOT_FOUND,
...
*/
    ...
    private VolumeSnapshotStruct saveChainTypeSnapshot(VolumeSnapshotVO vo, SnapshotMode mode) {
        ...
        if (!CoreGlobalProperty.UNIT_TEST_ON) {
            if (maxIncrementalSnapshotNum <= 1) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10022, "Unsupported maximum snapshot number (%d) for volume [uuid:%s]",
    ...
    private VolumeSnapshotStruct getVolumeSnapshotStruct(CreateVolumeSnapshotMsg msg) {
        ...
        if (!reply.isSuccess()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10023, "cannot ask primary storage[uuid:%s] for volume snapshot capability, see detail [%s]", vol.getUuid(),reply.getError()));
        }
        ...
        if (!capability.isSupport()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10024, "primary storage[uuid:%s] doesn't support volume snapshot;" +
    ...
    private void handle(MarkRootVolumeAsSnapshotMsg msg) {
        ...
        if (psType == null) {
            ret.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10025, "cannot find type for primaryStorage [%s]", vol.getPrimaryStorageUuid()));
            bus.reply(msg, ret);
            return;
        }
            ...
            public void setup() {
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        MessageReply reply = bus.call(askMsg);
                        if (!reply.isSuccess()) {
                            ret.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10026, reply.getError(),
                        ...
                        if (!capability.isSupport()) {
                            ret.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10027, "primary storage[uuid:%s] doesn't support volume snapshot;" +
                                    " cannot create snapshot for volume[uuid:%s]", primaryStorageUuid, vol.getUuid()));
                            bus.reply(msg, ret);
                            trigger.fail(ret.getError());
                            return;
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (vo == null) {
                            trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10028, "cannot find snapshot: %s", uuid));
                            return;
                        }
    ...
    private void handle(APIGetMemorySnapshotGroupReferenceMsg msg) {
        ...
        reply.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10029, "this resource type %s does not support querying memory snapshot references", msg.getResourceType()));
        bus.reply(msg, reply);
    }
