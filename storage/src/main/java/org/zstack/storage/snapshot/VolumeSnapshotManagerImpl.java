...
public class VolumeSnapshotManagerImpl extends AbstractService implements
    ...
    private void passThrough(VolumeSnapshotMessage msg) {
        ...
        if (vo == null) {
            throw new OperationFailureException(err(ORG_ZSTACK_STORAGE_SNAPSHOT_10007, SysErrors.RESOURCE_NOT_FOUND,
    ...
    private void passThrough(VolumeSnapshotGroupMessage msg) {
        ...
        if (vo == null) {
            throw new OperationFailureException(err(ORG_ZSTACK_STORAGE_SNAPSHOT_10008, SysErrors.RESOURCE_NOT_FOUND,
...
*/
    ...
    private VolumeSnapshotStruct saveChainTypeSnapshot(VolumeSnapshotVO vo, SnapshotMode mode) {
        ...
        if (!CoreGlobalProperty.UNIT_TEST_ON) {
            if (maxIncrementalSnapshotNum <= 1) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10009, "Unsupported maximum snapshot number (%d) for volume [uuid:%s]",
    ...
    private VolumeSnapshotStruct getVolumeSnapshotStruct(CreateVolumeSnapshotMsg msg) {
        ...
        if (!reply.isSuccess()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10010, "cannot ask primary storage[uuid:%s] for volume snapshot capability, see detail [%s]", vol.getUuid(),reply.getError()));
        }
        ...
        if (!capability.isSupport()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10011, "primary storage[uuid:%s] doesn't support volume snapshot;" +
        ...
        chain.then(new ShareFlow() {
            ...
            public void setup() {
                ...
                flow(new NoRollbackFlow() {
                    ...
                    @Override
                    public void run(FlowTrigger trigger, Map data) {
                        if (snapshot.getSize() == volumeSize) {
                            trigger.next();
                            return;
                        }

                        if (snapshot.getSize() - volumeSize > 0) {
                            logger.debug(String.format("reserving primary storage capacity: " +
                                            "snapshot[uuid:%s, size:%s] is larger than Volume[uuid:%s, size:%s]",
                                    snapshot.getUuid(), snapshot.getSize(), vol.getUuid(), volumeSize));
                            AllocatePrimaryStorageSpaceMsg amsg = new AllocatePrimaryStorageSpaceMsg();
                            amsg.setRequiredPrimaryStorageUuid(vol.getPrimaryStorageUuid());
                            ...
                            bus.send(amsg, new CloudBusCallBack(trigger) {
                                ...
                                public void run(MessageReply reply) {
                                    ...
                                    allocatedSize = snapshot.getSize();
                                    trigger.next();
                                }
                            });
                            return;
                        ...
                        if (volumeSize - snapshot.getSize() > 0) {
                            logger.debug(String.format("releasing primary storage capacity: " +
                                            "snapshot[uuid:%s, size:%s] is smaller than Volume[uuid:%s, size:%s]",
                                    snapshot.getUuid(), snapshot.getSize(), vol.getUuid(), volumeSize));
                            ReleasePrimaryStorageSpaceMsg rmsg = new ReleasePrimaryStorageSpaceMsg();
                            rmsg.setPrimaryStorageUuid(vol.getPrimaryStorageUuid());
                            rmsg.setDiskSize(volumeSize - snapshot.getSize());
                            rmsg.setNoOverProvisioning(true);
                            rmsg.setAllocatedInstallUrl(allocatedInstall);
                            bus.makeTargetServiceIdByResourceUuid(rmsg, PrimaryStorageConstant.SERVICE_ID, vol.getPrimaryStorageUuid());
                            bus.send(rmsg);
    ...
    private void handle(MarkRootVolumeAsSnapshotMsg msg) {
        ...
        if (psType == null) {
            ret.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10012, "cannot find type for primaryStorage [%s]", vol.getPrimaryStorageUuid()));
            bus.reply(msg, ret);
            return;
        }
            ...
            public void setup() {
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        MessageReply reply = bus.call(askMsg);
                        if (!reply.isSuccess()) {
                            ret.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10013, reply.getError(),
                        ...
                        if (!capability.isSupport()) {
                            ret.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10014, "primary storage[uuid:%s] doesn't support volume snapshot;" +
                                    " cannot create snapshot for volume[uuid:%s]", primaryStorageUuid, vol.getUuid()));
                            bus.reply(msg, ret);
                            trigger.fail(ret.getError());
                            return;
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (vo == null) {
                            trigger.fail(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10015, "cannot find snapshot: %s", uuid));
                            return;
                        }
    ...
    private void handle(APIGetMemorySnapshotGroupReferenceMsg msg) {
        ...
        reply.setError(operr(ORG_ZSTACK_STORAGE_SNAPSHOT_10016, "this resource type %s does not support querying memory snapshot references", msg.getResourceType()));
        bus.reply(msg, reply);
    }
