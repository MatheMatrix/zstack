...
public class ImageManagerImpl extends AbstractService implements ImageManager, ManagementNodeReadyExtensionPoint,
                ...
                flow(new Flow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (CollectionUtils.isEmpty(volumes)) {
                            trigger.fail(operr(ORG_ZSTACK_IMAGE_10020, "vm instance[uuid:%s] has no volume", vmUuid));
                            return;
                        }
                        ...
                        if (rootVolumeUuid == null) {
                            trigger.fail(operr(ORG_ZSTACK_IMAGE_10021, "vm instance[uuid:%s] has no root volume", vmUuid));
                            return;
                        }
    ...
    private void passThrough(ImageMessage msg) {
        ...
        if (vo == null) {
            ErrorCode err = err(ORG_ZSTACK_IMAGE_10022, SysErrors.RESOURCE_NOT_FOUND, "Cannot find image[uuid:%s], it may have been deleted", msg.getImageUuid());
    ...
    private void handle(final APICloneImageMsg msg) {
        ...
        if (sourceImage == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_IMAGE_10023, "source image [%s] not existed", msg.getImageUuid()));
        }
        if (sourceImage.getStatus() != ImageStatus.Ready) {
            throw new OperationFailureException(operr(ORG_ZSTACK_IMAGE_10024, "cannot clone image, because image [%s] status is not ready", msg.getImageUuid()));
        }
        ...
        class BootModeValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            ...
            public void validateSystemTagInCreateMessage(APICreateMessage msg) {
                ...
                for (String systemTag : msg.getSystemTags()) {
                    if (ImageSystemTags.BOOT_MODE.isMatch(systemTag)) {
                        if (++bootModeCount > 1) {
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10025, "only one bootMode system tag is allowed, but %d got", bootModeCount));
                        }
            ...
            private void validateBootMode(String systemTag, String bootMode) {
                ...
                if (!valid) {
                    ...
                    ORG_ZSTACK_IMAGE_10026,         "[%s] specified in system tag [%s] is not a valid boot mode", bootMode, systemTag)
                    );
                }
            }
        }
    ...
    private void handleAddImageMsg(AddImageLongJobData msgData, Message evt) {
        ...
        new LoopAsyncBatch<DownloadImageMsg>(msgData.getNeedReplyMessage()) {
            ...
            protected void done() {
                ...
                if (vo == null) {
                    event.error = (operr(ORG_ZSTACK_IMAGE_10027, "image [uuid:%s] has been deleted", ivo.getUuid()));
                ...
                if (resultMap.entrySet().stream().allMatch(entry -> entry.getValue().get())) {
                    ...
                    final ErrorCode err;
                    if (errors.isEmpty()) {
                        err = err(ORG_ZSTACK_IMAGE_10028, SysErrors.CREATE_RESOURCE_ERROR, "Failed to download image[name:%s] on all backup storage%s.",
                                inv.getName(), msgData.getBackupStorageUuids());
                    } else {
                        err = err(ORG_ZSTACK_IMAGE_10029, SysErrors.CREATE_RESOURCE_ERROR, errors.get(0), "Failed to download image[name:%s] on all backup storage%s.",
                                inv.getName(), msgData.getBackupStorageUuids());
                    }
                            ...
                            bus.send(amsgs, new CloudBusListCallBack(trigger) {
                                ...
                                public void run(List<MessageReply> replies) {
                                    ...
                                    if (targetBackupStorages.isEmpty()) {
                                        trigger.fail(operr(ORG_ZSTACK_IMAGE_10030, "unable to allocate backup storage specified by uuids%s, list errors are: %s",
                        ...
                        bus.send(cmsgs, new CloudBusListCallBack(trigger) {
                            ...
                            public void run(List<MessageReply> replies) {
                                ...
                                for (MessageReply r : replies) {
                                    ...
                                    if (dbf.reload(imageVO) == null) {
                                        SQL.New("delete from ImageBackupStorageRefVO where imageUuid = :uuid")
                                                ...
                                                .execute();
                                        trigger.fail(operr(ORG_ZSTACK_IMAGE_10031, "image [uuid:%s] has been deleted", imageVO.getUuid()));
                                        return;
                                    }
                                ...
                                if (success) {
                                    ...
                                    trigger.fail(operr(ORG_ZSTACK_IMAGE_10032, "failed to create image from root volume[uuid:%s] on all backup storage, see cause for one of errors",
                            ...
                            bus.send(amsg, new CloudBusCallBack(trigger) {
                                ...
                                public void run(MessageReply reply) {
                                    if (reply.isSuccess()) {
                                        ...
                                        trigger.fail(operr(ORG_ZSTACK_IMAGE_10033, "cannot find proper backup storage").causedBy(reply.getError()));
                                    }
                                }
                            });
                        } else {
                            ...
                            bus.send(amsgs, new CloudBusListCallBack(trigger) {
                                ...
                                public void run(List<MessageReply> replies) {
                                    ...
                                    if (backupStorages.isEmpty()) {
                                        trigger.fail(operr(ORG_ZSTACK_IMAGE_10034, "failed to allocate all backup storage[uuid:%s], a list of error: %s",
                                                msgData.getBackupStorageUuids(), JSONObjectUtil.toJsonString(errs)));
                                    } else {
                                        saveRefVOByBsInventorys(backupStorages, image.getUuid());
                                        trigger.next();
                        ...
                        bus.send(cmsgs, new CloudBusListCallBack(trigger) {
                            ...
                            public void run(List<MessageReply> replies) {
                                ...
                                if (fail == backupStorageNum) {
                                    ErrorCode errCode = operr(ORG_ZSTACK_IMAGE_10035, "failed to create data volume template from volume[uuid:%s] on all backup storage%s. See cause for one of errors",
