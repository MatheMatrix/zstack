...
public class ImageBase implements Image {
    ...
    private void syncImageSize(String backupStorageUuid, final ReturnValueCompletion<ImageSize> completion) {
        if (backupStorageUuid == null) {
            ...
            if (bsUuids.isEmpty()) {
                throw new OperationFailureException(operr(ORG_ZSTACK_IMAGE_10036, "the image[uuid:%s, name:%s] is not on any backup storage", self.getUuid(), self.getName()));
            }
            ...
            if (backupStorageUuid == null) {
                completion.fail(operr(ORG_ZSTACK_IMAGE_10037, "No connected backup storage found for image[uuid:%s, name:%s]",
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                new While<>(detachIsoFromVmInstanceMsgs).all((detachIsoFromVmInstanceMsg, completion) -> {
                    ...
                    public void done(ErrorCodeList errorCodeList) {
                        if (errors.size() != 0) {
                            trigger.fail(operr(ORG_ZSTACK_IMAGE_10038, "detach iso[uuid=%s] from vm failed, errors are %s"
    ...
    private void handle(APIRecoverImageMsg msg) {
        ...
        if (msg.getBackupStorageUuids() == null || msg.getBackupStorageUuids().isEmpty()) {
            ...
            if (toRecoverBsUuids.isEmpty()) {
                throw new OperationFailureException(operr(ORG_ZSTACK_IMAGE_10039, "the image[uuid:%s, name:%s] is not deleted on any backup storage",
                                self.getUuid(), self.getName()));
            }
        } else {
            ...
            for (final String bsUuid : msg.getBackupStorageUuids()) {
                ...
                if (ref == null) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_IMAGE_10040, "the image[uuid:%s, name:%s] is not on the backup storage[uuid:%s]",
                                    self.getUuid(), self.getName(), bsUuid));
                }

                if (ref.getStatus() != ImageStatus.Deleted) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_IMAGE_10041, "the image[uuid:%s, name:%s]'s status[%s] is not Deleted on the backup storage[uuid:%s]",
    ...
    private void handle(final APIExpungeImageMsg msg) {
        ...
        if (msg.getBackupStorageUuids() == null || msg.getBackupStorageUuids().isEmpty()) {
            ...
            if (bsUuids.isEmpty()) {
                throw new OperationFailureException(operr(ORG_ZSTACK_IMAGE_10042, "the image[uuid:%s, name:%s] is not deleted on any backup storage",
                        self.getUuid(), self.getName()));
            }
        } else {
            for (final String bsUuid : msg.getBackupStorageUuids()) {
                ...
                if (ref == null) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_IMAGE_10043, "the image[uuid:%s, name:%s] is not on the backup storage[uuid:%s]",
                            self.getUuid(), self.getName(), bsUuid));
                }

                if (ref.getStatus() != ImageStatus.Deleted) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_IMAGE_10044, "the image[uuid:%s, name:%s] is not deleted on the backup storage[uuid:%s]",
                            self.getUuid(), self.getName(), bsUuid));
                }
    ...
    private void handle(APIDeleteImageMsg msg) {
        ...
        chain.done(new FlowDoneHandler(msg) {
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                evt.setError(err(ORG_ZSTACK_IMAGE_10045, SysErrors.DELETE_RESOURCE_ERROR, errCode, errCode.getDetails()));
                bus.publish(evt);
            }
        }).start();
    }
