...
public class ImageApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APISetImageBootModeMsg msg){
        ...
        if (ImageBootMode.Legacy.toString().equals(msg.getBootMode())
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10000, "The aarch64 architecture does not support legacy."));
        }
    }
    ...
    private void validate(APICreateDataVolumeTemplateFromVolumeMsg msg) {
        ...
        if (VolumeStatus.Ready != vol.getStatus()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_IMAGE_10001, "volume[uuid:%s] is not Ready, it's %s", vol.getUuid(), vol.getStatus()));
        }

        if (VolumeState.Enabled != vol.getState()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_IMAGE_10002, "volume[uuid:%s] is not Enabled, it's %s", vol.getUuid(), vol.getState()));
        }
    }
    ...
    private void validate(APICreateDataVolumeTemplateFromVolumeSnapshotMsg msg) {
        ...
        if (VolumeSnapshotStatus.Ready != vsvo.getStatus()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_IMAGE_10003, "volume snapshot[uuid:%s] is not Ready, it's %s", vsvo.getUuid(), vsvo.getStatus()));
        }

        if (VolumeSnapshotState.Enabled != vsvo.getState()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_IMAGE_10004, "volume snapshot[uuid:%s] is not Enabled, it's %s", vsvo.getUuid(), vsvo.getState()));
        }
    }
    ...
    private void validate(APIAddImageMsg msg) {
        ...
        if (msg.isSystem() && (ImageMediaType.ISO.toString().equals(msg.getMediaType()) || ImageConstant.ISO_FORMAT_STRING.equals(msg.getFormat()))) {
            ...
            ORG_ZSTACK_IMAGE_10005,         "ISO cannot be used as system image"
            ));
        }

        if (!VolumeFormat.hasType(msg.getFormat())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10006, "unknown format[%s]", msg.getFormat()));
        }

        if (msg.getType() != null && !ImageType.hasType(msg.getType())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10007, "unsupported image type[%s]", msg.getType()));
        }
        ...
        if (msg.getBackupStorageUuids() != null) {
            ...
            if (bsUuids.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_IMAGE_10008, "no backup storage specified in uuids%s is available for adding this image; they are not in status %s or not in state %s, or the uuid is invalid backup storage uuid",
        ...
        if (msg.getUrl().startsWith("/")) {
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10009, "url must starts with 'file:///', 'http://', 'https://'， 'ftp://', 'sftp://' or '/'"));
        }
    ...
    private void validateLocalPath(String url) {
        ...
        if (!path.startsWith("/")) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10010, "absolute path must be used", path));
        }
    ...
    private static void initPathValidatorsFromConfig() {
        ...
        PathValidator blackPathValidator = new PathValidator() {
            ...
            void validate(String path) {
                ...
                if (inBlackList) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10011, "image path [%s] is in black list %s", path, blackList.value()));
                }
            }
        };
        PathValidator whitePathValidator = new PathValidator() {
            ...
            void validate(String path) {
                ...
                if (StringUtils.isBlank(whiteList.value())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10012, "all images on this server cannot be used"));
                }
                String[] wl = whiteList.value().split(";");
                boolean inWhiteList = Arrays.stream(wl).anyMatch(pattern -> antPathMatcher.match(pattern, path));
                if (!inWhiteList) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IMAGE_10013, "image path is not in white list: %s", whiteList.value()));
                }
            }
        };
