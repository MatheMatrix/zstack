            ...
            if (value instanceof Collection) {
                    ...
                    if (uuids.size() != col.size()) {
                        ...
                        if (!invalids.isEmpty()) {
                            throw new ApiMessageInterceptionException(err(SysErrors.RESOURCE_NOT_FOUND,
                                    "invalid field[%s] for api[%s], resource[uuids:%s, type:%s] not found",
                                    f.getName(), msg.getMessageName(), invalids, at.resourceType().getSimpleName()));
                        }
                    }
                }
                ...
                if (!dbf.isExist(value, at.resourceType())) {
                    if (at.successIfResourceNotExisting()) {
                        ...
                        throw new ApiMessageInterceptionException(err(SysErrors.RESOURCE_NOT_FOUND,
                                "invalid field[%s] for api[%s], resource[uuid:%s, type:%s] not found",
                                f.getName(), msg.getMessageName(), value, at.resourceType().getSimpleName()));
                    }
                }
            }
        }
