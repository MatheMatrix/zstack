...
public abstract class AbstractConsoleProxyBackend implements ConsoleBackend, Component, ManagementNodeReadyExtensionPoint {
    ...
    public void grantConsoleAccess(final SessionInventory session, final VmInstanceInventory vm, final ReturnValueCompletion<ConsoleInventory> complete) {
        if (!isAgentConnected()) {
            complete.fail(operr(ORG_ZSTACK_CONSOLE_10010, "the console agent is not connected; it's mostly like the management node just starts, " +
        ...
        if (hostIp == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_CONSOLE_10011, "cannot find host IP of the vm[uuid:%s], is the vm running???", vm.getUuid()));
        }
