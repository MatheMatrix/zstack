...
public class CephPrimaryStorageMonBase extends CephMonBase {
    ...
    public void changeStatus(MonStatus status) {
        ...
        if (self == null) {
            ...
            ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10046,         "cannot update status of the ceph primary storage mon[uuid:%s], it has been deleted." +
                    ...
                    flow(new NoRollbackFlow() {
                        ...
                        public void run(FlowTrigger trigger, Map data) {
                            ...
                            try {
                                ...
                                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10047, ex.toString()));
                            }
        ...
        restf.asyncJsonPost(CephAgentUrl.primaryStorageUrl(self.getHostname(), path),
                cmd, new JsonAsyncRESTCallback<T>(completion) {
                    ...
                    public void success(T ret) {
                        if (!ret.isSuccess()) {
                            completion.fail(Platform.operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10048, "operation error, because:%s", ret.getError()));
                            return;
                        }
    ...
    private void doPing(final ReturnValueCompletion<PingResult> completion) {
        ...
        String poolName = Q.New(CephPrimaryStoragePoolVO.class)
                ...
                .findValue();
        if (poolName == null) {
            completion.fail(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10049, "Ceph ps[uuid=%s] root pool name not found", primaryStorageUuid));
            return;
        }
