...
public class CephPrimaryStorageFactory implements PrimaryStorageFactory, CephCapacityUpdateExtensionPoint, KVMStartVmExtensionPoint,
    ...
    private IsoTO convertIsoToCephIfNeeded(final IsoTO to) {
        ...
        if (cto.getMonInfo().isEmpty()) {
            ...
            ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10034,         "cannot find any Connected ceph mon for the primary storage[uuid:%s]", pri.getUuid()
            ));
        }
    ...
    private VolumeTO convertVolumeToCephIfNeeded(VolumeInventory vol, VolumeTO to) {
        ...
        if (ts.isEmpty() || ts.stream().noneMatch(t -> t.get(2, MonStatus.class) == MonStatus.Connected)) {
            throw new OperationFailureException(operr(
            ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10036,         "cannot find any Connected ceph mon for the primary storage[uuid:%s]", vol.getPrimaryStorageUuid())
            );
        }
    ...
    public void beforeTakeLiveSnapshotsOnVolumes(CreateVolumesSnapshotOverlayInnerMsg msg, TakeVolumesSnapshotOnKvmMsg otmsg, Map flowData, Completion completion) {
        ...
        if (otmsg.getSnapshotJobs().isEmpty()) {
            ...
            completion.fail(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10037, "not support take volumes snapshots " +
    ...
    private void settingRootVolume(CreateVmInstanceMsg msg) {
            ...
            if (config.getAllocate() != null && config.getAllocate().getPrimaryStorage() != null) {
                ...
                if (config.getAllocate().getPrimaryStorage() instanceof CephPrimaryStorageAllocateConfig) {
                    ...
                    if (cephPoolName != null && !cephPoolName.equals(targetCephPoolName)) {
                        throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10038, "ceph pool conflict, the ceph pool specified by the instance offering is %s, and the ceph pool specified in the creation parameter is %s"
        ...
        String targetCephPoolName = primaryStorageAllocateConfig.getPoolNames().get(0);
        if (cephPoolName != null && !cephPoolName.equals(targetCephPoolName)) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10039, "ceph pool conflict, the ceph pool specified by the disk offering is %s, and the ceph pool specified in the creation parameter is %s"
                    ,targetCephPoolName, cephPoolName));
        }
    ...
    public void preInstantiateVmResource(VmInstanceSpec spec, Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                if (!reply.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10041, "get rootVolume[%s] rbd image watchers fail, %s",
                ...
                String installPath = Q.New(VolumeVO.class)
                        ...
                        .findValue();
                completion.fail(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10042, "rootVolume[%s] is already in use(ceph rbd image[%s] already has watchers), in order to prevent brain splitting, Starting VM is prohibited.",
    ...
    private CephPrimaryStoragePoolVO getPoolFromPoolName(String poolName, String psUuid, String poolType) {
        ...
        if (poolVOS.size() == 0) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10043, "cannot find cephPrimaryStorage pool[poolName=%s]", poolName));
        }
    ...
    private void checkCephPoolCapacityForNewVolume(String poolName, long volumeSize, String psUuid) {
        ...
        if (!new CephOsdGroupCapacityHelper(psUuid).checkVirtualSizeByRatio(poolVO.getUuid(), volumeSize)) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10044, "cephPrimaryStorage pool[poolName=%s] available virtual capacity not enough for size %s",
                    poolName, volumeSize));
        }
    }
    ...
    private String getPreAllocatedInstallUrl(AllocatePrimaryStorageSpaceMsg msg, String psUuid) {
        ...
        throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_CEPH_PRIMARY_10045, "cannot allocate pool for primaryStorage[%s], purpose: %s", psUuid, purpose));
    }
