...
public class CephBackupStorageBase extends BackupStorageBase {
    ...
    public static class CopyFileProgressCmd extends AgentCommand implements Serializable {
        public String targetFilePath;
        public String dstPath;
        public int dstHostSSHPort;
        public String dstHostSSHUser;
        @NoLogging
        public String dstHostSSHPasswd;
        public String dstHostIP;
    }
    ...
    public static final String COPY_FILE_TO_HOST_PATH = "/ceph/file/copytohost";
    ...
    protected void handle(final DeleteFilesOnBackupStorageHostMsg msg) {
        ...
        monBase.httpCall(DELETE_FILE_PATH, cmd, DownloadFileResponse.class, new ReturnValueCompletion<DownloadFileResponse>(msg) {
            @Override
            public void fail(ErrorCode err) {
                reply.setError(err);
                bus.reply(msg, reply);
    ...
    protected void handle(CopyAndExecuteSoftwareUpgradePackageMsg msg) {
        CopyAndExecuteSoftwareUpgradePackageReply reply = new CopyAndExecuteSoftwareUpgradePackageReply();
        ...
        cmd.targetFilePath = msg.getTargetFilePath();
        cmd.dstPath = msg.getDstFilePath();
        cmd.dstHostSSHPort = msg.getDstHostSSHPort();
        cmd.dstHostSSHUser = msg.getDstHostSSHUser();
        cmd.dstHostSSHPasswd = msg.getDstHostSSHPasswd();
        cmd.dstHostIP = msg.getDstHostIP();
        CephPrimaryStorageMonBase monBase = transform(Collections.singletonList(mon), CephPrimaryStorageMonBase::new).get(0);
        monBase.httpCall(COPY_FILE_TO_HOST_PATH, cmd, CopyFileProgressResponse.class, new ReturnValueCompletion<CopyFileProgressResponse>(msg) {
            @Override
            public void fail(ErrorCode err) {
                reply.setError(err);
                bus.reply(msg, reply);
