...
public class CephBackupStorageBase extends BackupStorageBase {
    ...
    protected void handle(final UploadFileToBackupStorageHostMsg msg) {
        ...
        if (msg.getUrl() == null) {
            reply.setError(operr("url cannot be null"));
            bus.reply(msg, reply);
            return;
        }
    ...
    protected void handle(SoftwareUpgradePackageDeployMsg msg) {
        ...
        if (msg.getTargetHostSshPort() <= 0) {
            reply.setError(operr("targetHostSshPort must be positive, but got [%d]", msg.getTargetHostSshPort()));
            bus.reply(msg, reply);
            return;
        }
