...
public class XmlHookManagerImpl extends AbstractService implements XmlHookManager, Component,
    ...
    public String preUpdateClusterOSCheck(ClusterVO cls, String updatePackages, Boolean force) {
        if (!force) {
            if (updatePackages.contains("qemu-kvm-ev") || updatePackages.contains("qemu-kvm") || updatePackages.contains("libvirt")) {
                //Determine whether the VM in the cluster is bound to XML hook
                List<VmInstanceVO> vms = Q.New(VmInstanceVO.class).eq(VmInstanceVO_.clusterUuid, cls.getUuid()).list();
                if (vms != null && vms.size() != 0) {
                    ...
                    if (vmList.size() != 0) {
                        return String.format("detect %s vms in cluster[uuid: %s] have been set XML hook, " +
                                ...
                                "and if so, add param [force=true] with the api", vmList.size(), cls.getUuid());
                    }
                }
            }
        }
