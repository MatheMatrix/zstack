    ...
    public void generateConsoleUrl(final VmInstanceInventory vm, final ReturnValueCompletion<ConsoleUrl> complete) {
        ...
        bus.send(msg, new CloudBusCallBack(complete) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    complete.fail(operr(ORG_ZSTACK_KVM_10159, "operation error, because:%s", rsp.getError()));
                    return;
                }

                if (rsp.getPort() < 0) {
                    complete.fail(operr(ORG_ZSTACK_KVM_10160, "unexpected VNC port number[%d] for VM [uuid:%s]", rsp.getPort(), vm.getUuid()));
                    return;
                }
                ...
                try {
                    ...
                    complete.fail(inerr(ORG_ZSTACK_KVM_10161, e.getMessage()));
                }
            }
        });
    }
