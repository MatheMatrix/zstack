...
public class KVMApiInterceptor implements ApiMessageInterceptor, GlobalApiMessageInterceptor {
    ...
    private void validate(APIExpungeVmUserDefinedXmlHookScriptMsg msg) {
        ...
        if (refVOs != null && refVOs.size() > 0) {
            ...
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_KVM_10133, "the xml hook[%s] has been set to vm %s," +
    ...
    private void validate(APIUpdateVmUserDefinedXmlHookScriptMsg msg) {
        ...
        if (StringUtils.isNotEmpty(name)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_KVM_10134, "the xml hook name[%s] already exists", msg.getName()));
        }
    }
    ...
    private static void failIfChangeSystemDefinedHook(XmlHookMessage msg) {
        ...
        if (XmlHookType.System.equals(vo.getType())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_KVM_10135, "System-type xml hooks are not allowed to be operated"));
        }
    }
    ...
    private void validate(APICreateVmUserDefinedXmlHookScriptMsg msg) {
        ...
        if (StringUtils.isNotEmpty(name)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_KVM_10136, "the xml hook name[%s] already exists", msg.getName()));
        }
    }
    ...
    private void validate(APIAddKVMHostMsg msg) {
        ...
        if (q.isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_KVM_10137, "there has been a kvm host having management ip[%s]", msg.getManagementIp()));
        }
    }
    ...
    private void validate(APIAttachL2NetworkToClusterMsg msg) {
        ...
        if (NetworkUtils.generateVlanDeviceName(l2.getPhysicalInterface(), l2.getVlan()).length()
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_KVM_10138, "cannot create vlan-device on %s because it's too long"
