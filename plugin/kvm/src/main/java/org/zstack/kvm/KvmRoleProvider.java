...
public class KvmRoleProvider implements PhysicalServerRoleProvider {
    ...
    /**
     * Returns the used CPU / memory for this KVM host by reading the already-aggregated
     * {@code HostCapacityVO}. {@code HostCapacityVO.getUsedCpu() / getUsedMemory()} compute
     * {@code total − available} from the ledger that existing VM start/stop/migrate flows
     * keep current — so this provider never re-aggregates {@code VmInstanceVO} and stays at
     * the Host/HCV abstraction layer.
     *
     * <p>If HCV is missing for the given host (e.g. a transient state mid-AddHost before the
     * InitPhysicalServerCapacityFlow in U11 has persisted the row), returns zero consumption
     * with a warn log — the next recalculate tick will converge once HCV appears.
     */
    @Override
    public CapacityUsage getCapacityConsumption(String serverUuid, String roleUuid) {
        HostCapacityVO hcv = Q.New(HostCapacityVO.class)
                .eq(HostCapacityVO_.uuid, roleUuid)
                .find();
        if (hcv == null) {
            logger.warn(String.format("[KvmRoleProvider] HostCapacityVO not found for host roleUuid=%s; " +
                    "returning zero capacity consumption.", roleUuid));
            return new CapacityUsage();
        }
        CapacityUsage usage = new CapacityUsage();
        usage.setUsedCpu(hcv.getUsedCpu());
        usage.setUsedMemory(hcv.getUsedMemory());
        return usage;
    }
