    ...
    private void incrementallyApplyRules(final HostRuleTO hto, final Completion complete) {
        ...
        bus.send(msg, new CloudBusCallBack(complete) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_KVM_10147, "failed to apply rules of security group rules to kvm host[uuid:%s], because %s", hto.getHostUuid(), rsp.getError());
                    complete.fail(err);
                    return;
                }
    ...
    private void reApplyAllRulesOnHost(final HostRuleTO hto, final Completion complete) {
        ...
        bus.send(msg, new CloudBusCallBack(complete) {
            ...
            public void run(MessageReply reply) {
                ...
                ApplySecurityGroupRuleResponse rsp = hreply.toResponse(ApplySecurityGroupRuleResponse.class);
                if (!rsp.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_KVM_10148, "failed to apply rules of security group rules to kvm host[uuid:%s], because %s", hto.getHostUuid(), rsp.getError());
                    complete.fail(err);
                    return;
                }
    ...
    public void checkDefaultRules(String hostUuid, Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_KVM_10149, "failed to check default rules of security group on kvm host[uuid:%s], because %s", hostUuid, rsp.getError());
                    completion.fail(err);
                    return;
                }
    ...
    public void updateGroupMembers(SecurityGroupMembersTO gto, String hostUuid, Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_KVM_10150, "operation error, because:%s", rsp.getError()));
                    return;
                }
    ...
    public void cleanUpUnusedRuleOnHost(String hostUuid, final Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_KVM_10151, "operation error, because:%s", rsp.getError()));
                    return;
                }
