...
public class VmNicLifecycleKvmBridge implements KVMPingAgentNoFailureExtensionPoint {
    ...
    @Override
    public void kvmPingAgentNoFailure(KVMHostInventory host,
                                      NoErrorCompletion completion) {
        final List<VmNicLifecycleExtensionPoint> extensions = getExtensions();
        ...
        // Step 1: Collect expected NICs of Running VMs on this host
        final List<VmNicInventory> allExpectedNics;
        try {
        ...
        // Step 2: Parallel dispatch with per-item timeout
        final long timeoutSeconds = VmNicLifecycleGlobalConfig.RECONCILE_TIMEOUT
                .value(Long.class);
        ...
        new While<>(extensions).step((ext, whileCompletion) -> {
            ...
            try {
                ...
                logger.warn(String.format("[VmNicLifecycle] %s.isApplicable threw " +
                        "exception during reconciliation on host[uuid:%s]",
            ...
            final ThreadFacadeImpl.TimeoutTaskReceipt receipt =
                    thdf.submitTimeoutTask(() -> {
                        if (completed.compareAndSet(false, true)) {
                            logger.warn(String.format("[VmNicLifecycle] " +
                                    "%s.reconcileOnHost timed out after %ds on " +
                                    "host[uuid:%s]",
                                    ext.getClass().getSimpleName(),
                                    timeoutSeconds, hostUuid));
                            whileCompletion.done();
                        }
            ...
            try {
                ...
                if (completed.compareAndSet(false, true)) {
                    receipt.cancel();
                    logger.warn(String.format("[VmNicLifecycle] " +
                            "%s.reconcileOnHost(host=%s) threw exception",
                            ext.getClass().getSimpleName(), hostUuid), t);
                    whileCompletion.done();
                }
            }
