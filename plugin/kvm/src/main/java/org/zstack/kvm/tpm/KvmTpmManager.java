...
import static org.zstack.header.errorcode.SysErrors.OPERATION_ERROR;
...
public class KvmTpmManager extends AbstractService {
    ...
    private void handle(AddTpmMsg msg) {
        if (msg.getKeyProviderUuid() != null && msg.getResourceUuidKeyFrom() != null) {
            throw new OperationFailureException(operr("keyProviderUuid and resourceUuidKeyFrom cannot be set at the same time"));
        }
    ...
    static class AddTpmToVmContext {
        ...
        boolean resourceKeyCreateAttempted;
        String createdTpmUuid;
        EncryptedResourceKeyManager.ResourceKeyResult resourceKeyResult;
        ...
        SimpleFlowChain.of("add-tpm-to-vm-" + context.vmInstanceUuid)
            ...
            .then(Flow.of("attach-key-provider-to-tpm")
                ...
                .handle(trigger -> {
                    ...
                    context.resourceKeyCreateAttempted = true;
                    resourceKeyManager.getOrCreateKey(keyCtx, new ReturnValueCompletion<EncryptedResourceKeyManager.ResourceKeyResult>(trigger) {
                        ...
                        public void success(EncryptedResourceKeyManager.ResourceKeyResult returnValue) {
                            context.resourceKeyResult = returnValue;
                            trigger.next();
                        }
                ...
                .rollback(trigger -> {
                    if (context.keyProviderAttached && context.createdTpmUuid != null) {
                        rollbackKeyProviderAttachment(context, trigger);
                        return;
                    }
                    trigger.rollback();
                })
    ...
    private void rollbackKeyProviderAttachment(AddTpmToVmContext context, FlowRollback trigger) {
        if (!context.resourceKeyCreateAttempted) {
            detachKeyProviderFromTpmQuietly(context.createdTpmUuid);
            trigger.rollback();
            return;
        }

        EncryptedResourceKeyManager.ResourceKeyResult result = context.resourceKeyResult;
        if (result == null) {
            result = new EncryptedResourceKeyManager.ResourceKeyResult();
            result.setResourceUuid(context.createdTpmUuid);
            result.setResourceType(TpmVO.class.getSimpleName());
            result.setKeyProviderUuid(context.keyProviderUuid);
            result.setCreatedNewKey(true);
        }

        resourceKeyManager.rollbackCreatedKey(result, new Completion(trigger) {
            @Override
            public void success() {
                detachKeyProviderFromTpmQuietly(context.createdTpmUuid);
                trigger.rollback();
            ...
            @Override
            public void fail(ErrorCode errorCode) {
                logger.warn(String.format("failed to rollback resource key for TPM[uuid:%s] while adding TPM to VM[uuid:%s]: %s",
                        context.createdTpmUuid,
                        context.vmInstanceUuid,
                        errorCode == null ? "" : errorCode.getDetails()));
                detachKeyProviderFromTpmQuietly(context.createdTpmUuid);
                trigger.rollback();
            }
        });
    ...
    private void detachKeyProviderFromTpmQuietly(String tpmUuid) {
        try {
    ...
    private void handle(APIUpdateTpmMsg msg) {
        throw new OperationFailureException(err(OPERATION_ERROR, "UpdateTpm is not supported in current version"));
    }
}
