...
public class KVMHost extends HostBase implements Host {
    ...
    protected void startVm(final VmInstanceSpec spec, final NeedReplyMessage msg, final NoErrorCompletion completion) {
        ...
        if (isWindowsVm(spec, platform)) {
            String cpuHardwareVirtualization = rcf.getResourceConfigValue(
                    KVMGlobalConfig.VM_CPU_HARDWARE_VIRTUALIZATION,
                    spec.getVmInventory().getUuid(),
                    String.class);
            if (KVMConstant.CPU_HARDWARE_VIRTUALIZATION_DISABLED.equals(cpuHardwareVirtualization)) {
                if (KVMConstant.CPU_MODE_NONE.equals(vmCpuMode)) {
                    ...
                    // Only disabled is sent; enabled and null keep the kvmagent default CPU feature behavior.
