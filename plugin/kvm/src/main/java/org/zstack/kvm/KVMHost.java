...
public class KVMHost extends HostBase implements Host {
    ...
    public KVMHost(KVMHostVO self, KVMHostContext context) {
        ...
        ub = UriComponentsBuilder.fromHttpUrl(baseUrl);
        ub.path(KVMConstant.KVM_HOST_DELETE_BITS_PATH);
        deleteBitsPath = ub.build().toString();
    }
    ...
    protected void handleLocalMessage(Message msg) {
        ...
        } else if (msg instanceof DeleteBitsMsg) {
            handle((DeleteBitsMsg) msg);
        } else {
    ...
    private void uploadFileToHost(UploadFileToHostMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(fileDownloadPath, cmd, DownloadFileResponse.class).call(new ReturnValueCompletion<DownloadFileResponse>(msg) {
            ...
            public void success(DownloadFileResponse rsp) {
                ...
                if (msg.needTrack()) {
                    new UploadFileTracker().runTrackTask(self.getUuid(), cmd.installPath);
                }
    ...
    private void handle(GetFileDownloadProgressMsg msg) {
        ...
        new Http<>(fileDownloadProgressPath, cmd, GetDownloadFileProgressResponse.class).call(new ReturnValueCompletion<GetDownloadFileProgressResponse>(msg) {
            ...
            public void success(GetDownloadFileProgressResponse resp) {
                if (!resp.isSuccess()) {
                    r.setError(operr("failed to query download progress for api[%s] on host[uuid:%s], because: %s",
                            msg.getApiId(), self.getUuid(), resp.getError()));
                } else {
    ...
    private void handle(DeleteBitsMsg msg) {
        DeleteBitsReply reply = new DeleteBitsReply();
        DeleteBitsCmd cmd = new DeleteBitsCmd();
        cmd.path = msg.getPath();
        cmd.folder = msg.isFolder();
        new Http<>(deleteBitsPath, cmd, AgentResponse.class).call(new ReturnValueCompletion<AgentResponse>(msg) {
            @Override
            public void success(AgentResponse rsp) {
                if (!rsp.isSuccess()) {
                    reply.setError(operr("failed to delete bits[path:%s, folder:%s], because: %s",
                            cmd.path, cmd.folder, rsp.getError()));
                }
                bus.reply(msg, reply);
            }
