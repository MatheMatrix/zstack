...
public class KVMHost extends HostBase implements Host {
                ...
                restf.asyncJsonPost(path, commandStr, header, new JsonAsyncRESTCallback<T>(completion) {
                    ...
                    public void success(T ret) {
                        if (dbf.isExist(self.getUuid(), HostVO.class)) {
                            ...
                            completion.fail(operr(ORG_ZSTACK_KVM_10015, "host[uuid:%s] has been deleted", self.getUuid()));
                        }
                    }
                ...
                restf.asyncJsonPost(path, cmd, header, new JsonAsyncRESTCallback<T>(completion) {
                    ...
                    public void success(T ret) {
                        if (dbf.isExist(self.getUuid(), HostVO.class)) {
                            ...
                            completion.fail(operr(ORG_ZSTACK_KVM_10016, "host[uuid:%s] has been deleted", self.getUuid()));
                        }
                    }
        ...
        thdf.singleFlightSubmit(new SingleFlightTask(msg)
                ...
                .run(completion -> {
                    if (!destMaker.isManagedByUs(msg.getHostUuid())) {
                        completion.fail(operr(ORG_ZSTACK_KVM_10017, "host %s is not managed by current mn node", msg.getHostUuid()));
                        return;
                    }
        ...
        chain.then(new Flow() {
            ...
            public void run(FlowTrigger trigger, Map data) {
                if (self.getStatus() != HostStatus.Connected) {
                    trigger.fail(operr(ORG_ZSTACK_KVM_10018, "host %s is not connected, skip to restart kvmagent", self.getUuid()));
                    return;
                }
                ...
                noRunningTaskOnHost(new ReturnValueCompletion<Boolean>(trigger) {
                    ...
                    public void success(Boolean noTask) {
                        if (noTask) {
                            ...
                            trigger.fail(operr(ORG_ZSTACK_KVM_10019, "running task exists on host %s", self.getUuid()));
                        }
                    }
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                if (ret.isSshFailure() || ret.getReturnCode() != 0) {
                    trigger.fail(operr(ORG_ZSTACK_KVM_10020, ret.getExitErrorMessage()));
                } else {
                    trigger.next();
                }
            }
    ...
    private void takeVmConsoleScreenshot(String vmInstanceUuid, String hostUuid, ReturnValueCompletion<TakeVmConsoleScreenshotRsp> completion) {
        ...
        bus.send(kmsg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_KVM_10021, rsp.getError()));
                } else {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        new Http<>(blockCommitPath, cmd, BlockCommitResponse.class).call(new ReturnValueCompletion<BlockCommitResponse>(msg, completion) {
                            ...
                            public void success(BlockCommitResponse ret) {
                                if (!ret.isSuccess()) {
                                    ErrorCode err = operr(ORG_ZSTACK_KVM_10022, "operation error, because:%s", ret.getError());
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        new Http<>(blockPullPath, cmd, BlockPullResponse.class).call(new ReturnValueCompletion<BlockPullResponse>(msg, completion) {
                            ...
                            public void success(BlockPullResponse ret) {
                                if (!ret.isSuccess()) {
                                    ErrorCode err = operr(ORG_ZSTACK_KVM_10023, "operation error, because:%s", ret.getError());
    ...
    private void handle(GetHostWebSshUrlMsg msg) {
        ...
        try (Response r = hb.callWithException()) {
            ...
            if (!r.isSuccessful()) {
                reply.setError(inerr(ORG_ZSTACK_KVM_10024, "webssh server is unreachable for %s", r.message()));
                ...
                thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask() {
                    ...
                    public boolean run() {
                        if (timeHelper.getCurrentTimeMillis() > deadline) {
                            reply.setError(operr(ORG_ZSTACK_KVM_10026, String.format("Host[%s] has not been power on within %d seconds for an unknown reason. Please check host status in BMC[%s]", msg.getHostUuid(), timeoutInSec, host.getIpmi().getIpmiAddress())));
                            reply.setSuccess(false);
                            bus.reply(msg, reply);
                ...
                thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask() {
                    ...
                    public boolean run() {
                        ...
                        if (timeHelper.getCurrentTimeMillis() > deadline) {
                            ...
                            if (!msg.isReturnEarly()) {
                                reply.setError(operr(ORG_ZSTACK_KVM_10027, String.format("Host[%s] has not been shut down within %d seconds for an unknown reason. Please check host status in BMC[%s]", msg.getHostUuid(), timeoutInSec, host.getIpmi().getIpmiAddress())));
                                reply.setSuccess(false);
                                bus.reply(msg, reply);
                            }
                            return true;
        ...
        new Http<>(rebootHost, cmd, RebootHostResponse.class).call(new ReturnValueCompletion<RebootHostResponse>(msg, completion) {
            ...
            public void success(RebootHostResponse returnValue) {
                if (!returnValue.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10028, "operation error, because:%s", returnValue.getError()));
                    bus.reply(msg, reply);
                    completion.done();
                    return;
                }
    ...
    private void handle(SyncVmDeviceInfoMsg msg) {
        ...
        new Http<>(syncVmDeviceInfo, cmd, SyncVmDeviceInfoResponse.class)
            ...
            public void success(SyncVmDeviceInfoResponse ret) {
                if (!ret.isSuccess()) {
                    ErrorCode err = Platform.err(ORG_ZSTACK_KVM_10029, SysErrors.OPERATION_ERROR, ret.getError());
                    reply.setError(err);
                }
    ...
    private void handle(GetHostNumaTopologyMsg msg) {
        ...
        new Http<>(getHostNumaPath, cmd, GetHostNUMATopologyResponse.class).call(msg.getHostUuid(), new ReturnValueCompletion<GetHostNUMATopologyResponse>(msg) {
            ...
            public void success(GetHostNUMATopologyResponse ret) {
                if (!ret.isSuccess()) {
                    ErrorCode err = Platform.err(ORG_ZSTACK_KVM_10030, SysErrors.OPERATION_ERROR, ret.getError());
                    reply.setError(err);
                } else {
    ...
    private void compareCpuFunctionOnHost(final CompareCpuFunctionOnHostMsg msg, ReturnValueCompletion<VmCompareCpuFunctionResponse> completion) {
        ...
        restf.asyncJsonPost(compareCpuFunctionPath, cmd, new JsonAsyncRESTCallback<VmCompareCpuFunctionResponse>(completion) {
            ...
            public void success(VmCompareCpuFunctionResponse ret) {
                if (!ret.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_KVM_10031, ret.getError()));
                    return;
                }
                completion.success(ret);
            }
    ...
    private void getCpuFunctionXml(final GetCpuFunctionXmlOnHostMsg msg, ReturnValueCompletion<GetCpuFunctionXmlOnHostReply> completion) {
        ...
        restf.asyncJsonPost(getCpuXmlPath, new VmGetCpuXmlCmd(), new JsonAsyncRESTCallback<VmGetCpuXmlResponse>(completion) {
            ...
            public void success(VmGetCpuXmlResponse ret) {
                if (!ret.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_KVM_10032, ret.getError()));
                    return;
                }
    ...
    private void handle(CheckHostCapacityMsg msg) {
        ...
        bus.send(kmsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply reply) {
                if (!reply.isSuccess()) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10033, "check host capacity failed, because:%s", reply.getError()));
                }

                KVMHostAsyncHttpCallReply r = reply.castReply();
                HostCapacityResponse rsp = r.toResponse(HostCapacityResponse.class);
                if (!rsp.isSuccess()) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10034, "operation error, because:%s", rsp.getError()));
                }
                ...
                if (rsp.getTotalMemory() < reservedSize) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10035, "The host[uuid:%s]'s available memory capacity[%s] is lower than the reserved capacity[%s]",
    ...
    private void registerPrimaryVmHeartbeat(RegisterColoPrimaryCheckMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(registerPrimaryVmHeartbeatPath, cmd, AgentResponse.class).call(new ReturnValueCompletion<AgentResponse>(msg, completion) {
            ...
            public void success(AgentResponse ret) {
                ...
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10036, "unable to register colo heartbeat for vm[uuid:%s] on kvm host [uuid:%s, ip:%s], because %s",
    ...
    private void startColoSync(StartColoSyncMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(startColoSyncPath, cmd, AgentResponse.class).call(new ReturnValueCompletion<AgentResponse>(msg, completion) {
            ...
            public void success(AgentResponse ret) {
                final StartColoSyncReply reply = new StartColoSyncReply();
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10037, "unable to start colo sync vm[uuid:%s] on kvm host [uuid:%s, ip:%s], because %s",
                            msg.getVmInstanceUuid(), self.getUuid(), self.getManagementIp(), ret.getError()));
                } else {
    ...
    private void configSecondaryVm(ConfigSecondaryVmMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(configSecondaryVmPath, cmd, AgentResponse.class).call(new ReturnValueCompletion<AgentResponse>(msg, completion) {
            ...
            public void success(AgentResponse ret) {
                ...
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10038, "unable to config secondary vm[uuid:%s] on kvm host [uuid:%s, ip:%s], because %s",
                            msg.getVmInstanceUuid(), self.getUuid(), self.getManagementIp(), ret.getError()));
                } else {
    ...
    private void configPrimaryVm(ConfigPrimaryVmMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(configPrimaryVmPath, cmd, AgentResponse.class).call(new ReturnValueCompletion<AgentResponse>(msg, completion) {
            ...
            public void success(AgentResponse ret) {
                final ConfigPrimaryVmReply reply = new ConfigPrimaryVmReply();
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10039, "unable to config primary vm[uuid:%s] on kvm host [uuid:%s, ip:%s], because %s",
                            msg.getVmInstanceUuid(), self.getUuid(), self.getManagementIp(), ret.getError()));
                } else {
    ...
    private void getVmFirstBootDevice(final GetVmFirstBootDeviceOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(getVmFirstBootDevicePath, cmd, GetVmFirstBootDeviceResponse.class).call(new ReturnValueCompletion<GetVmFirstBootDeviceResponse>(msg, completion) {
            ...
            public void success(GetVmFirstBootDeviceResponse ret) {
                ...
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10040, "unable to get first boot dev of vm[uuid:%s] on kvm host [uuid:%s, ip:%s], because %s",
                            msg.getVmInstanceUuid(), self.getUuid(), self.getManagementIp(), ret.getError()));
                } else {
    ...
    private void getVmDeviceAddress(final GetVmDeviceAddressMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(getVmDeviceAddressPath, cmd, GetVmDeviceAddressRsp.class).call(new ReturnValueCompletion<GetVmDeviceAddressRsp>(msg, completion) {
            ...
            public void success(GetVmDeviceAddressRsp rsp) {
                if (!rsp.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10041, "failed to get vm[uuid:%s] device address, because:%s", msg.getVmInstanceUuid(), rsp.getError()));
                    bus.reply(msg, reply);
                    completion.done();
                    return;
                }
    ...
    private void getVirtualizerInfo(final GetVirtualizerInfoMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(getVirtualizerInfo, cmd, GetVirtualizerInfoRsp.class).call(new ReturnValueCompletion<GetVirtualizerInfoRsp>(msg, completion) {
            ...
            public void success(GetVirtualizerInfoRsp rsp) {
                if (!rsp.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10042, "failed to get host[uuid:%s] virtualizer info, because:%s", msg.getHostUuid(), rsp.getError()));
                    bus.reply(msg, reply);
                    completion.done();
                    return;
                }
    ...
    private void handle(final IncreaseVmCpuMsg msg) {
        ...
        new Http<>(onlineIncreaseCpuPath, cmd, IncreaseCpuResponse.class).call(new ReturnValueCompletion<IncreaseCpuResponse>(msg) {
            ...
            public void success(IncreaseCpuResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10043, "failed to increase vm cpu, error details: %s", ret.getError()));
                } else {
    ...
    private void directlyDestroy(final VmDirectlyDestroyOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(destroyVmPath, cmd, DestroyVmResponse.class).call(new ReturnValueCompletion<DestroyVmResponse>(completion) {
            ...
            public void success(DestroyVmResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(err(ORG_ZSTACK_KVM_10045, HostErrors.FAILED_TO_DESTROY_VM_ON_HYPERVISOR, ret.getError()));
                }
    ...
    private void handle(KvmRunShellMsg msg) {
        ...
        if (result.isSshFailure()) {
            reply.setError(operr(ORG_ZSTACK_KVM_10046, "unable to connect to KVM[ip:%s, username:%s, sshPort:%d ] to do DNS check," +
    ...
    private void handle(final GetVmConsoleAddressFromHostMsg msg) {
        ...
        new Http<>(getConsolePortPath, cmd, GetVncPortResponse.class).call(new ReturnValueCompletion<GetVncPortResponse>(msg) {
            ...
            public void success(GetVncPortResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10047, "operation error, because:%s", ret.getError()));
                } else {
    ...
    private void handle(final UpdateVmPriorityMsg msg) {
        ...
        if (self.getStatus() != HostStatus.Connected) {
            reply.setError(operr(ORG_ZSTACK_KVM_10048, "the host[uuid:%s, status:%s] is not Connected", self.getUuid(), self.getStatus()));
            bus.reply(msg, reply);
            return;
        }
    ...
    protected void handle(final CheckVmStateOnHypervisorMsg msg) {
        ...
        if (self.getStatus() != HostStatus.Connected) {
            reply.setError(operr(ORG_ZSTACK_KVM_10050, "the host[uuid:%s, status:%s] is not Connected", self.getUuid(), self.getStatus()));
            bus.reply(msg, reply);
            return;
        }
    ...
    private void attachIso(final AttachIsoOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(attachIsoPath, cmd, AttachIsoRsp.class).call(new ReturnValueCompletion<AttachIsoRsp>(msg, completion) {
            ...
            public void success(AttachIsoRsp ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10053, "operation error, because:%s", ret.getError()));
                }
    ...
    protected void changeVmNicState(final ChangeVmNicStateOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(changeNicStatePath, cmd, ChangeVmNicStateRsp.class).call(new ReturnValueCompletion<ChangeVmNicStateRsp>(msg, completion) {
            ...
            public void success(ChangeVmNicStateRsp ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10054, "operation error, because:%s", ret.getError()));
                }
                bus.reply(msg, reply);
                completion.done();
            }
    ...
    protected void detachNic(final DetachNicFromVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(detachNicPath, cmd, DetachNicRsp.class).call(new ReturnValueCompletion<DetachNicRsp>(msg, completion) {
            ...
            public void success(DetachNicRsp ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10055, "operation error, because:%s", ret.getError()));
                }
                bus.reply(msg, reply);
                completion.done();
            }
        ...
        new Http<>(url, msg.getCommand(), msg.getCommandClassName(), LinkedHashMap.class)
            ...
            public void fail(ErrorCode err) {
                ...
                if (err.isError(SysErrors.HTTP_ERROR, SysErrors.IO_ERROR)) {
                    reply.setError(err(ORG_ZSTACK_KVM_10056, HostErrors.OPERATION_FAILURE_GC_ELIGIBLE, err, "cannot do the operation on the KVM host"));
                } else {
                    reply.setError(err);
                }
    ...
    private void mergeVolumeSnapshot(final MergeVolumeSnapshotOnKvmMsg msg, final NoErrorCompletion completion) {
        ...
        if (volume.getVmInstanceUuid() != null) {
            ...
            if (state != VmInstanceState.Stopped && state != VmInstanceState.Running && state != VmInstanceState.Paused && state != VmInstanceState.Destroyed) {
                throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10057, "cannot do volume snapshot merge when vm[uuid:%s] is in state of %s." +
            ...
            if (state == VmInstanceState.Running) {
                ...
                if (new VersionComparator(KVMConstant.MIN_LIBVIRT_LIVE_BLOCK_COMMIT_VERSION).compare(libvirtVersion) > 0) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10058, "live volume snapshot merge needs libvirt version greater than %s," +
        ...
        new Http<>(mergeSnapshotPath, cmd, MergeSnapshotRsp.class)
            ...
            public void success(MergeSnapshotRsp ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10059, "operation error, because:%s", ret.getError()));
    ...
    private void doCheckSnapshot(final CheckSnapshotOnHypervisorMsg msg, final Completion completion) {
        ...
        if (msg.getVmUuid() != null) {
            ...
            if (vmState != VmInstanceState.Running && vmState != VmInstanceState.Stopped && vmState != VmInstanceState.Paused) {
                throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10060, "vm[uuid:%s] is not Running or Stopped, current state[%s]", msg.getVmUuid(), vmState));
            }
        }
            ...
            public void run(FlowTrigger trigger, Map data) {
                new Http<>(checkSnapshotPath, cmd, CheckSnapshotResponse.class).call(new ReturnValueCompletion<CheckSnapshotResponse>(msg, trigger) {
                    ...
                    public void success(CheckSnapshotResponse ret) {
                        if (!ret.isSuccess()) {
                            trigger.fail(operr(ORG_ZSTACK_KVM_10061, "operation error, because:%s", ret.getError()));
                            return;
                        }
    ...
    private void doTakeSnapshot(final TakeSnapshotOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        if (msg.getVmUuid() != null) {
            ...
            VmInstanceState vmState = q.findValue();
            if (vmState != VmInstanceState.Running && vmState != VmInstanceState.Stopped && vmState != VmInstanceState.Paused) {
                throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10062, "vm[uuid:%s] is not Running or Stopped, current state[%s]", msg.getVmUuid(), vmState));
            }

            if (!HostSystemTags.LIVE_SNAPSHOT.hasTag(self.getUuid())) {
                if (vmState != VmInstanceState.Stopped) {
                    reply.setError(err(ORG_ZSTACK_KVM_10063, SysErrors.NO_CAPABILITY_ERROR,
                    ...
                    public void success(TakeSnapshotResponse ret) {
                        if (ret.isSuccess()) {
                            if (Objects.equals(ret.getNewVolumeInstallPath(), ret.getSnapshotInstallPath())) {
                                throw new OperationFailureException(Platform.inerr(ORG_ZSTACK_KVM_10064, "SERIOUS BUG: the agent returns the " +
                            ...
                            ErrorCode err = operr(ORG_ZSTACK_KVM_10065, "operation error, because:%s", ret.getError());
                    ...
                    public void run(final FlowTrigger trigger, Map data) {
                        ...
                        new Http<>(migrateUrl, cmd, MigrateVmResponse.class).call(migrateFromDestination ? dstHostUuid : srcHostUuid, new ReturnValueCompletion<MigrateVmResponse>(trigger) {
                            ...
                            public void success(MigrateVmResponse ret) {
                                if (!ret.isSuccess()) {
                                    ErrorCode err = err(ORG_ZSTACK_KVM_10066, HostErrors.FAILED_TO_MIGRATE_VM_ON_HYPERVISOR,
    ...
    private void updateNic(VmUpdateNicOnHypervisorMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(updateNicPath, cmd, AttachNicResponse.class).call(new ReturnValueCompletion<AttachNicResponse>(msg, completion) {
            ...
            public void success(AttachNicResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10067, "failed to update nic[vm:%s] on kvm host[uuid:%s, ip:%s]," +
            ...
            public void success(AttachNicResponse ret) {
                if (!ret.isSuccess()) {
                    if (ret.getError().contains("Device or resource busy")) {
                        reply.setError(operr(ORG_ZSTACK_KVM_10068, "failed to attach nic[uuid:%s, vm:%s] on kvm host[uuid:%s, ip:%s]," +
                        ...
                        reply.setError(operr(ORG_ZSTACK_KVM_10069, "failed to attach nic[uuid:%s, vm:%s] on kvm host[uuid:%s, ip:%s]," +
    ...
    protected void detachVolume(final DetachVolumeFromVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(detachDataVolumePath, cmd, DetachDataVolumeResponse.class).call(new ReturnValueCompletion<DetachDataVolumeResponse>(msg, completion) {
            ...
            public void success(DetachDataVolumeResponse ret) {
                if (!ret.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_KVM_10070, "failed to detach data volume[uuid:%s, installPath:%s] from vm[uuid:%s, name:%s] on kvm host[uuid:%s, ip:%s], because %s",
    ...
    private boolean allowToAttachVolumeOnHypervisor(AttachVolumeToVmOnHypervisorMsg msg) {
        ...
        if (!allowed) {
            ErrorCode errorCode = err(ORG_ZSTACK_KVM_10071, VmErrors.ATTACH_VOLUME_ERROR,
    ...
    protected void attachVolume(final AttachVolumeToVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(attachDataVolumePath, cmd, AttachDataVolumeResponse.class).call(new ReturnValueCompletion<AttachDataVolumeResponse>(msg, completion) {
            ...
            public void success(AttachDataVolumeResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10072, "failed to attach data volume[uuid:%s, installPath:%s] to vm[uuid:%s, name:%s]" +
    ...
    protected void destroyVm(final DestroyVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        try {
            ...
            ErrorCode err = operr(ORG_ZSTACK_KVM_10073, "failed to destroy vm[uuid:%s name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(), vminv.getName(),
        ...
        new Http<>(destroyVmPath, cmd, DestroyVmResponse.class).call(new ReturnValueCompletion<DestroyVmResponse>(msg, completion) {
            ...
            public void success(DestroyVmResponse ret) {
                ...
                if (!ret.isSuccess()) {
                    reply.setError(err(ORG_ZSTACK_KVM_10074, HostErrors.FAILED_TO_DESTROY_VM_ON_HYPERVISOR, "unable to destroy vm[uuid:%s,  name:%s] on kvm host [uuid:%s, ip:%s], because %s", vminv.getUuid(),
            ...
            public void fail(ErrorCode err) {
                ...
                if (err.isError(SysErrors.HTTP_ERROR, SysErrors.IO_ERROR, SysErrors.TIMEOUT)) {
                    err = err(ORG_ZSTACK_KVM_10075, HostErrors.OPERATION_FAILURE_GC_ELIGIBLE, err, "unable to destroy a vm");
                }
    ...
    private void rebootVm(final RebootVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        try {
            ...
            String err = String.format("failed to reboot vm[uuid:%s name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(), vminv.getName(),
                    self.getUuid(), self.getManagementIp(), e.getMessage());
            logger.warn(err, e);
            throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10076, err));
        }
        ...
        new Http<>(rebootVmPath, cmd, RebootVmResponse.class).call(new ReturnValueCompletion<RebootVmResponse>(msg, completion) {
            ...
            public void success(RebootVmResponse ret) {
                ...
                if (!ret.isSuccess()) {
                    reply.setError(err(ORG_ZSTACK_KVM_10077, HostErrors.FAILED_TO_REBOOT_VM_ON_HYPERVISOR, "unable to reboot vm[uuid:%s, name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(),
                            vminv.getName(), self.getUuid(), self.getManagementIp(), ret.getError()));
    ...
    protected void stopVm(final StopVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        try {
            ...
            ErrorCode err = operr(ORG_ZSTACK_KVM_10078, "failed to stop vm[uuid:%s name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(), vminv.getName(),
                    self.getUuid(), self.getManagementIp(), e.getMessage());
            throw new OperationFailureException(err);
        }
        ...
        new Http<>(stopVmPath, cmd, StopVmResponse.class).call(new ReturnValueCompletion<StopVmResponse>(msg, completion) {
            ...
            public void success(StopVmResponse ret) {
                ...
                if (!ret.isSuccess()) {
                    reply.setError(err(ORG_ZSTACK_KVM_10079, HostErrors.FAILED_TO_STOP_VM_ON_HYPERVISOR, "unable to stop vm[uuid:%s,  name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(),
                            vminv.getName(), self.getUuid(), self.getManagementIp(), ret.getError()));
            ...
            public void fail(ErrorCode err) {
                ...
                if (err.isError(SysErrors.IO_ERROR, SysErrors.HTTP_ERROR)) {
                    err = err(ORG_ZSTACK_KVM_10080, HostErrors.OPERATION_FAILURE_GC_ELIGIBLE, err, "unable to stop a vm");
                }
        ...
        chain.then(new NoRollbackFlow() {
            ...
            public void run(FlowTrigger chain, Map data) {
                ...
                bus.send(msg, new CloudBusCallBack(chain) {
                    ...
                    public void run(MessageReply reply) {
                        if (!reply.isSuccess()) {
                            ErrorCode err = operr(ORG_ZSTACK_KVM_10081, "host[uuid:%s] capacity is not enough to offer cpu[%s], memory[%s bytes]",
    ...
    private void handle(final UpdateSpiceChannelConfigMsg msg) {
        ...
        new Http<>(updateSpiceChannelConfigPath, cmd, UpdateSpiceChannelConfigResponse.class).call(new ReturnValueCompletion<UpdateSpiceChannelConfigResponse>(msg) {
            ...
            public void success(UpdateSpiceChannelConfigResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10082, "Host[%s] update spice channel config faild, because %s", msg.getHostUuid(), ret.getError()));
                    logger.warn(reply.getError().getDetails());
                }
    ...
    private void checkPlatformWithOther(VmInstanceSpec spec) {
        ...
        if (total > 3) {
            throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10083, "when the vm platform is Other, the number of dataVolumes and cdroms cannot exceed 3, currently %s", total));
        }
    }
        ...
        new Http<>(startVmPath, cmd, StartVmResponse.class).call(new ReturnValueCompletion<StartVmResponse>(msg, completion) {
            ...
            public void success(StartVmResponse ret) {
                ...
                if (ret.isSuccess()) {
                    ...
                    reply.setError(err(ORG_ZSTACK_KVM_10084, HostErrors.FAILED_TO_START_VM_ON_HYPERVISOR, "failed to start vm[uuid:%s name:%s] on kvm host[uuid:%s, ip:%s], because %s",
                            spec.getVmInventory().getUuid(), spec.getVmInventory().getName(),
                            self.getUuid(), self.getManagementIp(), ret.getError()));
                    logger.warn(reply.getError().getDetails());
            ...
            public void fail(ErrorCode err) {
                StartVmOnHypervisorReply reply = new StartVmOnHypervisorReply();
                reply.setError(err(ORG_ZSTACK_KVM_10085, HostErrors.FAILED_TO_START_VM_ON_HYPERVISOR, "failed to start vm[uuid: %s] on kvm host[uuid: %s], because %s",
    ...
    private void pauseVm(final PauseVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(pauseVmPath, cmd, PauseVmResponse.class).call(new ReturnValueCompletion<PauseVmResponse>(msg, completion) {
            ...
            public void success(PauseVmResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(err(ORG_ZSTACK_KVM_10086, HostErrors.FAILED_TO_STOP_VM_ON_HYPERVISOR, "unable to pause vm[uuid:%s,  name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(),
                            vminv.getName(), self.getUuid(), self.getManagementIp(), ret.getError()));
                    logger.warn(reply.getError().getDetails());
                }
                bus.reply(msg, reply);
    ...
    private void resumeVm(final ResumeVmOnHypervisorMsg msg, final NoErrorCompletion completion) {
        ...
        new Http<>(resumeVmPath, cmd, ResumeVmResponse.class).call(new ReturnValueCompletion<ResumeVmResponse>(msg, completion) {
            ...
            public void success(ResumeVmResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(err(ORG_ZSTACK_KVM_10087, HostErrors.FAILED_TO_STOP_VM_ON_HYPERVISOR, "unable to resume vm[uuid:%s,  name:%s] on kvm host[uuid:%s, ip:%s], because %s", vminv.getUuid(),
                            vminv.getName(), self.getUuid(), self.getManagementIp(), ret.getError()));
                    logger.warn(reply.getError().getDetails());
                }
                bus.reply(msg, reply);
    ...
    private void batchCheckPhysicalInterface(BatchCheckNetworkPhysicalInterfaceMsg msg, NoErrorCompletion completion) {
        ...
        if (!rsp.isSuccess()) {
            if (rsp.getFailedInterfaceNames().isEmpty()) {
                reply.setError(operr(ORG_ZSTACK_KVM_10088, "operation error, because:%s", rsp.getError()));
            } else {
                reply.setError(operr(ORG_ZSTACK_KVM_10089, "failed to check physical network interfaces[names : %s] on kvm host[uuid:%s, ip:%s]",
    ...
    private void checkPhysicalInterface(CheckNetworkPhysicalInterfaceMsg msg, NoErrorCompletion completion) {
        ...
        if (!rsp.isSuccess()) {
            if (rsp.getFailedInterfaceNames().isEmpty()) {
                reply.setError(operr(ORG_ZSTACK_KVM_10090, "operation error, because:%s", rsp.getError()));
            } else {
                reply.setError(operr(ORG_ZSTACK_KVM_10091, "failed to check physical network interfaces[names : %s] on kvm host[uuid:%s, ip:%s]",
    ...
    private void doReconnectHostDueToPingResult(PingResponse ret) {
        ...
        changeConnectionState(HostStatusEvent.disconnected);
        new HostDisconnectedCanonicalEvent(self.getUuid(), argerr(ORG_ZSTACK_KVM_10092, info)).fire();
            ...
            public void run(SyncTaskChain chain) {
                ...
                restf.asyncJsonPost(updateHostConfigurationPath, cmd, new JsonAsyncRESTCallback<UpdateHostConfigurationResponse>(chain) {
                    ...
                    public void fail(ErrorCode err) {
                        ...
                        changeConnectionState(HostStatusEvent.disconnected);
                        new HostDisconnectedCanonicalEvent(self.getUuid(), argerr(ORG_ZSTACK_KVM_10093, info)).fire();
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        restf.asyncJsonPost(pingPath, cmd, new JsonAsyncRESTCallback<PingResponse>(trigger) {
                            ...
                            public void success(PingResponse ret) {
                                if (!ret.isSuccess()) {
                                    trigger.fail(operr(ORG_ZSTACK_KVM_10094, "%s", ret.getError()));
                                    return;
                                }
    ...
    protected void deleteTakeOverFlag(Completion completion) {
        ...
        if (ret.isSshFailure() || ret.getReturnCode() != 0) {
            completion.fail(operr(ORG_ZSTACK_KVM_10095, ret.getExitErrorMessage()));
            return;
        }
        completion.success();
    }
    ...
    private ErrorCode connectToAgent() {
        ...
        try {
            ...
            if (!rsp.isSuccess()) {
                errCode = operr(ORG_ZSTACK_KVM_10096, "unable to connect to kvm host[uuid:%s, ip:%s, url:%s], because %s",
            ...
            errCode = operr(ORG_ZSTACK_KVM_10097, "unable to connect to kvm host[uuid:%s, ip:%s, url:%s], because %s", self.getUuid(), self.getManagementIp(),
                    connectPath, e.getMessage());
        } catch (Throwable t) {
            logger.warn(t.getMessage(), t);
            errCode = inerr(ORG_ZSTACK_KVM_10098, t.getMessage());
        }
        ...
        chain.done(new FlowDoneHandler(completion) {
            ...
            public void handle(Map data) {
                if (noStorageAccessible()) {
                    ...
                    completion.fail(operr(ORG_ZSTACK_KVM_10099, "host can not access any primary storage, %s", errorCodeList != null && StringUtils.isNotEmpty(errorCodeList.getReadableDetails()) ? errorCodeList.getReadableDetails() : "please check network"));
                } else {
                    if (CoreGlobalProperty.UNIT_TEST_ON) {
                        completion.success();
                        return;
                    ...
                    if (ret.isSshFailure() || ret.getReturnCode() != 0) {
                        completion.fail(operr(ORG_ZSTACK_KVM_10100, ret.getExitErrorMessage()));
                        return;
                    }
                    completion.success();
                }
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                completion.fail(err(ORG_ZSTACK_KVM_10101, HostErrors.CONNECTION_ERROR, errCode, "connection error for KVM host[uuid:%s, ip:%s]", self.getUuid(),
    ...
    protected void checkConnectConditions(ConnectHostInfo info, Completion completion) {
        ...
        chain.then(new NoRollbackFlow() {
                ...
                thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask(trigger) {
                    ...
                    private boolean ifTimeout() {
                        if (System.currentTimeMillis() > timeout) {
                            trigger.fail(operr(ORG_ZSTACK_KVM_10102, "the host[%s] ssh port[%s] not open after %s seconds, connect timeout", getSelf().getManagementIp(), getSelf().getPort(), TimeUnit.MILLISECONDS.toSeconds(sshTimeout)));
                            return true;
                        } else {
                            return false;
                        }
            ...
            ErrorCode connectWithSSHPassword() {
                ...
                if (ret.isSshFailure()) {
                    return err(ORG_ZSTACK_KVM_10104, HostErrors.UNABLE_TO_RECONNECT_HOST,
            ...
            ErrorCode connectWithPrivateKey() {
                ...
                SshResult ret = sshShell.runCommand(cmd);
                if (ret.isSshFailure()) {
                    return err(ORG_ZSTACK_KVM_10105, HostErrors.UNABLE_TO_RECONNECT_HOST,
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                if (ret.isSshFailure()) {
                    trigger.fail(operr(ORG_ZSTACK_KVM_10106, "unable to connect to KVM[ip:%s, username:%s, sshPort: %d, ] to do DNS check, please check if username/password is wrong; %s", self.getManagementIp(), getSelf().getUsername(), getSelf().getPort(), ret.getExitErrorMessage()));
                } else if (ret.getReturnCode() != 0) {
                    trigger.fail(operr(ORG_ZSTACK_KVM_10107, "failed to ping all DNS/IP in %s; please check /etc/resolv.conf to make sure your host is able to reach public internet", checkList));
                } else {
                    trigger.next();
                }
            }
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                if (ret.isSshFailure()) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10108, "unable to connect to KVM[ip:%s, username:%s, sshPort:%d] to check the management node connectivity," +
                    ...
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10109, "the KVM host[ip:%s] cannot access the management node's callback url. It seems" +
            ...
            public void setup() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        try {
                            ...
                            if (hostRet.isSshFailure() || hostRet.getReturnCode() != 0) {
                                trigger.fail(operr(ORG_ZSTACK_KVM_10110, "unable to Check whether the host is taken over,  because %s", hostRet.getExitErrorMessage()));
                                return;
                            }
                            ...
                            if (timeRet.isSshFailure() || timeRet.getReturnCode() != 0) {
                                trigger.fail(operr(ORG_ZSTACK_KVM_10111, "Unable to get the timestamp of the flag,  because %s", timeRet.getExitErrorMessage()));
                                return;
                            }
                            ...
                            if (diff < HostGlobalConfig.PING_HOST_INTERVAL.value(int.class)) {
                                trigger.fail(operr(ORG_ZSTACK_KVM_10112, "the host[ip:%s] has been taken over, because the takeover flag[HostUuid:%s] already exists and utime[%d] has not exceeded host ping interval[%d]",
                            ...
                            if (lastHostInv == null) {
                                ...
                                trigger.fail(operr(ORG_ZSTACK_KVM_10113, "the host[ip:%s] has been taken over, because flag[HostUuid:%s] exists in the database",
                ...
                flow(new NoRollbackFlow() {
                    ...
                    private Object getHostCpuArchitectureBySsh() {
                        ...
                        if (ret.isSshFailure() || ret.getReturnCode() != 0) {
                            return operr(ORG_ZSTACK_KVM_10114, "unable to get host cpu architecture, please check if username/password is wrong; %s", ret.getExitErrorMessage());
                        }
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (cluster.getArchitecture() != null && !hostArchitecture.equals(cluster.getArchitecture()) && !cluster.getHypervisorType().equals("baremetal2")) {
                            trigger.fail(operr(ORG_ZSTACK_KVM_10115, "host cpu architecture[%s] is not matched the cluster[%s]", hostArchitecture, cluster.getArchitecture()));
                            return;
                        }
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        try {
                            ...
                            throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10116, ex.toString()));
                        }
                                ...
                                .call(new ReturnValueCompletion<UpdateDependencyRsp>(trigger) {
                                    ...
                                    public void success(UpdateDependencyRsp ret) {
                                        if (ret.isSuccess()) {
                                            ...
                                            trigger.fail(Platform.operr(ORG_ZSTACK_KVM_10117, "%s", ret.getError()));
                                        }
                                    }
                    ...
                    flow(new NoRollbackFlow() {
                        ...
                        public void run(FlowTrigger trigger, Map data) {
                            if (!checkQemuLibvirtVersionOfHost()) {
                                trigger.fail(operr(ORG_ZSTACK_KVM_10118, "host [uuid:%s] cannot be added to cluster [uuid:%s] because qemu/libvirt version does not match",
                            ...
                            if (KVMSystemTags.CHECK_CLUSTER_CPU_MODEL.hasTag(self.getClusterUuid())) {
                                if (KVMSystemTags.CHECK_CLUSTER_CPU_MODEL
                                        ...
                                        .equals("true")
                                        && !checkCpuModelOfHost()) {
                                    trigger.fail(operr(ORG_ZSTACK_KVM_10119, "host [uuid:%s] cannot be added to cluster [uuid:%s] because cpu model name does not match",
                                            self.getUuid(), self.getClusterUuid()));
                                    return;
                                }
                            ...
                            if (KVMGlobalConfig.CHECK_HOST_CPU_MODEL_NAME.value(Boolean.class) && !checkCpuModelOfHost()) {
                                trigger.fail(operr(ORG_ZSTACK_KVM_10120, "host [uuid:%s] cannot be added to cluster [uuid:%s] because cpu model name does not match",
                                        self.getUuid(), self.getClusterUuid()));
                                return;
                            }
    ...
    private NoRollbackFlow createCollectHostFactsFlow(final ConnectHostInfo info) {
        return new NoRollbackFlow() {
                ...
                new Http<>(hostFactPath, cmd, HostFactResponse.class)
                        .call(new ReturnValueCompletion<HostFactResponse>(trigger) {
                            ...
                            public void success(HostFactResponse ret) {
                                if (!ret.isSuccess()) {
                                    trigger.fail(operr(ORG_ZSTACK_KVM_10121, "operation error, because:%s", ret.getError()));
                                    return;
                                }

                                if (!checkVirtualizationEnabled(ret)) {
                                    trigger.fail(operr(ORG_ZSTACK_KVM_10122, "cannot find either 'vmx' or 'svm' in /proc/cpuinfo, please make sure you have enabled virtualization in your BIOS setting"));
                                    return;
                                }
            ...
            private void recordHardwareChangesAndCreateTag(PatternedSystemTag systemTag, String token, String newValue, ErrorCodeList errorCodeList) {
                ...
                errorCodeList.getCauses().add(operr(ORG_ZSTACK_KVM_10123, "host[uuid:%s]'s %s changed, old:%s, new:%s",
                        self.getUuid(),
                        systemTag.getTagFormat(),
                        oldValue, newValue));
    ...
    private void handle(final ShutdownHostMsg msg) {
        inQueue().name(String.format("shut-down-kvm-host-%s", self.getUuid()))
                ...
                .run(chain -> handleShutdownHost(msg, new NoErrorCompletion(chain) {
                    @Override
                    public void done() {
                        new HostBase.HostDisconnectedCanonicalEvent(msg.getHostUuid(), operr(ORG_ZSTACK_KVM_10124, "host[uuid:%s] becomes power off, send notify",
    ...
    private void handleShutdownHostByAgent(ShutdownHostMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(shutdownHost, cmd, ShutdownHostResponse.class).call(new ReturnValueCompletion<ShutdownHostResponse>(msg, completion) {
            ...
            public void success(KVMAgentCommands.ShutdownHostResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10125, "operation error, because:%s", ret.getError()));
                    bus.reply(msg, reply);
                    completion.done();
                    return;
                }
                ...
                thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask(msg, completion) {
                    ...
                    public boolean run() {
                        if (isTimeout()) {
                            if (!msg.isReturnEarly()) {
                                reply.setSuccess(false);
                                reply.setError(operr(ORG_ZSTACK_KVM_10126, "host[%s] not shutdown in %d seconds", msg.getHostUuid(), ctimeout));
                                bus.reply(msg, reply);
        ...
        new Http<>(cancelJob, cmd, CancelRsp.class).call(new ReturnValueCompletion<CancelRsp>(completion) {
            ...
            public void success(CancelRsp ret) {
                if (ret.isSuccess()) {
                    ...
                    completion.fail(Platform.operr(ORG_ZSTACK_KVM_10127, "%s", ret.getError()));
                }
            }
            ...
            new Http<>(hostCheckFilePath, cmd, CheckFileOnHostResponse.class).call(new ReturnValueCompletion<CheckFileOnHostResponse>(chain, msg) {
                ...
                public void success(CheckFileOnHostResponse response) {
                    if (response.isSuccess()) {
                        ...
                        reply.setError(Platform.operr(ORG_ZSTACK_KVM_10128, response.getError(),
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        if (self.getState() == HostState.PreMaintenance) {
                            trigger.fail(Platform.operr(ORG_ZSTACK_KVM_10129, "host is in the premaintenance state, cannot update os"));
                        } else if (self.getStatus() != HostStatus.Connected) {
                            trigger.fail(Platform.operr(ORG_ZSTACK_KVM_10130, "host is not in the connected status, cannot update os"));
                        } else {
                            trigger.next();
                        }
                    }
                        ...
                        new Http<>(updateHostOSPath, cmd, UpdateHostOSRsp.class)
                            ...
                            public void success(UpdateHostOSRsp ret) {
                                if (ret.isSuccess()) {
                                    ...
                                    trigger.fail(Platform.operr(ORG_ZSTACK_KVM_10131, "%s", ret.getError()));
                                }
                            }
    ...
    protected void scanVmPorts(ScanVmPortMsg msg) {
        ...
        new Http<>(scanVmPortPath, cmd, ScanVmPortResponse.class).call(new ReturnValueCompletion<ScanVmPortResponse>(msg) {
            ...
            public void success(ScanVmPortResponse ret) {
                if (!ret.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10132, "operation error, because:%s", ret.getError()));
                } else {
    ...
    private void doAttachVolume(AttachDataVolumeToHostMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(attachVolumePath, cmd, AttachVolumeRsp.class).call(new ReturnValueCompletion<AttachVolumeRsp>(msg) {
            ...
            public void success(AttachVolumeRsp rsp) {
                if (!rsp.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10133, "failed to attach volume to host, because:%s", rsp.getError()));
                    bus.reply(msg, reply);
                    completion.done();
                    return;
                }
    ...
    private void doDetachVolume(DetachDataVolumeFromHostMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(detachVolumePath, cmd, DetachVolumeRsp.class).call(new ReturnValueCompletion<DetachVolumeRsp>(msg) {
            ...
            public void success(DetachVolumeRsp rsp) {
                if (!rsp.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10134, "failed to detach volume from host, because:%s", rsp.getError()));
                } else {
                    new SQLBatch() {
                        @Override
                        protected void scripts() {
    ...
    private void doFstrimVm(FstrimVmMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(vmFstrimPath, cmd, VmFstrimRsp.class).call(new ReturnValueCompletion<VmFstrimRsp>(msg) {
            ...
            public void success(VmFstrimRsp rsp) {
                if (!rsp.isSuccess()) {
                    reply.setError(operr(ORG_ZSTACK_KVM_10135, "vm[%s] failed to fstrim, because:%s", msg.getVmUuid(), rsp.getError()));
                }
                bus.reply(msg, reply);
                completion.done();
            }
