    ...
    private void handle(final FenceVmOnHostMsg msg) {
        ...
        bus.send(fmsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply r) {
                ...
                if (!rsp.isSuccess()) {
                    logger.warn(String.format("[HA pre-fence] vm[%s] failed via sibling[%s] for suspect[%s]: agent-error=[%s]",
                            msg.getVmUuid(), peerHostUuid, suspectHostUuid,
                            rsp.getError()));
                ...
                logger.info(String.format("[HA pre-fence] vm[%s] cleared by sibling[%s] for suspect[%s]",
                        msg.getVmUuid(), peerHostUuid, suspectHostUuid));
                bus.reply(msg, reply);
            }
        });
