...
public class KVMHost extends HostBase implements Host {
    ...
    private void handle(UpdateHostNqnMsg msg) {
        UpdateHostNqnReply reply = new UpdateHostNqnReply();
                ...
                bus.send(kmsg, new CloudBusCallBack(msg) {
                    ...
                    public void run(MessageReply r) {
                        if (!r.isSuccess()) {
                            ...
                            reply.setError(operr("fail to update iscsi initiator name of host[uuid:%s], because %s", msg.getHostUuid(), r.getError()));
                        } else {
    ...
    private void updateHostOsInformation(String distro, String release, String version, String iscsiInitiatorName, String nqn) {
    ...
    private NoRollbackFlow createCollectHostFactsFlow(final ConnectHostInfo info) {
        return new NoRollbackFlow() {
            ...
            private void saveKvmHostRelatedFacts(HostFactResponse ret) {
                updateHostOsInformation(ret.getOsDistribution(), ret.getOsRelease(), ret.getOsVersion(), ret.getIscsiInitiatorName(), ret.getNqn());
