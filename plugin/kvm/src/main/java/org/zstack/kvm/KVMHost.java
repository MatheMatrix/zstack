...
public class KVMHost extends HostBase implements Host {
    ...
    private void uploadFile(UploadFileMsg msg, NoErrorCompletion completion) {
        ...
        new Http<>(fileDownloadPath, cmd, DownloadFileResponse.class).call(new ReturnValueCompletion<DownloadFileResponse>(msg) {
            ...
            private void taskTracker(DownloadFileResponse rsp) {
                tracker.addTrackTask(rsp.taskUuid, self.getUuid(), cmd.url);

                tracker.waitForCompleted(new ReturnValueCompletion<UploadFileReply>(msg) {
                    @Override
                    public void success(UploadFileReply r) {
                        bus.reply(msg, r);
                        completion.done();
                    }
                ...
                tracker.runTrackTask(rsp.taskUuid);
            }
