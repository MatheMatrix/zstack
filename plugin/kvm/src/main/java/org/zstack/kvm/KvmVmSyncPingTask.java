        ...
        bus.installBeforeDeliveryMessageInterceptor(new AbstractBeforeDeliveryMessageInterceptor() {
            ...
            public void beforeDeliveryMessage(Message msg) {
                ...
                if (msg instanceof VmInstanceMessage) {
                    ...
                    throw new OperationFailureException(operr(ORG_ZSTACK_KVM_10152, "cannot get vmUuid from msg %s", msg.getMessageName()));
                }
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (ret.isSuccess()) {
                    ...
                    ErrorCode errorCode = operr(ORG_ZSTACK_KVM_10153, "unable to do vm sync on host[uuid:%s, ip:%s] because %s", host.getUuid(), host.getManagementIp(), ret.getError());
    ...
    private void checkVmInShutdown(final List<String> vmInShutdowns, final Map<String, VmInstanceState> states) {
        ...
        for (String vmUuid : states.keySet()) {
            ...
            if (vmInShutdownMap.get(vmUuid) < vmInShutdownMaxnum) {
                ...
                data.setVmUuid(vmUuid);
                ErrorCode err = operr(ORG_ZSTACK_KVM_10154, "The vm[%s] state is in shutdown for a long time, check whether the vm is normal", vmUuid);
                data.setReason(err);
