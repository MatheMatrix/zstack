...
public class PhysicalServerApiInterceptor implements ApiMessageInterceptor {
    ...
    // Pre-check (serverUuid, roleType) uniqueness so the caller gets a named, actionable
    // error up-front; the handler still holds the PESSIMISTIC_WRITE lock as the race guard.
    ...
    private void validate(APICreatePhysicalServerMsg msg) {
        ...
        // Pre-check serialNumber uniqueness within zone so callers see a readable error
        // instead of a DB ConstraintViolationException; the unique key still guards races.
    ...
    private void validate(APICreateProvisionNetworkMsg msg) {
        ...
        // GATEWAY_PXE requires DHCP wiring to be usable as a PXE network; without this guard
        // the API persists a half-built Enabled ProvisionNetwork that fails later at
        // provisioning time. Surface every missing field up-front in one consolidated error.
