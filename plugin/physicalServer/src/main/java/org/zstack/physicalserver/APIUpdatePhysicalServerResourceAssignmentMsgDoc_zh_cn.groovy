...
doc {
    ...
    desc """增量更新指定物理服务器上一个Role的CPU和内存分配并异步触发同步。未填写的字段保持原值；同一物理服务器上的变更由服务端队列串行处理。ZBS Role会影响计算容量；COMPUTE和MANAGEMENT Role的软隔离不扣减计算容量。"""
        ...
        request {
			...
			params {
				...
				column {
					name "roleType"
					enclosedIn "updatePhysicalServerResourceAssignment"
					desc "节点Role：ZBS、MANAGEMENT或COMPUTE"
					location "url"
					type "String"
					optional false
					since "5.5.38"
