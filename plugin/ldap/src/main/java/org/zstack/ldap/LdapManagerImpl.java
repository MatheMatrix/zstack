...
public class LdapManagerImpl extends AbstractService implements LdapManager, LoginBackend {
    ...
    private void handle(APIAddLdapServerMsg msg) {
        ...
        if (Q.New(LdapServerVO.class)
                ...
                .count() == 1) {
            evt.setError(err(ORG_ZSTACK_LDAP_10005, LdapErrors.MORE_THAN_ONE_LDAP_SERVER,
    ...
    private void handle(APICreateLdapBindingMsg msg) {
        ...
        if (avo == null) {
            evt.setError(err(ORG_ZSTACK_LDAP_10006, LdapErrors.CANNOT_FIND_ACCOUNT,
        ...
        if (!ldapUtil.validateDnExist(ldapTemplateContextSource, fullDn)) {
            throw new OperationFailureException(err(ORG_ZSTACK_LDAP_10007, LdapErrors.UNABLE_TO_GET_SPECIFIED_LDAP_UID,
        ...
        try {
            ...
            if (ExceptionDSL.isCausedBy(e, SQLIntegrityConstraintViolationException.class)) {
                evt.setError(err(ORG_ZSTACK_LDAP_10008, LdapErrors.BIND_SAME_LDAP_UID_TO_MULTI_ACCOUNT,
    ...
    private void handle(APIUpdateLdapServerMsg msg) {
        ...
        if (ldapServerVO == null) {
            evt.setError(err(ORG_ZSTACK_LDAP_10009, LdapErrors.UNABLE_TO_GET_SPECIFIED_LDAP_SERVER_RECORD,
    ...
    public void login(LoginContext loginContext, ReturnValueCompletion<LoginSessionInfo> completion) {
        ...
        if (!isValid(ldapLoginName, loginContext.getPassword())) {
            completion.fail(err(ORG_ZSTACK_LDAP_10010, IdentityErrors.AUTHENTICATION_ERROR,
        ...
        if (vo == null) {
            completion.fail(err(ORG_ZSTACK_LDAP_10011, IdentityErrors.AUTHENTICATION_ERROR,
        ...
        if (avo == null) {
            ...
            ORG_ZSTACK_LDAP_10012,         "Account[uuid:%s] Not Found!!!", vo.getAccountUuid()));
            return;
        }
