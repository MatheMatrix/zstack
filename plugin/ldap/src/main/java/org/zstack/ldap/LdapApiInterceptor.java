...
public class LdapApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APIAddLdapServerMsg msg) {
        if (!LdapEffectiveScope.hasScope(msg.getScope())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_LDAP_10013, "unsupported LDAP/AD server scope"));
        }
    ...
    private void validate(LdapServerInventory inv) {
        ...
        if (errorCode != null) {
            throw new ApiMessageInterceptionException(
                    err(ORG_ZSTACK_LDAP_10014, LdapErrors.TEST_LDAP_CONNECTION_FAILED,
    ...
    private void validateLdapType(List<String> systemTags){
        ...
        if(!(LdapConstant.OpenLdap.TYPE.equals(type) || LdapConstant.WindowsAD.TYPE.equals(type))){
            throw new ApiMessageInterceptionException(
                    argerr(ORG_ZSTACK_LDAP_10015, "Wrong LdapServerType[%s], valid values: [%,%s]", type, LdapConstant.OpenLdap.TYPE, LdapConstant.WindowsAD.TYPE)
            );
        }
    }
    ...
    private void validateLdapServerExist(){
        if(!Q.New(LdapServerVO.class).isExists()){
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_LDAP_10016, "There is no LDAP/AD server in the system, Please add a LDAP/AD server first."));
        }
    }
    ...
    public ErrorCode testAddLdapServerConnection(LdapServerInventory inv) {
        ...
        try {
            ...
            return operr(ORG_ZSTACK_LDAP_10017, "Cannot connect to LDAP/AD server, Invalid Credentials, please checkout User DN and password");
        } catch (CommunicationException e) {
            logger.debug("Cannot connect to LDAP/AD server, communication false, please checkout IP, port and Base DN", e);
            return operr(ORG_ZSTACK_LDAP_10018, "Cannot connect to LDAP/AD server, communication false, please checkout IP, port and Base DN");
        } catch (Exception e) {
            logger.debug("Cannot connect to LDAP/AD server", e);
            return operr(ORG_ZSTACK_LDAP_10019, "Cannot connect to LDAP/AD server, %s", e.toString());
        }
