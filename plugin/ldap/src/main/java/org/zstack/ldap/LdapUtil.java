...
public class LdapUtil {
            ...
            searchResult = ldapTemplate.search("", filter, searchCtls, new AbstractContextMapper<Object>() {
                ...
                protected Object doMapFromContext(DirContextOperations ctx) {
                    ...
                    try {
                        ...
                        throw new OperationFailureException(operr(ORG_ZSTACK_LDAP_10000, "query ldap entry fail, %s", e.toString()));
                    }
                ...
                List<Object> subResult = ldapTemplate.search("", filter, searchCtls, new AbstractContextMapper<Object>() {
                    ...
                    protected Object doMapFromContext(DirContextOperations ctx) {
                        ...
                        try {
                            ...
                            logger.error("query ldap entry attributes fail", e.getCause());
                            throw new OperationFailureException(operr(ORG_ZSTACK_LDAP_10001, "query ldap entry fail, %s", e.toString()));
                        }
    ...
    public List<Object> searchLdapEntry(String ldapServerUuid, String filter, Integer count, String[] returningAttributes, ResultFilter resultFilter, boolean searchAllAttributes) {
        ...
        throw new OperationFailureException(operr(ORG_ZSTACK_LDAP_10002, "query ldap entry[filter: %s] fail, because %s", filter, errorMessage));
    }
    ...
    private String getFullUserDn(LdapTemplate ldapTemplate, String filter) {
        ...
        try {
            ...
            if (result.size() == 1) {
                ...
                ORG_ZSTACK_LDAP_10003,         LdapErrors.UNABLE_TO_GET_SPECIFIED_LDAP_UID, "More than one ldap search result"));
            } else {
                return "";
            }
            ...
            LdapServerVO ldapServerVO = getLdapServer();
            throw new OperationFailureException(err(
            ORG_ZSTACK_LDAP_10004,         LdapErrors.UNABLE_TO_GET_SPECIFIED_LDAP_UID, "You'd better check the LDAP/AD server[url:%s, baseDN:%s, encryption:%s, username:%s, password:******]" +
