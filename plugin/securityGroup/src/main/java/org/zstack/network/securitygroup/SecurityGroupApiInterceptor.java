    ...
    private void validatePorts(String ports) {
        ...
        if (ports.contains(SecurityGroupConstant.IP_SPLIT)) {
            ...
            if (tmpPorts.length > SecurityGroupConstant.PORT_GROUP_NUMBER_LIMIT) {
                throw new ApiMessageInterceptionException(err(ORG_ZSTACK_NETWORK_SECURITYGROUP_10065, SecurityGroupErrors.RULE_PORT_FIELD_ERROR, "invalid ports[%s], port range[%s] number[%d] is out of max limit[%d]", ports, Arrays.toString(tmpPorts), tmpPorts.length, SecurityGroupConstant.PORT_GROUP_NUMBER_LIMIT));
            }
    ...
    private void validateIps(String ips, Integer ipVersion) {
        ...
        if (ips.contains(SecurityGroupConstant.IP_SPLIT)) {
            ...
            if (ipArray.length > SecurityGroupConstant.IP_GROUP_NUMBER_LIMIT) {
                throw new ApiMessageInterceptionException(err(ORG_ZSTACK_NETWORK_SECURITYGROUP_10074, SecurityGroupErrors.RULE_IP_FIELD_ERROR, "invalid ips[%s], ip number[%d] is out of max limit[%d]", ips, ipArray.length, SecurityGroupConstant.IP_GROUP_NUMBER_LIMIT));
            }
