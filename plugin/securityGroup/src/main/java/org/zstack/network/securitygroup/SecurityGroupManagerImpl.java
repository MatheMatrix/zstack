...
public class SecurityGroupManagerImpl extends AbstractService implements SecurityGroupManager, ManagementNodeReadyExtensionPoint,
    ...
    public void validateSystemtagL3SecurityGroup(String l3Uuid, List<String> securityGroupUuids) {
        if (!Q.New(NetworkServiceL3NetworkRefVO.class).eq(NetworkServiceL3NetworkRefVO_.l3NetworkUuid, l3Uuid)
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SECURITYGROUP_10000, "the netwotk service[type:%s] not enabled on the l3Network[uuid:%s]", SecurityGroupConstant.SECURITY_GROUP_NETWORK_SERVICE_TYPE, l3Uuid));
        }
    }
    ...
    private void validate(AddVmNicToSecurityGroupMsg msg) {
        ...
        if (!new HashSet<>(uuids).containsAll(msg.getVmNicUuids())) {
            ...
            throw new OperationFailureException(err(ORG_ZSTACK_NETWORK_SECURITYGROUP_10002, SysErrors.RESOURCE_NOT_FOUND,
        ...
        if (!refs.isEmpty()) {
            refs.stream().forEach(ref -> {
                if (uuids.contains(ref.getVmNicUuid())) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SECURITYGROUP_10003, "vm nic[uuid:%s] has been attach to security group[uuid:%s]", ref.getVmNicUuid(), msg.getSecurityGroupUuid()));
                }
            });
        }
        ...
        for(VmNicVO nic : nics) {
            if (!Q.New(NetworkServiceL3NetworkRefVO.class).eq(NetworkServiceL3NetworkRefVO_.l3NetworkUuid, nic.getL3NetworkUuid())
                    .eq(NetworkServiceL3NetworkRefVO_.networkServiceType, SecurityGroupConstant.SECURITY_GROUP_NETWORK_SERVICE_TYPE).isExists()) {
                throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SECURITYGROUP_10004, "the netwotk service[type:%s] not enabled on the l3Network[uuid:%s] of nic[uuid:%s]", SecurityGroupConstant.SECURITY_GROUP_NETWORK_SERVICE_TYPE, nic.getL3NetworkUuid(), nic.getUuid()));
            }
            ...
            if (!AccountConstant.isAdminPermission(sgOwnerAccountUuid) && !AccountConstant.isAdminPermission(vmAccountUuid) && !sgOwnerAccountUuid.equals(vmAccountUuid)) {
                throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SECURITYGROUP_10005, "security group[uuid:%s] is not owned by account[uuid:%s] or admin", msg.getSecurityGroupUuid(), vmAccountUuid));
            }
        }
