...
public class SMPPrimaryStorageFactory implements PrimaryStorageFactory, CreateTemplateFromVolumeSnapshotExtensionPoint,
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (!q(PrimaryStorageClusterRefVO.class).eq(PrimaryStorageClusterRefVO_.primaryStorageUuid, psUuid).isExists()) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_SMP_10018, "the SMP primary storage[uuid:%s] is not attached" +
    ...
    public List<HostInventory> getConnectedHostForOperation(PrimaryStorageInventory pri, int startPage, int pageLimit) {
        if (pri.getAttachedClusterUuids().isEmpty()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_SMP_10019, "cannot find a Connected host to execute command for smp primary storage[uuid:%s]", pri.getUuid()));
        }
        ...
        if (ret.isEmpty() && startPage == 0) { //check is first page
            ...
            ORG_ZSTACK_STORAGE_PRIMARY_SMP_10020,         "cannot find a host which has Connected host-SMP connection to execute command for smp primary storage[uuid:%s]",
                    pri.getUuid()));
        } else {
