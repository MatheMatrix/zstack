...
public class KvmBackend extends HypervisorBackend {
    ...
    @Override
    void handle(DeleteVolumeSnapshotOnPrimaryStorageMsg msg, final ReturnValueCompletion<DeleteVolumeSnapshotOnPrimaryStorageReply> completion) {
        VolumeInventory vol = msg.getVolume();
        String hostUuid;
        String connectedHostUuid = primaryStorageFactory.getConnectedHostForOperation(getSelfInventory()).get(0).getUuid();
        if (vol.getVmInstanceUuid() != null) {
        ...
        if (msg.isOnline()) {
            ...
            bus.send(hmsg, new CloudBusCallBack(msg) {
                ...
                public void run(MessageReply r) {
                    ...
                    DeleteVolumeSnapshotOnPrimaryStorageReply ret = new DeleteVolumeSnapshotOnPrimaryStorageReply();
