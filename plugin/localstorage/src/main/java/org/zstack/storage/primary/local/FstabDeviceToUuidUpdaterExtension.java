...
public class FstabDeviceToUuidUpdaterExtension implements Component, ManagementNodeReadyExtensionPoint {
        ...
        tuples.forEach(tuple -> {
            ...
            hostVOS.forEach(host -> {
                ...
                try {
                    ...
                    String devicePath = fstabEntry.split("\\s+")[0];

                    // 4 Backup the current fstab file
                    ssh.command("cp -f /etc/fstab /etc/fstab.bak").run();

                    // 5 Get the UUID of the device using blkid
                    SshResult blkidResult = ssh.command(String.format("blkid -s UUID -o value '%s'", devicePath)).run();
                    if (blkidResult.getReturnCode() != 0 || blkidResult.getStdout().isEmpty()) {
                        logger.warn(String.format("Failed to get UUID for device[%s] on host[%s]: %s", devicePath, host.getUuid(), blkidResult.getStderr()));
                        return;
                    }
                    String uuid = blkidResult.getStdout().trim();

                    // 6 Delete the existing fstab entry using sed
                    SshResult delResult = ssh.command(String.format("sed -i '\\|^%s|d' /etc/fstab", devicePath.replace("/", "\\/"))).run();
                    if (delResult.getReturnCode() != 0) {
                        logger.warn(String.format("sed delete failed: " + delResult.getStderr()));
                        return;
                    }

                    // Update the fstab entry with UUID
                    String newFstabEntry = fstabEntry.replace(devicePath, String.format("UUID=%s", uuid));
                    SshResult appendResult = ssh.command(String.format("echo '%s' >> /etc/fstab", newFstabEntry)).run();
                    if (appendResult.getReturnCode() != 0) {
                        logger.warn("Failed to append new fstab entry: " + appendResult.getStderr());
                        return;
                    }

                    SshResult verifyResult = ssh.command(String.format("grep 'UUID=%s' /etc/fstab", uuid)).run();
                    ...
                    logger.info(String.format("Updated fstab for device[%s] to UUID[%s] on host[%s]",
                            devicePath, uuid, host.getUuid()));
                    ...
                    if (ssh != null) {
                        ...
                        if (rollback.getReturnCode() != 0) {
                            logger.error("CRITICAL: Rollback failed on host " + host.getUuid());
                        }
                    }
                } finally {
                    if (ssh != null) {
