...
public class ZbsStorageController implements PrimaryStorageControllerSvc, PrimaryStorageNodeSvc {
    ...
    public synchronized void activateHeartbeatVolume(HostInventory h, ReturnValueCompletion<HeartbeatVolumeTO> comp) {
        ...
        cmd.installPath = makeVolumeInstallPathByTargetPool(
                config.getPoolNames().get(0).split("/")[0],
                config.getPoolNames().get(0).split("/")[1],
                ZbsHelper.zbsHeartbeatVolumeName
        );
        ...
        httpCall(CREATE_VOLUME_PATH, cmd, CreateVolumeRsp.class, new ReturnValueCompletion<CreateVolumeRsp>(comp) {
            ...
            public void success(CreateVolumeRsp returnValue) {
                ...
                to.setInstallPath(cmd.installPath);
    ...
    public String allocateSpace(AllocateSpaceSpec aspec) {
        ...
        return buildVolumePath(config.getPoolNames().get(0).split("/")[0], config.getPoolNames().get(0).split("/")[1], "");
    }
    ...
    public void createVolume(CreateVolumeSpec v, ReturnValueCompletion<VolumeStats> comp) {
        ...
        httpCall(CREATE_VOLUME_PATH, cmd, CreateVolumeRsp.class, new ReturnValueCompletion<CreateVolumeRsp>(comp) {
            ...
            public void success(CreateVolumeRsp returnValue) {
                ...
                stats.setInstallPath(makeVolumeInstallPathByTargetPool(
                        config.getPoolNames().get(0).split("/")[0],
                        config.getPoolNames().get(0).split("/")[1],
    ...
    public static String makeVolumeInstallPathByTargetPool(String physicalPoolName, String logicalPoolName, String volName) {
        return String.format("cbd:%s/%s/%s", physicalPoolName, logicalPoolName, volName);
    }
