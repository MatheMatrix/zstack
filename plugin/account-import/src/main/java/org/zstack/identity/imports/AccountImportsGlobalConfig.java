...
public class AccountImportsGlobalConfig {
     ...
     */
    @GlobalConfigValidation(validValues = {"NoAction", "DestroyAccount", "DisableAccount"})
    @BindResourceConfig({AccountImportSourceVO.class})
    @GlobalConfigDef(defaultValue = "NoAction", description = "the strategy for deleted users when synchronizing accounts from remote server")
    public static GlobalConfig SYNC_RETIREES_STRATEGY = new GlobalConfig(CATEGORY, "sync.retirees.strategy");

    /**
     * @see org.zstack.identity.imports.entity.SyncNewcomersStrategy
     */
    @GlobalConfigValidation(validValues = {"NoAction", "CreateAccount"})
    @BindResourceConfig({AccountImportSourceVO.class})
    @GlobalConfigDef(defaultValue = "CreateAccount", description = "the strategy for newly created users when synchronizing accounts from remote server")
    public static GlobalConfig SYNC_NEWCOMERS_STRATEGY = new GlobalConfig(CATEGORY, "sync.newcomers.strategy");
}
