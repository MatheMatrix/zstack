...
public abstract class AbstractAccountSourceBase {
    ...
    public void handle(ImportThirdPartyAccountMsg message) {
        ...
        threadFacade.chainSubmit(new ChainTask(message) {
            ...
            public void run(SyncTaskChain chain) {
                importAccounts(message.getSpec(), new ReturnValueCompletion<List<AccountImportContext.Result>>(chain) {
                    @Override
                    public void success(List<AccountImportContext.Result> results) {
                        chain.next();
    ...
    public final void importAccounts(ImportAccountSpec batch, ReturnValueCompletion<List<AccountImportContext.Result>> completion) {
        ...
        chain.then(new NoRollbackFlow() {
            ...
            public void handle(Map data) {
                completion.success(transform(contexts, AccountImportContext::generateResult));
            }
    ...
    private void handle(BindThirdPartyAccountMsg message) {
        ...
        threadFacade.chainSubmit(new ChainTask(message) {
            @Override
            public void run(SyncTaskChain chain) {
                importAccounts(batch, new ReturnValueCompletion<List<AccountImportContext.Result>>(chain) {
                    @Override
                    public void success(List<AccountImportContext.Result> results) {
                        chain.next();
                        reply.setResults(results);
