...
public class HardwareVxlanNetworkPoolFactory implements L2NetworkFactory, GlobalApiMessageInterceptor {
    ...
    private void validate(APICreateL3NetworkMsg msg) {
        ...
        if (type.equals(SdnControllerConstant.HARDWARE_VXLAN_NETWORK_POOL_TYPE)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10006, "hardware vxlan network pool doesn't support create l3 network"));
        }
    }
    ...
    private void validate(APICreateL2HardwareVxlanNetworkPoolMsg msg) {
        if (msg.getPhysicalInterface() == null || msg.getPhysicalInterface().equals("")) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10007, "hardware vxlan network pool must configure the physical interface"));
        }
    }
    ...
    private void validate(APICreateL2VxlanNetworkMsg msg) {
        ...
        if (poolVO.getType().equals(SdnControllerConstant.HARDWARE_VXLAN_NETWORK_POOL_TYPE)
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10008, "ONLY hardware vxlan network can be created in hardware vxlan pool"));
        }

        if (!poolVO.getType().equals(SdnControllerConstant.HARDWARE_VXLAN_NETWORK_POOL_TYPE)
                && msg.getType().equals(SdnControllerConstant.HARDWARE_VXLAN_NETWORK_TYPE)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10009, "hardware vxlan network can ONLY be created in hardware vxlan pool"));
        }
    }
    ...
    private void validate(APICreateL2HardwareVxlanNetworkMsg msg) {
        ...
        if (msg.getZoneUuid() != null && !msg.getZoneUuid().equals(poolVO.getZoneUuid())) {
            throw new ApiMessageInterceptionException(Platform.err(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10010, SysErrors.INVALID_ARGUMENT_ERROR,
        ...
        if (msg.getVni() != null && NetworkUtils.generateVlanDeviceName(
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10011, "cannot create vlan-device on %s because it's too long"
