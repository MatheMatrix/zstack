...
public class H3cVcfcApiInterceptor implements ApiMessageInterceptor, GlobalApiMessageInterceptor {
    ...
    private void validate(APICreateVniRangeMsg msg) {
        ...
        if ( pool == null ) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10000, "Could not create VNI range because the specified L2 network [uuid:%s] is not a VXLAN network pool", msg.getL2NetworkUuid()));
        }
        ...
        if (msg.getStartVni() > 4094 || msg.getEndVni() > 4094) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10001, "Could not create VNI range [%s-%s] because H3C controllers use VNI as VLAN ID and the range must be within 1-4094", msg.getStartVni(), msg.getEndVni()));
        }
        ...
        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10002, "Could not create VNI range [%s-%s] because it is not covered by any of the SDN controller's configured VNI ranges", userVniRange.startVni, userVniRange.endVni));
    }
    ...
    private void validate(APIAddIpv6RangeMsg msg) {
        ...
        if (vo.getStatus() != SdnControllerStatus.Connected) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10003, "Could not add IPv6 range because the SDN controller [uuid:%s] is not connected. Current status: %s", 
    ...
    private void validate(APIAddIpv6RangeByNetworkCidrMsg msg) {
        ...
        if (vo.getStatus() != SdnControllerStatus.Connected) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10004, "Could not add IPv6 range by network CIDR because the SDN controller [uuid:%s] is not connected. Current status: %s", 
                    sdnControllerUuid, vo.getStatus()));
        }
    }
    ...
    private void validate(APIAddIpRangeMsg msg) {
        ...
        if (vo.getStatus() != SdnControllerStatus.Connected) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10005, "Could not add IP range because the SDN controller [uuid:%s] is not connected. Current status: %s", 
                    sdnControllerUuid, vo.getStatus()));
        }
    }
    ...
    private void validate(APIAddIpRangeByNetworkCidrMsg msg) {
        ...
        if (vo.getStatus() != SdnControllerStatus.Connected) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10006, "Could not add IP range by network CIDR because the SDN controller [uuid:%s] is not connected. Current status: %s", 
                    sdnControllerUuid, vo.getStatus()));
        }
    }
    ...
    private void validate(APICreateL3NetworkMsg msg) {
        ...
        if (SdnControllerConstant.H3C_VCFC_CONTROLLER.equals(vo.getVendorType()) &&
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10007, "can not create l3 network" +
    ...
    private void validate(APICreateL2HardwareVxlanNetworkMsg msg) {
        ...
        if (sdnControllerVO != null
            ...
            if (!tenantExist) {
                ...
                ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10008,         "Could not create hardware VXLAN network because tenant UUID is a mandatory parameter for the H3C VCFC V2 controller"));
            }
        }
