    ...
    public void check(final L2NetworkInventory l2Network, final String hostUuid, boolean noStatusCheck, final Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_SDNCONTROLLER_HARDWAREVXLAN_10005, "failed to check physical interface for HardwareVxlanPool[uuid:%s, name:%s] on kvm host[uuid: %s], %s",
