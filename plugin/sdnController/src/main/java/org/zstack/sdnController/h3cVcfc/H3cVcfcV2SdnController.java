...
public class H3cVcfcV2SdnController extends H3cVcfcSdnController {
    ...
    private void addL3NetworkIpRangeOnController(L3NetworkInventory inv, IpRangeInventory ipr, Completion completion) {
        try {
            ...
            if (subnet == null) {
                completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10024, "Could not add IP range because subnet creation failed on the SDN controller"));
                return;
            }
            ...
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10025, "Could not add IP range to L3 network because %s", e.getMessage()));
        }
    }
    ...
    private void deleteL3NetworkIpRangeOnController(L3NetworkInventory inv, IpRangeInventory ipr, Completion completion) {
        try {
            ...
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10026, "Could not delete IP range from L3 network because %s", e.getMessage()));
        }
    }
    ...
    private void getH3cControllerLeaderIp(Completion completion) {
        ...
        try {
            ...
            if (rsp == null) {
                completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10027, "Could not determine cluster leader because the SDN controller [ip:%s] did not respond", self.getIp()));
                return;
            }
            ...
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10028, "Could not determine cluster leader for SDN controller [ip:%s] because %s", self.getIp(), e.getMessage()));
        }
    }
    ...
    public void getH3cControllerToken(Completion completion) {
        ...
        try {
            ...
            if (rsp == null) {
                completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10029, "Could not authenticate with SDN controller because controller [ip:%s] did not respond", self.getIp()));
                return;
            }
            ...
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10030, "Could not authenticate with SDN controller [ip:%s] because %s", self.getIp(), e.getMessage()));
        }
    }
    ...
    public void getH3cVniRanges(Completion completion) {
        ...
        try {
            ...
            if (rsp == null) {
                completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10031, "Could not retrieve VNI ranges because the SDN controller [ip:%s] did not respond", self.getIp()));
                return;
            }
            ...
            if (count == 0) {
                completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10032, "Could not initialize SDN controller because no VNI ranges are configured on controller [ip:%s]", self.getIp()));
                return;
            }

            completion.success();
        } catch (Exception e) {
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10033, "Could not retrieve VNI ranges from SDN controller [ip:%s] because %s", self.getIp(), e.getLocalizedMessage()));
        }
    }
    ...
    private void doCreateVxlanNetworkOnController(L2NetworkInventory vxlan, APICreateL2HardwareVxlanNetworkMsg msg, Completion completion) {
        ...
        if (msg.getH3cTenantUuid() == null) {
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10034, "Could not create hardware VXLAN network because H3C tenant UUID is a mandatory parameter for the H3C VCFC V2 controller"));
            return;
        }
        H3cSdnControllerTenantVO tenantVO = Q.New(H3cSdnControllerTenantVO.class)
                ...
                .find();
        if (tenantVO == null) {
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10035, "Could not create hardware VXLAN network because the specified tenant does not exist in the SDN controller"));
            return;
        }
        ...
        try {
            ...
            if (rsp == null) {
                completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10036, "Could not create VXLAN network because the SDN controller [ip:%s] did not respond", self.getIp()));
                return;
            }
            ...
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10037, "Could not create VXLAN network on SDN controller [ip:%s] because %s", self.getIp(), e.getMessage()));
        }
    }
    ...
    private void doDeleteVxlanNetworkOnController(L2NetworkInventory vxlan, Completion completion) {
        ...
        try {
            ...
            completion.fail(operr(ORG_ZSTACK_SDNCONTROLLER_H3CVCFC_10038, "Could not delete VXLAN network on SDN controller [ip:%s] because %s", self.getIp(), e.getMessage()));
        }
    }
