...
public class XInfiniStorageController implements PrimaryStorageControllerSvc, PrimaryStorageNodeSvc {
    ...
    public String getActivePath(BaseVolumeInfo v, HostInventory h, boolean shareable) {
        ...
        throw new OperationFailureException(operr(ORG_ZSTACK_XINFINI_10014, "not supported protocol[%s]", v.getProtocol()));
    }
    ...
    public BaseVolumeInfo getActiveVolumeInfo(String activePath, HostInventory h, boolean shareable) {
        ...
        if (activePath.startsWith(getVhostSocketDir())) {
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_XINFINI_10015, "not supported get volume info from [%s]", activePath));
        }
    ...
    public List<ActiveVolumeClient> getActiveClients(String installPath, String protocol) {
        if (VolumeProtocol.Vhost.toString().equals(protocol)) {
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_XINFINI_10016, "not supported protocol[%s] for active", protocol));
        }
    }
    ...
    public void connect(String config, String url, ReturnValueCompletion<AddonInfo> comp) {
        ...
        if (connectedNode == null) {
            comp.fail(operr(ORG_ZSTACK_XINFINI_10017, "no connected node found"));
            return;
        }
    ...
    public void setVolumeQos(BaseVolumeInfo v, Completion comp) {
        ...
        if ((v.getQos().getReadBandwidth() != null && v.getQos().getReadBandwidth() > 0)
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_XINFINI_10024, "xinfini only support set total qos"));
        }
    ...
    public void export(ExportSpec espec, VolumeProtocol protocol, ReturnValueCompletion<RemoteTarget> comp) {
        if (protocol == VolumeProtocol.NVMEoF) {
            // TODO
            comp.fail(operr(ORG_ZSTACK_XINFINI_10025, "not support export nvmeof yet"));
        } else if (protocol == VolumeProtocol.iSCSI) {
    ...
    private IscsiRemoteTarget createIscsiRemoteTarget(String clientIp, String clientIqn, String installPath) {
        ...
        if (groupRelatedGateways.isEmpty()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_XINFINI_10026, "no active gateway found for client[%s]", clientIqn));
        }
    ...
    public void unexport(ExportSpec espec, RemoteTarget remoteTarget, VolumeProtocol protocol, Completion comp) {
        if (protocol == VolumeProtocol.NVMEoF) {
            comp.fail(operr(ORG_ZSTACK_XINFINI_10027, "not support unexport nvmeof yet"));
        } else if (protocol == VolumeProtocol.iSCSI) {
            unexportIscsi(espec.getInstallPath(), espec.getClientQualifiedName());
        } else {
            comp.fail(operr(ORG_ZSTACK_XINFINI_10028, "unsupported protocol %s", protocol.name()));
            return;
        }
        comp.success();
    }
