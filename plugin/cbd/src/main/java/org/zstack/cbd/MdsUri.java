...
public class MdsUri {
    ...
    public MdsUri(String url) {
        try {
            ...
            if (at == -1) {
                throw new OperationFailureException(operr(ORG_ZSTACK_CBD_10001, "invalid mdsUrl[%s], the sshUsername:sshPassword part is invalid. A valid mdsUrl is" +
                        " in format of %s", url, MDS_URL_FORMAT));
            }

            String userInfo = url.substring(0, at);
            if (!userInfo.contains(":")) {
                throw new OperationFailureException(operr(ORG_ZSTACK_CBD_10002, "invalid mdsUrl[%s], the sshUsername:sshPassword part is invalid. A valid mdsUrl is" +
                        " in format of %s", url, MDS_URL_FORMAT));
            }
            ...
            if (ssh.length != 2 || ssh[0].isEmpty() || ssh[1].isEmpty()) {
                throw new OperationFailureException(operr(ORG_ZSTACK_CBD_10003, "invalid mdsUrl[%s]. SSH username and password must be separated by ':' and cannot be empty. A valid monUrl format is %s", url, MDS_URL_FORMAT));
            }
            ...
            if (hostname == null) {
                throw new OperationFailureException(operr(ORG_ZSTACK_CBD_10004, "invalid mdsUrl[%s], hostname cannot be null. A valid mdsUrl is" +
                                " in format of %s", url, MDS_URL_FORMAT)
                );
            }
            ...
            if (sshPort < 1 || sshPort > 65535) {
                throw new OperationFailureException(operr(ORG_ZSTACK_CBD_10005, "invalid mdsUrl[%s], the ssh port is greater than 65535 or smaller than 1. A valid mdsUrl is" +
                                " in format of %s", url, MDS_URL_FORMAT)
                );
            }
