...
public class LoadBalancerApiInterceptor implements ApiMessageInterceptor, GlobalApiMessageInterceptor {
    ...
    private void validate(APICreateLoadBalancerListenerMsg msg) {
        ...
        if (LB_PROTOCOL_HTTP.equals(msg.getProtocol()) || LB_PROTOCOL_HTTPS.equals(msg.getProtocol())) {
            if (LoadBalancerConstants.BALANCE_ALGORITHM_ROUND_ROBIN.equals(algorithm) || LoadBalancerConstants.BALANCE_ALGORITHM_WEIGHT_ROUND_ROBIN.equals(algorithm)) {
                ...
                if (LoadBalancerConstants.HTTP_MODE_HTTP_TUNNEL.equals(httpMode) && LoadBalancerSessionPersistence.rewrite.toString().equals(enableSession)) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10085, "loadBalancer[%s] listener[%s] doesn't support assigning session persistence rewrite when the http mode is http-tunnel", msg.getLoadBalancerUuid(), msg.getName()));
                }
                ...
                for (String tag : msg.getSystemTags()) {
                    if (LoadBalancerSystemTags.SESSION_PERSISTENCE.isMatch(tag)) {
                        ...
                        if (!LoadBalancerSessionPersistence.iphash.toString().equals(enableSession)) {
                            ...
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10086, "loadBalancer[%s] listener[%s] doesn't support assigning other session persistence when the source balancer algorithm is source", msg.getLoadBalancerUuid(), msg.getName()));
                        }
                    }
                }
            }
                ...
                if (LoadBalancerSystemTags.SESSION_PERSISTENCE.isMatch(tag)) {
                    ...
                    if (LoadBalancerSessionPersistence.iphash.toString().equals(enableSession)) {
                        if (!LoadBalancerConstants.BALANCE_ALGORITHM_LEAST_SOURCE.equals(algorithm)) {
                            ...
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10087, "loadBalancer[%s] listener[%s] doesn't support assigning session persistence iphash", msg.getLoadBalancerUuid(), msg.getName()));
                        }
                    }
                }
            }
                ...
                * */
                if (LoadBalancerSystemTags.MAX_CONNECTION.isMatch(tag)) {
                    ...
                    if (Long.parseLong(s) > LoadBalancerConstants.MAX_CONNECTION_LIMIT) {
                        throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10088, "invalid max connection[%s], %s is larger than upper threshold %d", tag, s, LoadBalancerConstants.MAX_CONNECTION_LIMIT));
                    }
                }
        ...
        if (luuid != null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10089, "conflict loadBalancerPort[%s], a listener[uuid:%s] has used that port", msg.getLoadBalancerPort(), luuid));
        }

        if (msg.getSecurityPolicyType() != null) {
            if (!msg.getProtocol().equals(LB_PROTOCOL_HTTPS)) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10090, "the listener with protocol [%s] doesn't support select security policy", msg.getProtocol(), msg.getHealthCheckProtocol()));
            }
        }
        ...
        if (!CollectionUtils.isEmpty(msg.getHttpVersions())) {
            if (!LoadBalancerConstants.LB_PROTOCOL_HTTPS.equals(msg.getProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10091, "cloud not change the loadbalancer listener, because the listener with protocol [%s] doesn't support select http version:[%s]",
                                msg.getProtocol(), msg.getHttpVersions()));
            }

            if (hasNotSupportedHttpVersion(msg.getHttpVersions())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10092, "cloud not create the loadbalancer listener, because the listener with protocol https only support http version:[h1, h2]"));
            }
        ...
        if (!StringUtils.isEmpty(msg.getTcpProxyProtocol())) {
            if (!LoadBalancerConstants.LB_PROTOCOL_TCP.equals(msg.getProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10093, "cloud not create the loadbalancer listener, because the listener with protocol tcp only support tcp proxy protocol param"));
            }

            if (!LbSupportTcpProxyProtocol.contains(msg.getTcpProxyProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10094, "cloud not create the loadbalancer listener, because only support tcp proxy protocol %s", LbSupportTcpProxyProtocol));
            }
        ...
        if (!CollectionUtils.isEmpty(msg.getHttpCompressAlgos())) {
            if (!LoadBalancerConstants.LB_PROTOCOL_HTTPS.equals(msg.getProtocol()) && !LB_PROTOCOL_HTTP.equals(msg.getProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10095, "cloud not create the loadbalancer listener, because the listener with protocol [%s] doesn't support compress content",
                                msg.getProtocol(), msg.getHttpVersions()));
            }

            if (hasNotSupportedHttpCompressAlgos(msg.getHttpCompressAlgos())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10096, "cloud not create the loadbalancer listener, because only support compress algos[%s]", LbSupportHttpCompressAlgos));
            }
    ...
    private void validate(APIAddCertificateToLoadBalancerListenerMsg msg) {
        ...
        if (!vo.getProtocol().equals(LoadBalancerConstants.LB_PROTOCOL_HTTPS)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10097, "loadbalancer listener with type %s does not need certificate", vo.getProtocol()));
        }
        ...
        if (Q.New(LoadBalancerListenerCertificateRefVO.class).eq(LoadBalancerListenerCertificateRefVO_.listenerUuid, msg.getListenerUuid()).isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10098, "loadbalancer listener [uuid:%s] already had certificate[uuid:%s]",
    ...
    private void validate(APIRemoveCertificateFromLoadBalancerListenerMsg msg) {
        if (!Q.New(LoadBalancerListenerCertificateRefVO.class).eq(LoadBalancerListenerCertificateRefVO_.listenerUuid, msg.getListenerUuid())
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10099, "certificate [uuid:%s] is not added to loadbalancer listener [uuid:%s]",
    ...
    private void validate(APIChangeLoadBalancerListenerMsg msg) {
        ...
        if (target != null) {
            if (!LoadBalancerConstants.HEALTH_CHECK_TARGET_DEFAULT.equals(target)) {
                try {
                    ...
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10101, "healthCheck target [%s] error, it must be 'default' or number between[1~65535] ",
                            target));
                }
            }
        }
        ...
        if ((msg.getRedirectPort() != null || msg.getStatusCode() != null) && (msg.getHttpRedirectHttps() == null || HttpRedirectHttps.disable.toString().equals(msg.getHttpRedirectHttps()))) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10102, "could not assign redirect port or status code without specifying http redirect https"));
        }

        if (HttpRedirectHttps.enable.toString().equals(msg.getHttpRedirectHttps())) {
            if (!LB_PROTOCOL_HTTP.equals(listener.getProtocol())) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10103, "could not support protocols other than HTTP when specifying http redirect https"));
            }
            ...
            if (sessionPersistence || (msg.getSessionPersistence() != null && !LoadBalancerSessionPersistence.disable.toString().equals(msg.getSessionPersistence()))) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10104, "could not support both HTTP redirect HTTPS and session persistence at the same time"));
            }
        }
        ...
        if (LoadBalancerConstants.LB_PROTOCOL_UDP.equals(listener.getProtocol()) || LoadBalancerConstants.LB_PROTOCOL_TCP.equals(listener.getProtocol())) {
            if (msg.getSessionPersistence() != null || msg.getSessionIdleTimeout() != null || msg.getCookieName() != null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10105, "l4[%s] loadBalancer listener[%s] doesn't support modifying session persistence state", listener.getProtocol(), listener.getName()));
            }
        }
        ...
        if (LoadBalancerConstants.LB_PROTOCOL_HTTP.equals(listener.getProtocol())) {
            ...
            if (httpRedirectHttps && (msg.getSessionPersistence() != null && !LoadBalancerSessionPersistence.disable.toString().equals(msg.getSessionPersistence()))) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10106, "could not support both HTTP redirect HTTPS and session persistence at the same time"));
            }
            if (LoadBalancerSessionPersistence.rewrite.toString().equals(msg.getSessionPersistence()) && LoadBalancerConstants.HTTP_MODE_HTTP_TUNNEL.equals(msg.getHttpMode())) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10107, "listener[%s] can not modifying session persistence rewrite when the http mode is http-tunnel", msg.getUuid()));
            }
            if (LoadBalancerSessionPersistence.rewrite.toString().equals(msg.getSessionPersistence()) && msg.getHttpMode() == null) {
                Boolean httpModeTunnel = Q.New(SystemTagVO.class).eq(SystemTagVO_.resourceType, LoadBalancerListenerVO.class.getSimpleName())
                        ...
                        .eq(SystemTagVO_.resourceUuid, listener.getUuid()).isExists();
                if (httpModeTunnel) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10108, "listener[%s] can not modifying session persistence rewrite when the http mode is http-tunnel", msg.getUuid()));
                }
            }
            if (LoadBalancerConstants.HTTP_MODE_HTTP_TUNNEL.equals(msg.getHttpMode()) && msg.getSessionPersistence() == null) {
                Boolean cookieRewrite = Q.New(SystemTagVO.class).eq(SystemTagVO_.resourceType, LoadBalancerListenerVO.class.getSimpleName())
                        ...
                        .eq(SystemTagVO_.resourceUuid, listener.getUuid()).isExists();
                if (cookieRewrite) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10109, "listener[%s] can not modifying httpMode http-tunnel when the session persistence is rewrite", msg.getUuid()));
                }
            }
        }
        ...
        if (!LoadBalancerConstants.BALANCE_ALGORITHM_LEAST_SOURCE.equals(msg.getBalancerAlgorithm()) && LoadBalancerSessionPersistence.iphash.toString().equals(msg.getSessionPersistence())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10110, "listener[%s] changes session persistence to iphash, it must specify source balancer algorithm", msg.getUuid()));
        }
        ...
        if (msg.getBalancerAlgorithm() == null && (msg.getSessionPersistence() != null || msg.getSessionIdleTimeout() != null || msg.getCookieName() != null)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10111, "listener[%s] modifies session persistence, it must specify balancer algorithm", msg.getUuid()));
        }
        ...
        if (LoadBalancerConstants.BALANCE_ALGORITHM_LEAST_SOURCE.equals(msg.getBalancerAlgorithm())) {
            if ((!LoadBalancerSessionPersistence.iphash.toString().equals(msg.getSessionPersistence()) && msg.getSessionPersistence() != null) || msg.getSessionIdleTimeout() != null || msg.getCookieName() != null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10112, "listener[%s] %s algorithm doesn't support modifying session persistence except assigning iphash explicitly", msg.getUuid(), msg.getBalancerAlgorithm()));
            }
        ...
        if (LoadBalancerConstants.BALANCE_ALGORITHM_LEAST_CONN.equals(msg.getBalancerAlgorithm())) {
            if ((!LoadBalancerSessionPersistence.disable.toString().equals(msg.getSessionPersistence()) && msg.getSessionPersistence() != null) || msg.getSessionIdleTimeout() != null || msg.getCookieName() != null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10113, "listener[%s] %s algorithm doesn't support modifying session persistence except assigning disable explicitly", msg.getUuid(), msg.getBalancerAlgorithm()));
            }
        ...
        if (BALANCE_ALGORITHM_ROUND_ROBIN.equals(msg.getBalancerAlgorithm()) || BALANCE_ALGORITHM_WEIGHT_ROUND_ROBIN.equals(msg.getBalancerAlgorithm())) {
            ...
            if (msg.getSessionPersistence() == null && (msg.getSessionIdleTimeout() != null || msg.getCookieName() != null)) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10114, "listener[%s] doesn't support modifying idle timeout and cookie name, it must specify session persistence", msg.getUuid()));
            }
            ...
            if (!LoadBalancerSessionPersistence.insert.toString().equals(msg.getSessionPersistence()) && msg.getSessionIdleTimeout() != null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10115, "listener[%s] doesn't support modifying idle timeout when the session persistence is not insert", msg.getUuid()));
            }
            ...
            if (!LoadBalancerSessionPersistence.rewrite.toString().equals(msg.getSessionPersistence()) && msg.getCookieName() != null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10116, "listener[%s] doesn't support modifying cookie name when the session persistence is not rewrite", msg.getUuid()));
            }
            /*can not modify session persistence rewrite without modifying session cookie name*/
            if (LoadBalancerSessionPersistence.rewrite.toString().equals(msg.getSessionPersistence()) && msg.getCookieName() == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10117, "listener[%s] doesn't support modifying session rewrite without modifying cookie name", msg.getUuid()));
            }
            if (LoadBalancerSessionPersistence.insert.toString().equals(msg.getSessionPersistence()) && msg.getSessionIdleTimeout() == null) {
        ...
        if (timeout != null) {
            if (timeout < LoadBalancerConstants.SESSION_IDLE_TIMEOUT_MIN || timeout > LoadBalancerConstants.SESSION_IDLE_TIMEOUT_MAX) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10118, "invalid session idle timeout[%s], it must be the number between[%s~%s]", timeout, LoadBalancerConstants.SESSION_IDLE_TIMEOUT_MIN, LoadBalancerConstants.SESSION_IDLE_TIMEOUT_MAX));
            }
        }
        ...
        if (msg.getHealthCheckProtocol() != null && LoadBalancerConstants.HEALTH_CHECK_TARGET_PROTOCL_HTTP.equals(msg.getHealthCheckProtocol())) {
            ...
            if (!msg.getHealthCheckProtocol().equals(ts[0]) && msg.getHealthCheckURI() == null) {
                throw new ApiMessageInterceptionException(
                        operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10119, "the http health check protocol must be specified its healthy checking parameter healthCheckURI"));
            }
        }
        ...
        if (msg.getHealthCheckHttpCode() != null) {
            if (!verifyHttpCode(msg.getHealthCheckHttpCode())) {
                throw new ApiMessageInterceptionException(
                        operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10120, "the http health check protocol's expecting code [%s] is invalidate", msg.getHealthCheckHttpCode()));
            }
        }
        ...
        if (msg.getSecurityPolicyType() != null) {
            if (!listenerVO.getProtocol().equals(LB_PROTOCOL_HTTPS)) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10121, "the listener with protocol [%s] doesn't support select security policy", listenerVO.getProtocol(), msg.getHealthCheckProtocol()));
            }
        }
        if (LoadBalancerConstants.HEALTH_CHECK_TARGET_PROTOCL_HTTP.equals(msg.getHealthCheckProtocol())) {
            ...
            if (ts.length != 2) {
                throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10122, "invalid health target[%s], the format is targetCheckProtocol:port, for example, tcp:default", target));
            }
            ...
            if (LoadBalancerConstants.LB_PROTOCOL_UDP.equals(listenerVO.getProtocol()) && !LoadBalancerConstants.HEALTH_CHECK_TARGET_PROTOCL_UDP.equals(msg.getHealthCheckProtocol()) ||
                ...
                throw new ApiMessageInterceptionException(
                        operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10123, "the listener with protocol [%s] doesn't support this health check:[%s]",
            ...
            if (LoadBalancerConstants.HEALTH_CHECK_TARGET_PROTOCL_TCP.equals(ts[0]) && (msg.getHealthCheckMethod() == null || msg.getHealthCheckURI() == null)) {
                throw new ApiMessageInterceptionException(
                        operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10124, "the http health check protocol must be specified its healthy checking parameters including healthCheckMethod and healthCheckURI"));
            }
        }
        ...
        if (!CollectionUtils.isEmpty(msg.getHttpVersions())) {
            if (!LoadBalancerConstants.LB_PROTOCOL_HTTPS.equals(listener.getProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10125, "cloud not change the loadbalancer listener, because the listener with protocol [%s] doesn't support select http version:[%s]",
                                listenerVO.getProtocol(), msg.getHttpVersions()));
            }

            if (hasNotSupportedHttpVersion(msg.getHttpVersions())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10126, "cloud not change the loadbalancer listener, because the listener with protocol https only support http version:[%s]", LbSupportHttpVersion));
            }
        }
        ...
        if (!StringUtils.isEmpty(msg.getTcpProxyProtocol())) {
            if (!LoadBalancerConstants.LB_PROTOCOL_TCP.equals(listener.getProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10127, "cloud not change the loadbalancer listener, because the listener with protocol tcp only support tcp proxy protocol for param"));
            }

            if (!LbSupportTcpProxyProtocol.contains(msg.getTcpProxyProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10128, "cloud not change the loadbalancer listener, because only support tcp proxy protocol %s", LbSupportTcpProxyProtocol));
            }
        }
        ...
        if (!CollectionUtils.isEmpty(msg.getHttpCompressAlgos())) {
            if (!LoadBalancerConstants.LB_PROTOCOL_HTTPS.equals(listener.getProtocol()) && !LB_PROTOCOL_HTTP.equals(listener.getProtocol())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10129, "cloud not change the loadbalancer listener, because the listener with protocol [%s] doesn't support compress content",
                                listenerVO.getProtocol(), msg.getHttpVersions()));
            }

            if (hasNotSupportedHttpCompressAlgos(msg.getHttpCompressAlgos())) {
                throw new ApiMessageInterceptionException(
                        argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10130, "cloud not change the loadbalancer listener, because only support compress algos[%s]", LbSupportHttpCompressAlgos));
            }
        }
    ...
    private void validate(APICreateLoadBalancerServerGroupMsg msg){
        ...
        if (msg.getIpVersion() == null) {
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10131, "invalid ip version[%s], it must be %s or %s", msg.getIpVersion(), IPv6Constants.IPv4, IPv6Constants.IPv6));
        }
    }
    ...
    private void validate(APIDeleteLoadBalancerServerGroupMsg msg){
        ...
        if(Q.New(LoadBalancerVO.class)
                ...
                .isExists()){
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10132, "could not allow to delete default serverGroup[uuid:%s]",msg.getUuid()));
        }	
    }
    ...
    private void validate(APIAddBackendServerToServerGroupMsg msg){
        ...
        if (loadBalancerUuid == null ) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10133, "loadbalacerServerGroup [%s] is non-existent", msg.getServerGroupUuid()));
        }
        ...
        if(vmNics != null && !vmNics.isEmpty()){
            for(Map<String,String> vmNic:vmNics){
                if(vmNic.containsKey("uuid")){
                    ...
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10134, "could not add backend server vmnic to serverGroup[uuid:%s],because vmnic uuid is null",msg.getServerGroupUuid()));
                }
                ...
                boolean isVmNicExist = Q.New(VmNicVO.class)
                        ...
                        .isExists();
                if(!isVmNicExist){
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10135, "could not add backend server vmnic[uuid:%s] to serverGroup[uuid:%s],because vmnic uuid is not exist",vmNic.get("uuid"),msg.getServerGroupUuid()));
                }
                ...
                if(vmNic.containsKey("weight") && vmNic.get("weight")!=null){
                    try{
                        ...
                        if (vmNicWeight < LoadBalancerConstants.BALANCER_WEIGHT_MIN || vmNicWeight > LoadBalancerConstants.BALANCER_WEIGHT_MAX) {
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10136, "invalid balancer weight[vimNic:%s,weight:%s], weight is not in the range [%d, %d]",
                                    vmNic.get("uuid"), vmNicWeight, LoadBalancerConstants.BALANCER_WEIGHT_MIN, LoadBalancerConstants.BALANCER_WEIGHT_MAX));
                        }
                    }catch (Exception e) {
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10137, "could not add backend server vmnic to serverGroup[uuid:%s] ,because vmnic weight[%s] not a correct number",vmNic.get("weight")));
                    }
                }
            ...
            if (l3Uuids.size() > 0) {
                throw new ApiMessageInterceptionException(
                        operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10138, "L3 networks[uuids:%s] of the vm nics has no network service[%s] enabled",
                                l3Uuids, LoadBalancerConstants.LB_NETWORK_SERVICE_TYPE_STRING));
            }
            ...
            if (!existingNics.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10139, "the vm nics[uuid:%s] are already on the load balancer servegroup [uuid:%s]", existingNics, msg.getServerGroupUuid()));
            }
            ...
            if(!Collections.disjoint(usedIps,vmNicIps)){
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10140, "could not add backend server vmnic to serverGroup [uuid:%s], because vmnic ip [ipAddress:%s] is repeated",msg.getServerGroupUuid(),vmNicIps));
            }else{
            ...
            if (!listenerUuids.isEmpty()) {
                ...
                for (LoadBalancerListenerVO listenerVO : listenerVOS) {
                    if (listenerVO.getAttachedVmNics().stream().anyMatch(uuid -> vmNicUuids.contains(uuid))) {
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10141, "could not add vm nic [uuid:%s] to server group" +
        ...
        if(servers != null && !servers.isEmpty()){
            for(Map<String,String> server:servers){
                if(server.containsKey("ipAddress") && NetworkUtils.isIpAddress(server.get("ipAddress"))){
                    if(usedIps.contains(server.get("ipAddress"))){
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10142, "could not add backend server ip to serverGroup [uuid:%s], because ip [ipAddress:%s] is repeated",msg.getServerGroupUuid(),server.get("ipAddress")));
                    }
                    serverIps.add(server.get("ipAddress"));
                }else{
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10143, "could not add backend server ip to serverGroup [uuid:%s], because ip [ipAddress:%s] is invalid",msg.getServerGroupUuid(),serverIps));
                }
                ...
                if(server.containsKey("weight") && server.get("weight")!=null){
                    try{
                        ...
                        if (serverIpWeight < LoadBalancerConstants.BALANCER_WEIGHT_MIN || serverIpWeight > LoadBalancerConstants.BALANCER_WEIGHT_MAX) {
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10144, "invalid  weight[serverIp:%s,weight:%s], weight is not in the range [%d, %d]",
                                    server.get("ipAddress"), serverIpWeight, LoadBalancerConstants.BALANCER_WEIGHT_MIN, LoadBalancerConstants.BALANCER_WEIGHT_MAX));
                        }
                    }catch (Exception e) {
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10145, "could not add backend server ip to serverGroup[uuid:%s] ,because vmnic weight[%s] not a correct number",server.get("weight")));
                    }
                }
            }
            ...
            if (!existingServerIps.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10146, "the server ips [uuid:%s] are already on the load balancer servegroup [uuid:%s]", existingServerIps, msg.getServerGroupUuid()));
            }

            if (lbVO.getType() == LoadBalancerType.Shared) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10147, "could not add server ip to share load balancer server group"));
            }
            canAddServerIp = true;
        }
        ...
        if( canAddVmNic || canAddServerIp){
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10148, "vmnic or ip is null"));
        }
    }
    ...
    private void validate(APIRemoveBackendServerFromServerGroupMsg msg){
        ...
        if(vmNicUuids != null && !vmNicUuids.isEmpty()){
            ...
            if(existingNics.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10149, "vmnics are all not in servergroup [%s]",msg.getServerGroupUuid()));
            }else{
                isNicExist = true;
        ...
        if(serverIps!=null && !serverIps.isEmpty()){
            Set<String> existingServerIps = new HashSet<>(Q.New(LoadBalancerServerGroupServerIpVO.class)
                    ...
                    .listValues());
            if(existingServerIps.isEmpty()){
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10150, "serverips are all not in servergroup [%s]", msg.getServerGroupUuid()));
            }else{
                isIpExist = true;
        ...
        if(isNicExist || isIpExist ){
            String loadBalancerUuid = Q.New(LoadBalancerServerGroupVO.class)
                    ...
                    .findValue();
            if (loadBalancerUuid == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10151, "loadbalacerServerGroup [%s] is non-existent", msg.getServerGroupUuid()));
            }
            msg.setLoadBalancerUuid(loadBalancerUuid);
        }else{
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10152, "vmnic or ip is null"));
        }
    }
    ...
    private void validate(APIAddServerGroupToLoadBalancerListenerMsg msg){
        ...
        if(existingRefs != null && !existingRefs.isEmpty()){
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10153, "could not add server group[uuid:%s} to listener [uuid:%s] because it is already added ",
            ...
            if (!newVmNicUuids.isEmpty() && !oldVmNicUuids.isEmpty()) {
                for (String nicUuid : newVmNicUuids) {
                    if (oldVmNicUuids.contains(nicUuid)) {
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10154, "could not add server group[uuid:%s} to listener [uuid:%s] because nic [uuid:%s] is already added",
            ...
            if (!newServerIps.isEmpty() && !oldServerIps.isEmpty()) {
                for (String ipAddress : newServerIps) {
                    if (oldServerIps.contains(ipAddress)) {
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10155, "could not add server group[uuid:%s} to listener [uuid:%s] because server ip [%s] is already added",
        ...
        * ipv6 vip --> ipv6 backend  */
        ...
        if (listenerVO.getProtocol().equals(LB_PROTOCOL_UDP)) {
            if (!StringUtils.isEmpty(lbVO.getVipUuid()) &&
                ...
                ORG_ZSTACK_NETWORK_SERVICE_LB_10156,         "could not add server group[uuid:%s} to listener [uuid:%s], " +
            ...
            if (groupVO.getIpVersion() == IPv6Constants.IPv4 && !StringUtils.isEmpty(lbVO.getIpv6VipUuid())) {
                throw new ApiMessageInterceptionException(operr(
                ORG_ZSTACK_NETWORK_SERVICE_LB_10157,         "could not add server group[uuid:%s} to listener [uuid:%s], " +
            ...
            if (groupVO.getIpVersion() == IPv6Constants.IPv6 && !StringUtils.isEmpty(lbVO.getVipUuid())) {
                throw new ApiMessageInterceptionException(operr(
                ORG_ZSTACK_NETWORK_SERVICE_LB_10158,         "could not add server group[uuid:%s} to listener [uuid:%s], " +
    ...
    private void validate(APIRemoveServerGroupFromLoadBalancerListenerMsg msg){
        ...
        if (httpRedirectHttps) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10159, "could not  remove server groups[uuid:%s} from listener [uuid:%s] because it has enable HTTP redirect HTTPS", msg.getServerGroupUuid(), msg.getListenerUuid()));
        }
        ...
        List<LoadBalancerListenerServerGroupRefVO> existingRefs
                ...
                .list();
        if(existingRefs == null || existingRefs.isEmpty()){
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10160, "could not remove server group[uuid:%s} from listener [uuid:%s] because it is not added",msg.getServerGroupUuid(),msg.getListenerUuid()));
        }
    ...
    private boolean isExist(String loadBalancerUuid){
        long count = Q.New(LoadBalancerVO.class)
                ...
                .count();
        if(count == 0){
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10161, "loadbalacerUuid [%s] is non-existent",loadBalancerUuid));
        }else{
            return true;
        }
    }
    ...
    private void validate(APIChangeLoadBalancerBackendServerMsg msg){
        ...
        String loadBalancerUuid = Q.New(LoadBalancerServerGroupVO.class)
                ...
                .findValue();
        if (loadBalancerUuid == null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10162, "could not find loadBalancer with serverGroup [uuid:%s]",msg.getServerGroupUuid()));
        }
        LoadBalancerVO lbVO = dbf.findByUuid(loadBalancerUuid, LoadBalancerVO.class);
            ...
            for(Map<String,String> vmNic:vmNics){
                if(vmNic.containsKey("uuid")){
                    ...
                    if(serverGroupVmNicRefVO == null){
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10163, "could not update backend server vmnic of serverGroup,because serverGroup[uuid:%s] don not have vmnic [uuid:%s] ",msg.getServerGroupUuid(),vmNic.containsKey("uuid")));
                    }
                    ...
                    if(vmNic.containsKey("weight")){
                        if(vmNic.containsKey("weight") && vmNic.get("weight")!=null){
                            try{
                                Long vmNicWeight = Long.valueOf(vmNic.get("weight"));
                                if (vmNicWeight < LoadBalancerConstants.BALANCER_WEIGHT_MIN || vmNicWeight > LoadBalancerConstants.BALANCER_WEIGHT_MAX) {
                                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10164, "invalid balancer weight[vimNic:%s,weight:%s], weight is not in the range [%d, %d]",
                                            vmNic.get("uuid"), vmNicWeight, LoadBalancerConstants.BALANCER_WEIGHT_MIN, LoadBalancerConstants.BALANCER_WEIGHT_MAX));
                                }
                                ...
                                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10165, "could not change backend server vmnic to serverGroup[uuid:%s] ,because vmnic weight[%s] not a correct number",vmNic.get("weight")));
                            }
                        }

                    }else{
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10166, "invalid balancer weight[vimNic:%s], weight is null",vmNic.get("uuid")));
                    }
                }else{
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10167, "could not update backend server vmnic of serverGroup[uuid:%s],because vmnic uuid is null",msg.getServerGroupUuid()));
                }
            }
        }
            ...
            for(Map<String,String> server:servers){
                if(server.containsKey("ipAddress") && NetworkUtils.isIpv4Address(server.get("ipAddress"))){
                    ...
                    LoadBalancerServerGroupServerIpVO serverIpVO = Q.New(LoadBalancerServerGroupServerIpVO.class)
                            ...
                            .find();
                    if(serverIpVO == null){
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10168, "could not update backend server ip of serverGroup,because serverGroup[uuid:%s] don not have ip [ipAddress:%s] ",msg.getServerGroupUuid(),ipAddress));
                    }
                    ...
                    if(server.containsKey("weight")){
                        try{
                            Long serverIpWeight = Long.valueOf(server.get("weight"));
                            if (serverIpWeight < LoadBalancerConstants.BALANCER_WEIGHT_MIN || serverIpWeight > LoadBalancerConstants.BALANCER_WEIGHT_MAX) {
                                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10169, "invalid balancer weight[serverIp:%s,weight:%s], weight is not in the range [%d, %d]",
                                        server.get("ipAddress"), serverIpWeight, LoadBalancerConstants.BALANCER_WEIGHT_MIN, LoadBalancerConstants.BALANCER_WEIGHT_MAX));
                            }
                            ...
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10170, "could not add backend server ip to serverGroup[uuid:%s] ,because vmnic weight[%s] not a correct number",server.get("weight")));
                        }
                    }else{
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10171, "invalid balancer weight[serverIp:%s], weight is null",server.get("ipAddress")));
                    }

                }else{
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_LB_10172, "could not add backend server ip to serverGroup [uuid:%s], because ip [ipAddress:%s] is invalid",msg.getServerGroupUuid(),serverIps));
                }
            }

            if (lbVO.getType() == LoadBalancerType.Shared) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10173, "could not add server ip to share load balancer server group"));
            }
        }
        ...
        if( canChangeVmNic || canChangeServerIp){
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_LB_10174, "could not change backendserver, beacause vmincs and serverips is null"));
        }
    }
