	...
	public void createVirtualRouterBootstrapIso(final VirtualRouterBootstrapIsoInventory iso, final VmInstanceSpec vrSpec, final Completion complete) {
        ...
        bus.send(msg, new CloudBusCallBack(complete) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10038, "failed to create VirtualRouterBootstrapIso[%s] on kvm host[uuid:%s, ip:%s] for virtual router[uuid:%s], because %s",
	...
	public void deleteVirtualRouterBootstrapIso(final VirtualRouterBootstrapIsoInventory iso, final VmInstanceInventory vrSpec, final Completion complete) {
        ...
        bus.send(msg, new CloudBusCallBack(complete) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
					ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10039, "failed to delete VirtualRouterBootstrapIso[%s] on kvm host[uuid:%s] for virtual router[uuid:%s], because %s",
