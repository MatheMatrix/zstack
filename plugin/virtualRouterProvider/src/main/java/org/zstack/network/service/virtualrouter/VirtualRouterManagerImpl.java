...
public class VirtualRouterManagerImpl extends AbstractService implements VirtualRouterManager,
                ...
                for(IpRangeInventory priIprange : priIpranges){
                    for(IpRangeInventory pubIprange : pubIpranges){
                        ...
                        if(NetworkUtils.isIpv4RangeOverlap(priStartIp,priEndIp,pubStartIp,pubEndIp)){
                            throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10021, "cannot create virtual Router vm while virtual router network overlaps with private network in ip "));
                        }
    ...
    private void installSystemValidator() {
        class VirtualRouterOfferingValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            ...
            private void validateVirtualRouterOffering(String sysTag, String resourceUuid) {
                ...
                if (offeringVO == null) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10023, "No virtual router instance offering with uuid:%s is found", offeringUuid));
                }

                if (resourceUuid != null && resourceUuid.equals(offeringVO.getPublicNetworkUuid())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10024, "the network of virtual router instance offering with uuid:%s can't be same with private l3 network uuid:%s", offeringUuid, resourceUuid));
                }
            }
    ...
    private void acquireVirtualRouterVmInternal(VirtualRouterStruct struct,  final ReturnValueCompletion<VirtualRouterVmInventory> completion) {
        ...
        if (offerings == null) {
            ErrorCode err = err(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10025, VirtualRouterErrors.NO_DEFAULT_OFFERING, "unable to find a virtual router offering for l3Network[uuid:%s] in zone[uuid:%s], please at least create a default virtual router offering in that zone",
        ...
        thdf.chainSubmit(new ChainTask(completion) {
            ...
            public void run(final SyncTaskChain chain) {
                ...
                acquireVirtualRouterVmInternal(struct, new ReturnValueCompletion<VirtualRouterVmInventory>(chain, completion) {
                    ...
                    public void fail(ErrorCode errorCode) {
                        lock.unlock();
                        completion.fail(operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10026, "Failed to start vr l3[uuid: %s]", struct.getL3Network().getUuid()).causedBy(errorCode));
                        chain.next();
                    }
                });
            }
    ...
    void validateIpv6Range(String l3NetworkUuid) {
        if (Q.New(VirtualRouterOfferingVO.class).eq(VirtualRouterOfferingVO_.managementNetworkUuid, l3NetworkUuid).isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10027, "cannot add ip range, because l3 network[uuid:%s] is " +
        ...
        if (Q.New(VmNicVO.class).eq(VmNicVO_.l3NetworkUuid, l3NetworkUuid).in(VmNicVO_.metaData, VirtualRouterNicMetaData.MANAGEMENT_NIC_MASK_STRING_LIST).isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10028, "cannot add ip range, because l3 network[uuid:%s] is " +
    ...
    private void validate(APIAddImageMsg msg) {
        ...
        for (String tag : msg.getSystemTags()) {
            ...
            try {
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10029, "couldn't add image, because systemTag [%s] " +
    ...
    private void validate(APIAttachNetworkServiceToL3NetworkMsg msg) {
        ...
        if (!snat && eip) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10030, "failed tot attach virtual router network services to l3Network[uuid:%s]. When eip is selected, snat must be selected too", msg.getL3NetworkUuid()));
        }

        if (!snat && portForwarding) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10031, "failed tot attach virtual router network services to l3Network[uuid:%s]. When port forwarding is selected, snat must be selected too", msg.getL3NetworkUuid()));
        }
    }
    ...
    public void changeVirutalRouterDefaultL3Network(String vrUuid, String newL3Uuid, String oldL3Uuid, Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!ret.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10032, "update virtual router [uuid:%s] default network failed, because %s",
                            vrUuid, ret.getError());
                    completion.fail(err);
                } else {
    ...
    public String convertFilterNameToZQL(String filterName) {
        ...
        try {
            ...
            throw new OperationFailureException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10033, "invalid ApplianceVmType %s", ss[1]));
        }
    }
    ...
    public void afterMigrateVm(VmInstanceInventory inv, String srcHostUuid, NoErrorCompletion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!ret.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10034, "failed to sync vips[ips: %s] on virtual router[uuid:%s]" +
