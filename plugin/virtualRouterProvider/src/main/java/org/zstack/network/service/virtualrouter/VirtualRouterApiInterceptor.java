...
public class VirtualRouterApiInterceptor implements ApiMessageInterceptor, GlobalApiMessageInterceptor {
    ...
    private void validate(APIAddBackendServerToServerGroupMsg msg) {
        ...
        if (msg.getVmNics().isEmpty()) {
            ...
            if (!vrExist) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10000, "could not add server ip to load balancer server group, because share lb has no service provider, please add vmnic first"));
            }
        }
    }
    ...
    private void validate(APIUpdateVirtualRouterMsg msg) {
        ...
        if (msg.getDefaultRouteL3NetworkUuid().equals(vrVO.getDefaultRouteL3NetworkUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10001, "l3 uuid[:%s] is same to default network of virtual router [uuid:%s]",
        ...
        if (target == null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10002, "l3 uuid[:%s] is not attached to virtual router [uuid:%s]", msg.getDefaultRouteL3NetworkUuid(), msg.getVmInstanceUuid()));
        }
        ...
        if (!VirtualRouterNicMetaData.isPublicNic(target) && !VirtualRouterNicMetaData.isAddinitionalPublicNic(target)) {
            if (VirtualRouterNicMetaData.isManagementNic(target)) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10003, "could not set the default network, because l3 uuid[:%s] is management network", msg.getDefaultRouteL3NetworkUuid()));
            } else {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10004, "could not set the default network, because l3 uuid[:%s] is not public network", msg.getDefaultRouteL3NetworkUuid()));
            }
        }
    }
    ...
    private void validate(APIUpdateVirtualRouterOfferingMsg msg) {
        if (msg.getIsDefault() != null) {
            if (!new QuotaUtil().isAdminAccount(msg.getSession().getAccountUuid())) {
                throw new ApiMessageInterceptionException(err(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10005, IdentityErrors.PERMISSION_DENIED,
        ...
        if (msg.getImageUuid() != null) {
            ...
            if (type != ImageMediaType.RootVolumeTemplate) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10006, "image[uuid:%s]'s mediaType is %s, the mediaType of a virtual router image must be %s",
                                msg.getImageUuid(), type, ImageMediaType.RootVolumeTemplate));
            }

            if (ImageConstant.ISO_FORMAT_STRING.equals(format)) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10007, "image[uuid:%s] is of format %s, cannot be used for virtual router", msg.getImageUuid(), format));
            }
        }
    }
    ...
    private void validate(APICreateVirtualRouterOfferingMsg msg) {
        if (msg.isDefault() != null) {
            if (!new QuotaUtil().isAdminAccount(msg.getSession().getAccountUuid())) {
                throw new ApiMessageInterceptionException(err(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10008, IdentityErrors.PERMISSION_DENIED,
        ...
        if (!mgtL3.getZoneUuid().equals(msg.getZoneUuid()))  {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10009, "management network[uuid:%s] is not in the same zone[uuid:%s] this offering is going to create",
        ...
        if (mgtL3.getIpVersions().contains(IPv6Constants.IPv6) && !mgtL3.getIpVersions().contains(IPv6Constants.IPv4)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10010, "can not create virtual router offering, because management network doesn't support ipv6 yet"));
        }
        ...
        if (!zoneUuid.equals(msg.getZoneUuid()))  {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10011, "public network[uuid:%s] is not in the same zone[uuid:%s] this offering is going to create",
                            msg.getManagementNetworkUuid(), msg.getZoneUuid()));
        }
        ...
        ImageMediaType type = t.get(0, ImageMediaType.class);
        if (type != ImageMediaType.RootVolumeTemplate) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10012, "image[uuid:%s]'s mediaType is %s, the mediaType of a virtual router image must be %s",
                            msg.getImageUuid(), type, ImageMediaType.RootVolumeTemplate));
        }

        String format = t.get(1, String.class);
        if (ImageConstant.ISO_FORMAT_STRING.equals(format)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10013, "image[uuid:%s] is of format %s, cannot be used for virtual router", msg.getImageUuid(), format));
        }
        ...
        for (NetworkServiceL3NetworkRefVO nref : nrefs) {
            if (NetworkServiceType.SNAT.toString().equals(nref.getNetworkServiceType())) {
                if (nref.getL3NetworkUuid().equals(msg.getManagementNetworkUuid())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10014, "the L3 network[uuid: %s] has the SNAT service enabled, it cannot be used as a management network", msg.getManagementNetworkUuid()));
                } else if (nref.getL3NetworkUuid().equals(msg.getPublicNetworkUuid())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10015, "the L3 network[uuid: %s] has the SNAT service enabled, it cannot be used as a public network", msg.getPublicNetworkUuid()));
                }
            }
        }
     ...
     throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10016, "the L3 network[uuid: %s] is same network address with [uuid: %s], it cannot be used for virtual router", msg.getManagementNetworkUuid(),msg.getPublicNetworkUuid()));
            }
        }
    }
    ...
    private void checkIfManagementNetworkReachable(String managementNetworkUuid) {
        ...
        if (iprs.isEmpty()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10017, "the management network[uuid:%s] doesn't have any IP range", managementNetworkUuid));
        }
        ...
        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10018, "the management network[uuid:%s, gateway:%s] is not reachable", managementNetworkUuid, gateway));
    }
