...
public class VirtualRouterPortForwardingBackend extends AbstractVirtualRouterBackend implements
    ...
    public void applyPortForwardingRule(PortForwardingStruct struct, Completion completion) {
        ...
        if ((rule.getVipPortStart() != rule.getPrivatePortStart() || rule.getVipPortEnd() != rule.getPrivatePortEnd()) && (rule.getVipPortStart() != rule.getVipPortEnd()) && (rule.getPrivatePortStart() != rule.getPrivatePortEnd())) {
            throw new OperationFailureException(operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_PORTFORWARDING_10002, "virtual router doesn't support port forwarding range redirection, the vipPortStart must be equals to privatePortStart and vipPortEnd must be equals to privatePortEnd;" +
    ...
    public void afterAttachNic(VmNicInventory nic, Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!ret.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_PORTFORWARDING_10003, "failed to add portforwardings on virtual router[uuid:%s], %s",
    ...
    public void beforeDetachNic(VmNicInventory nic, Completion completion) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!ret.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_PORTFORWARDING_10004, "failed to revoke port forwardings on virtual router[uuid:%s], %s",
                            vrVO.getUuid(), ret.getError());
                    completion.fail(err);
                } else {
