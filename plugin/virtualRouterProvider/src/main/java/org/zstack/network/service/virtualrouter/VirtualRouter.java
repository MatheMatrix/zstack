...
public class VirtualRouter extends ApplianceVmBase {
    ...
    public class virtualRouterAfterAttachNicFlow extends NoRollbackFlow {
        @Override
        public void run(FlowTrigger trigger, Map data) {
            boolean applyToVirtualRouter = (boolean)data.get(VirtualRouterParam.APPLY_TO_VIRTUALROUTER.toString());
            if (!applyToVirtualRouter) {
                trigger.next();
                return;
            }

            VmNicInventory nicInventory = (VmNicInventory) data.get(VirtualRouterParam.VR_NIC.toString());
    ...
    private class virtualRouterApplyServicesAfterAttachNicFlow implements Flow {
        ...
        @Override
        public void run(FlowTrigger trigger, Map data) {
            VmNicInventory nicInv = (VmNicInventory) data.get(VirtualRouterParam.VR_NIC.toString());
            boolean applyToVirtualRouter = (boolean)data.get(VirtualRouterParam.APPLY_TO_VIRTUALROUTER.toString());
            if (!applyToVirtualRouter) {
                trigger.next();
                return;
    ...
    protected void afterAttachNic(VmNicInventory nicInventory, boolean applyToBackend, Completion completion) {
        ...
        data.put(VirtualRouterParam.VR_NIC.toString(), VmNicInventory.valueOf(vo));
        data.put(VirtualRouterParam.SNAT.toString(), Boolean.FALSE);
        data.put(VirtualRouterParam.VR.toString(), VirtualRouterVmInventory.valueOf(vrVo));
        data.put(VirtualRouterConstant.VirtualRouterParam.APPLY_TO_VIRTUALROUTER.toString(), applyToBackend);
    ...
    private static class virtualRouterReleaseVipbeforeDetachNic extends NoRollbackFlow {
        ...
        @Override
        public void run(FlowTrigger trigger, Map data) {
            VmNicInventory nic = (VmNicInventory) data.get(VirtualRouterParam.VR_NIC.toString());
    ...
    public class virtualRouterbeforeDetachNic extends NoRollbackFlow {
        ...
        @Override
        public void run(FlowTrigger trigger, Map data) {
            VmNicInventory nicInventory = (VmNicInventory) data.get(VirtualRouterConstant.VirtualRouterParam.VR_NIC.toString());
    ...
    protected void beforeDetachNic(VmNicInventory nicInventory, Completion completion) {
        ...
        data.put(VirtualRouterParam.VR_NIC.toString(), nicInventory);
        data.put(VirtualRouterConstant.VirtualRouterParam.VR.toString(), vr);
