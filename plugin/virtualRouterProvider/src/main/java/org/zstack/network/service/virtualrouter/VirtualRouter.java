            ...
            public void success(PingRsp ret) {
                ...
                if (!ret.isSuccess()) {
                    ...
                    if ((ret.getHealthy() != null) && (!ret.getHealthy()) && (ret.getHealthDetail() != null)) {
                        fireServiceUnhealthyCanonicalEvent(inerr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10014, "virtual router %s unhealthy, detail %s", getSelf().getUuid(), ret.getHealthDetail()));
                    } else {
        ...
        thdf.chainSubmit(new ChainTask(msg) {
            ...
            public void run(final SyncTaskChain chain) {
                ...
                if (msg.isCheckStatus() && getSelf().getState() != VmInstanceState.Running) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10015, "the virtual router[name:%s, uuid:%s, current state:%s] is not running," +
                ...
                if (msg.isCheckStatus() && getSelf().getStatus() != ApplianceVmStatus.Connected) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10016, "virtual router[uuid:%s] is in status of %s that cannot make http call to %s",
                            self.getUuid(), getSelf().getStatus(), msg.getPath()));
                }

                if (vr.getManagementNic() == null) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10017, "virtual router[uuid:%s] has no management nic that cannot make http call to %s",
            ...
            bus.send(cmsg, new CloudBusCallBack(trigger) {
                ...
                public void run(MessageReply reply) {
                    ...
                    if (rsp.isSuccess()) {
                        ...
                        ErrorCode err = operr(ORG_ZSTACK_NETWORK_SERVICE_VIRTUALROUTER_10018, "unable to add nic[ip:%s, ip6:%s, mac:%s] to virtual router vm[uuid:%s ip:%s], because %s",
