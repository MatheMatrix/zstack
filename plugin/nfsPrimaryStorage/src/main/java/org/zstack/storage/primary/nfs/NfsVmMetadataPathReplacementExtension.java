...
@Component
public class NfsVmMetadataPathReplacementExtension implements VmMetadataPathReplacementExtensionPoint {
    private static final CLogger logger = Utils.getLogger(NfsVmMetadataPathReplacementExtension.class);
    ...
    public PathReplacementResult calculatePathReplacements(String targetPsUuid, List<String> allOldPaths) {
        String baseDir = Q.New(PrimaryStorageVO.class)
                .select(PrimaryStorageVO_.mountPath)
                .eq(PrimaryStorageVO_.uuid, targetPsUuid)
                .findValue();
        if (baseDir == null) {
            baseDir = Q.New(PrimaryStorageVO.class)
