...
public class NfsPrimaryStorageFactory implements NfsPrimaryStorageManager, PrimaryStorageFactory, Component, CreateTemplateFromVolumeSnapshotExtensionPoint, RecalculatePrimaryStorageCapacityExtensionPoint,
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (!q(PrimaryStorageClusterRefVO.class).eq(PrimaryStorageClusterRefVO_.primaryStorageUuid, psUuid).isExists()) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10063, "the NFS primary storage[uuid:%s] is not attached" +
    ...
    public List<HostInventory> getConnectedHostForPing(PrimaryStorageInventory pri) {
        if (pri.getAttachedClusterUuids().isEmpty()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10064, "cannot find a Connected host to execute command for nfs primary storage[uuid:%s]", pri.getUuid()));
        }
        ...
        if (ret.isEmpty()) {
            throw new OperationFailureException(
                    operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10065, "cannot find a connected host in cluster which ps [uuid: %s] attached", pri.getUuid()));
        } else {
    ...
    public List<HostInventory> getConnectedHostForOperation(PrimaryStorageInventory pri) {
        if (pri.getAttachedClusterUuids().isEmpty()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10066, "cannot find a Connected host to execute command for nfs primary storage[uuid:%s]", pri.getUuid()));
        }
        ...
        if (ret.isEmpty()) {
            throw new OperationFailureException(
                    operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10067, "cannot find a host which has Connected host-NFS connection to execute command " +
    ...
    public HypervisorType findHypervisorTypeByImageFormatAndPrimaryStorageUuid(String imageFormat, final String psUuid) {
        ...
        throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10068, "cannot find proper hypervisorType for primary storage[uuid:%s] to handle image format or volume format[%s]", psUuid, imageFormat));
    }
