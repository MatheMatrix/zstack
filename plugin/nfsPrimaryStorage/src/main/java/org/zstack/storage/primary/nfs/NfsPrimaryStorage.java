...
public class NfsPrimaryStorage extends PrimaryStorageBase {
    ...
    private void handle(final DeleteImageCacheOnPrimaryStorageMsg msg) {
        ...
        if (bkd == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10034, "cannot find usable backend"));
        }
    ...
    private void handle(final GetVolumeRootImageUuidFromPrimaryStorageMsg msg) {
        NfsPrimaryStorageBackend bkd = getUsableBackend();
        if (bkd == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10035, "no usable backend found"));
        }
    ...
    protected void handle(final RevertVolumeFromSnapshotOnPrimaryStorageMsg msg) {
        ...
        try {
            ...
            reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10036, "no host in Connected status to which nfs primary storage[uuid:%s, name:%s] attached" +
    ...
    protected void handle(final ReInitRootVolumeFromTemplateOnPrimaryStorageMsg msg) {
        ...
        if (destHost == null) {
            reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10037, "no host in Connected status to which nfs primary storage[uuid:%s, name:%s] attached" +
    ...
    private void handle(final CheckSnapshotMsg msg) {
        ...
        if (vol.getVmInstanceUuid() != null) {
            ...
            if (state == VmInstanceState.Running || state == VmInstanceState.Paused) {
                ...
                reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10038, "vm[uuid:%s] is not Running, Paused or Stopped, current state is %s",
    ...
    private void handle(final TakeSnapshotMsg msg) {
        ...
        if (vol.getVmInstanceUuid() != null) {
            ...
            if (state == VmInstanceState.Running || state == VmInstanceState.Paused) {
                ...
                reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10039, "vm[uuid:%s] is not Running, Paused or Stopped, current state is %s",
                        vol.getVmInstanceUuid(), state));
                bus.reply(msg, reply);
                return;
            }
    ...
    private void handle(PrimaryStorageRemoveCachedImageMsg msg) {
        if (self.getAttachedClusterRefs().isEmpty()) {
            ...
            reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10040, "primary storage[uuid:%s] doesn't attach to any cluster", self.getUuid()));
            bus.reply(msg, reply);
            return;
        }
    ...
    public void detachHook(String clusterUuid, Completion completion) {
        ...
        try {
            ...
            completion.fail(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10041, e.getMessage()));
        }
    }
    ...
    private void createMemoryVolume(InstantiateMemoryVolumeOnPrimaryStorageMsg msg) {
        ...
        if (msg.getDestHost() != null) {
            ...
            if (backend == null) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10042, "the NFS primary storage[uuid:%s, name:%s] cannot find any usable host to" +
    ...
    private void createEmptyVolume(final InstantiateVolumeOnPrimaryStorageMsg msg) {
        ...
        if (msg.getDestHost() != null) {
            ...
            backend = getUsableBackend();
            if (backend == null) {
                throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10043, "the NFS primary storage[uuid:%s, name:%s] cannot find any usable host to" +
                                " create the data volume[uuid:%s, name:%s]", self.getUuid(), self.getName(),
                        msg.getVolume().getUuid(), msg.getVolume().getName()));
            }
        }
    ...
    protected void handle(final SyncVolumeSizeOnPrimaryStorageMsg msg) {
        ...
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10044, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    protected void handle(EstimateVolumeTemplateSizeOnPrimaryStorageMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10045, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    private void handle(NfsToNfsMigrateBitsMsg msg) {
        thdf.chainSubmit(new ChainTask(msg) {
            ...
            public void run(SyncTaskChain chain) {
                NfsPrimaryStorageBackend backend = getUsableBackend();
                if (backend == null) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10046, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                            self.getUuid(), self.getName()));
                }
    ...
    private void handle(DownloadBitsFromKVMHostToPrimaryStorageMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10047, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    private void handle(CancelDownloadBitsFromKVMHostToPrimaryStorageMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10048, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    private void handle(GetDownloadBitsFromKVMHostProgressMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10049, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    private void handle(NfsRebaseVolumeBackingFileMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10050, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    private void handle(GetVolumeBackingChainFromPrimaryStorageMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10051, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    protected void connectHook(ConnectParam param, final Completion completion) {
        ...
        if (backend == null) {
            ...
            completion.fail(err(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10052, PrimaryStorageErrors.DISCONNECTED,
    ...
    protected void pingHook(Completion completion) {
        ...
        if (bkd == null) {
            // the nfs primary storage has not been attached to any clusters, or no connected hosts
            completion.fail(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10053, "the NFS primary storage[uuid:%s, name:%s] has not attached to any clusters, or no hosts in the" +
                    " attached clusters are connected", self.getUuid(), self.getName()));
        } else {
    ...
    protected void handle(GetVolumeSnapshotEncryptedOnPrimaryStorageMsg msg) {
        NfsPrimaryStorageBackend backend = getUsableBackend();
        if (backend == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10054, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    public void handle(AskInstallPathForNewSnapshotMsg msg) {
        NfsPrimaryStorageBackend bkd = getUsableBackend();
        if (bkd == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10055, "the NFS primary storage[uuid:%s, name:%s] cannot find hosts in attached clusters to perform the operation",
                    self.getUuid(), self.getName()));
        }
    ...
    protected void handle(CheckVolumeSnapshotOperationOnPrimaryStorageMsg msg) {
        ...
        if (msg.getVmInstanceUuid() != null) {
            ...
            if (hostStatus == null && getAvailableHostUuidForOperation() == null) {
                reply.setError(err(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10056, HostErrors.HOST_IS_DISCONNECTED, "cannot find available host for operation on" +
                        " primary storage[uuid:%s].", self.getUuid()));
            } else if (hostStatus != HostStatus.Connected && hostStatus != null) {
                reply.setError(err(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10057, HostErrors.HOST_IS_DISCONNECTED, "host where vm[uuid:%s] locate is not Connected.", msg.getVmInstanceUuid()));
            }
        }
    ...
    private ErrorCode checkChangeVolumeType(String volumeUuid) {
        ...
        return operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10058, "volume[uuid:%s] has reference volume[%s], can not change volume type before flatten " +
    ...
    protected void handle(CommitVolumeSnapshotOnPrimaryStorageMsg msg) {
        ...
        if (hostUuid == null || hostUuid.isEmpty()) {
            reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10061, "no host found for volume[uuid:%s]", msg.getVolume().getUuid()));
            bus.reply(msg, reply);
            return;
        }
    ...
    protected void handle(PullVolumeSnapshotOnPrimaryStorageMsg msg) {
        ...
        String hostUuid = getHostUuidFromVolume(msg.getVolume().getUuid());
        if (hostUuid == null || hostUuid.isEmpty()) {
            reply.setError(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10062, "no host found for volume[uuid:%s]", msg.getVolume().getUuid()));
            bus.reply(msg, reply);
            return;
        }
