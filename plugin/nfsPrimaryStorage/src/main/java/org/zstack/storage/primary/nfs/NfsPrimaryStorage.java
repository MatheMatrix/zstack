...
public class NfsPrimaryStorage extends PrimaryStorageBase {
    ...
    protected void handle(ReadVmInstanceMetadataOnPrimaryStorageMsg msg) {
        ...
        if (hostUuid == null || hostUuid.isEmpty()) {
            reply.setError(operr("no host uuid provided for reading vm metadata on NFS primary storage[uuid:%s]", self.getUuid()));
            bus.reply(msg, reply);
            return;
        }
        ...
        backend.handle(msg, hostUuid, new ReturnValueCompletion<ReadVmInstanceMetadataOnPrimaryStorageReply>(msg) {
            @Override
            public void success(ReadVmInstanceMetadataOnPrimaryStorageReply r) {
                bus.reply(msg, r);
            }
