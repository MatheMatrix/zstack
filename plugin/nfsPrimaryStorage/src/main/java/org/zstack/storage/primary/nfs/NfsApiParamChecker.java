...
public class NfsApiParamChecker {
    ...
    public void checkUrl(String zoneUuid, List<String> systemTags, String url) {
        ...
        if (q.isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10069, "there has been a nfs primary storage having url as %s in zone[uuid:%s]", url, zoneUuid));
        }

        String[] results = url.split(":");
        if (results.length == 2 && (
                results[1].startsWith("/dev") || results[1].startsWith("/proc") || results[1].startsWith("/sys"))) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10070, " the url contains an invalid folder[/dev or /proc or /sys]"));
        }
        ...
        if (systemTags != null) {
            ...
            for (String sysTag: systemTags) {
                if (PrimaryStorageSystemTags.PRIMARY_STORAGE_GATEWAY.isMatch(sysTag)) {
                    if (found) {
                        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10071, "found multiple CIDR"));
                    }
    ...
    private void validateCidrTag(String sysTag, String ipAddr) {
        ...
        if (!NetworkUtils.isIpv4InCidr(ipAddr, cidr)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10073, "IP address[%s] is not in CIDR[%s]", ipAddr, cidr));
        }
    }
    ...
    public void checkRunningVmForUpdateUrl(String psuuid) {
        ...
        if (!ts.isEmpty()) {
            ...
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10074, "there are %s running VMs on the NFS primary storage, please" +
