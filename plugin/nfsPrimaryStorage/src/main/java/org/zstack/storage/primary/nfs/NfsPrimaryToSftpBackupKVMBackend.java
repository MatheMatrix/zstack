            ...
            public void run(MessageReply reply) {
                ...
                bus.send(msg, new CloudBusCallBack(completion) {
                    ...
                    public void run(MessageReply reply) {
                        ...
                        if (!rsp.isSuccess()) {
                            completion.fail(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10001, "failed to download[%s] from SftpBackupStorage[hostname:%s] to nfs primary storage[uuid:%s, path:%s], %s",
            ...
            private void upload(final String hostname, String sshKey, int sshPort, String username) {
                ...
                bus.send(msg, new CloudBusCallBack(completion) {
                    ...
                    public void run(MessageReply reply) {
                        ...
                        if (!rsp.isSuccess()) {
                            completion.fail(operr(ORG_ZSTACK_STORAGE_PRIMARY_NFS_10002, "failed to upload bits from nfs primary storage[uuid:%s, path:%s] to SFTP backup storage[hostname:%s, path: %s], %s",
