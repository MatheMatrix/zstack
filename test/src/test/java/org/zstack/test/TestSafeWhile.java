...
public class TestSafeWhile {
    public void testSafeWhile() {
        ...
        new While<>(Arrays.asList(1, 2, 3)).each((item, completion) -> {
            throw new OperationFailureException(operr(ORG_ZSTACK_TEST_10000, "on purpose %d", item));
        ...
        new While<>(Arrays.asList(1, 2, 3)).each((item, completion) -> {
            completion.addError(operr(ORG_ZSTACK_TEST_10001, "on purpose %d", item));
            completion.addError(operr(ORG_ZSTACK_TEST_10002, "I should not be in error list %d", item));
            throw new OperationFailureException(operr(ORG_ZSTACK_TEST_10003, "I should not be in error list either %d", item));
        ...
        new While<>(Arrays.asList(1, 2, 3)).each((item, completion) -> {
            completion.addError(operr(ORG_ZSTACK_TEST_10004, "on purpose %d", item));
            completion.done();
            ...
            public void done(ErrorCodeList errs) {
                assert errs.getCauses().size() == 3 : "errors:" + errs.getCauses().toString();
                assert errs.getCauses().stream().allMatch(it -> it.getDetails().startsWith("on purpose"));
                throw new OperationFailureException(operr(ORG_ZSTACK_TEST_10005, "done, on purpose"));
            }
        });
        ...
        new While<>(Arrays.asList(1, 2, 3)).each((item, completion) -> {
            new While<>(Arrays.asList(1, 2, 3)).each((innerItem, innerCompl) -> {
                ...
                public void done(ErrorCodeList errorCodeList) {
                    completion.addError(operr(ORG_ZSTACK_TEST_10006, "on purpose"));
                    completion.addError(operr(ORG_ZSTACK_TEST_10007, "I should not be errs list"));
                    throw new OperationFailureException(operr(ORG_ZSTACK_TEST_10008, "I should not be errs list either."));
                }
            });
