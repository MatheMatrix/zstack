...
class AnsibleRunnerTargetAwareArgumentsCase {
    ...
    private static final String IPV4_TARGET = "127.0.0.2"
    ...
    void test() {
        ...
        try {
            ...
            testInvalidTargetKeepsRestFacadeHost()
            testCallbackCheckerUsesSameSelectedHost()
            testNoSameFamilyReturnsErrorWithoutMessage()
        } finally {
    ...
    void testInvalidTargetKeepsRestFacadeHost() {
        withManagementServerIpProperties([
                "management.server.ip" : IPV4,
                "management.server.ip6": IPV6,
        ]) {
            String restHost = "mn.example.com"
            RunAnsibleMsg msg = runAnsible("not-an-ip!!", restHost,
                    "http://${restHost}:${REST_PORT}")

            assert msg.deployArguments.trustedHost == restHost
        }
    }

    void testNoSameFamilyReturnsErrorWithoutMessage() {
        withManagementServerIpProperties([
                "management.server.ip": IPV4,
        ]) {
            def selection = Platform.selectManagementServerIpForRemoteStrict(
                    IPV6_TARGET, IPV6_TARGET, null)
            def result = runAnsibleResult(IPV6_TARGET, IPV4,
                    "http://${IPV4}:${REST_PORT}", selection)

            assert result.message == null
            assert result.error != null
            assert result.error.globalErrorCode ==
                    "ORG_ZSTACK_CORE_MANAGEMENT_SERVER_IP_10000"
        }
    }

    void testCallbackCheckerUsesSameSelectedHost() {
        withManagementServerIpProperties([
                "management.server.ip" : IPV4,
                "management.server.ip6": IPV6,
        ]) {
            def result = runAnsibleResult(IPV6_TARGET, IPV4,
                    "http://${IPV4}:${REST_PORT}", null)

            assert result.error == null
            assert result.callbackChecker.callbackIp == IPV6
        }
    }

    private RunAnsibleMsg runAnsible(String targetIp, String restHost, String baseUrl) {
        def result = runAnsibleResult(targetIp, restHost, baseUrl, null)
        assert result.error == null: result.error
        assert result.message != null
        return result.message as RunAnsibleMsg
    }

    private Map runAnsibleResult(String targetIp, String restHost, String baseUrl, def selection) {
        RunAnsibleMsg[] captured = new RunAnsibleMsg[1]
        ErrorCode[] capturedError = new ErrorCode[1]
        ...
        try {
            ...
            callbackChecker.setCallbackIp("legacy-selection")
            ...
            ReturnValueCompletion<Boolean> completion = new ReturnValueCompletion<Boolean>(null) {
                ...
                void fail(ErrorCode errorCode) {
                    capturedError[0] = errorCode
                }
            }
            runner.run(completion)

            return [message: captured[0], error: capturedError[0],
                    callbackChecker: callbackChecker]
        } finally {
