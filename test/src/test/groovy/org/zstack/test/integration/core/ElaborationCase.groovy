...
class ElaborationCase extends SubCase {
    ...
    void testElaborationWithLongName() {
        def err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10036, "host[uuid:%s, name:%s] is in status[%s], cannot perform required operation", Platform.uuid, "long long long long long long long long long host name", "Connecting") as ErrorCode
    ...
    void testElaboration() {
        def err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10037, "certificate has expired or is not yet valid") as ErrorCode
        assert err.elaboration != null
        assert err.elaboration.trim() == "错误信息: 当前系统时间不在镜像仓库证书有效期内，可能因为镜像仓库服务器的系统时间被调整，或者证书被修改。"

        err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10038, "The state of vm[uuid:%s] is %s. Only these state[Running,Stopped] is allowed to update cpu or memory.", Platform.uuid, "Rebooting") as ErrorCode
        assert err.elaboration != null
        assert err.elaboration.trim() == "错误信息: 云主机的状态为 Rebooting，只有状态 [Running，Stopped] 允许升级 CPU/内存。"

        err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10039, "test for missed error") as ErrorCode
        assert err.elaboration == null

        err = Platform.err(ORG_ZSTACK_TEST_INTEGRATION_CORE_10040, IdentityErrors.INVALID_SESSION, "xxxxxxxxx") as ErrorCode
        assert err.elaboration != null
    }
    ...
    void testElaborationEnglish() {
        def err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10041, "certificate has expired or is not yet valid") as ErrorCode
        assert err.elaboration != null
        ...
        err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10042, "The state of vm[uuid:%s] is %s. Only these state[Running,Stopped] is allowed to update cpu or memory.", Platform.uuid, "Rebooting") as ErrorCode
        assert err.elaboration != null
        ...
        err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10043, "test for missed error") as ErrorCode
        assert err.elaboration == null

        err = Platform.err(ORG_ZSTACK_TEST_INTEGRATION_CORE_10044, IdentityErrors.INVALID_SESSION, "xxxxxxxxx") as ErrorCode
        assert err.elaboration != null
    }
    ...
    void testElaborationWithUnknownFormatConversion() {
        def err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10045, "%!s(int=0) %!s(bytes.readOp=0)", "nowadays") as ErrorCode
        assert err.elaboration == null
    ...
    void testErrorList() {
        ...
        def err1 = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10046, "host[uuid:%s, name:%s] is in state[%s], cannot perform required operation", Platform.uuid, "host-1", "Maintenance") as ErrorCode
        def err2 = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10047, "host[uuid:%s, name:%s] is in state[%s], cannot perform required operation", Platform.uuid, "host-2", "Maintenance") as ErrorCode

        list.addAll([err1, err2])
        def errlist = new ErrorCodeList().causedBy(list)

        def err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10048, errlist, "unable to commit backup storage because: %s", err1.details)
    ...
    void testNestedError() {
        ...
        def err = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10049, str, 3)
        assert err.elaboration != null

        def errEla = err.elaboration
        def errCn = err.messages.message_cn

        def err1 = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10051, err, "test %s err", String.format(str, 3))
        assert err1.elaboration == errEla
        assert err1.messages.message_cn == errCn

        def err2 = Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10052, "test %s err", String.format(str, 3))
