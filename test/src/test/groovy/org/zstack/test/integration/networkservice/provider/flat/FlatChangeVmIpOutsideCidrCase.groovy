...
class FlatChangeVmIpOutsideCidrCase extends SubCase {
    ...
    void test() {
        ...
        env.create {
            ...
            testResolve_C2_netmaskMismatch_defaultNic_error()
            testResolve_C3_netmaskMismatch_soleNic_error()
    ...
    /**
     * c-2: netmask != CIDR, NIC is default NIC (single-NIC VM on default L3).
     * Expected: error — default NIC must have gateway.
     */
    void testResolve_C2_netmaskMismatch_defaultNic_error() {
        L3NetworkInventory destL3 = env.inventoryByName("flatL3_range_noDhcp")
        L3NetworkInventory srcL3 = env.inventoryByName("flatL3_dest")

        // Single-NIC VM: srcL3 is both default and sole
        VmInstanceInventory vm = createVmOnL3("vm-resolve-c2", srcL3.uuid)
        VmNicInventory vmNic = vm.vmNics[0]
        ...
        String defaultL3 = Q.New(VmInstanceVO.class)
                ...
                .findValue()
        assert defaultL3 == srcL3.uuid : "srcL3 should be the default L3"
        ...
        expect(AssertionError.class) {
            changeVmNicNetwork {
                ...
                destL3NetworkUuid = destL3.uuid
                systemTags = [
                        String.format("staticIp::%s::192.168.100.70", destL3.uuid),
    ...
    /**
     * c-3: netmask != CIDR, NIC is the sole NIC (VM has only 1 NIC, on non-default L3).
     * Since a single-NIC VM's L3 is always the default, this effectively tests
     * the isOnlyNic branch (vmNicCount == 1).
     * Expected: error — sole NIC must have gateway.
     */
    void testResolve_C3_netmaskMismatch_soleNic_error() {
        L3NetworkInventory destL3 = env.inventoryByName("flatL3_range_noDhcp")
        L3NetworkInventory secondL3 = env.inventoryByName("flatL3_second")

        // Single-NIC VM on secondL3
        VmInstanceInventory vm = createVmOnL3("vm-resolve-c3", secondL3.uuid)
        ...
        expect(AssertionError.class) {
            changeVmNicNetwork {
                ...
                destL3NetworkUuid = destL3.uuid
                systemTags = [
                        String.format("staticIp::%s::192.168.100.80", destL3.uuid),
                        String.format("ipv4Netmask::%s::255.255.0.0", destL3.uuid)
                ]
            }
        }
