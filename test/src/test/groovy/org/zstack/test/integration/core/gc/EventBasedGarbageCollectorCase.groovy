...
class EventBasedGarbageCollectorCase extends SubCase {
    ...
    static class EventBasedGCInDb extends EventBasedGarbageCollector {
        ...
        protected void triggerNow(GCCompletion completion) {
            ...
            if (ret == EventBasedGCInDbBehavior.SUCCESS) {
                ...
                completion.fail(org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_GC_10003, "on purpose"))
    ...
    static class EventBasedGCInDbTriggerNow extends EventBasedGarbageCollector {
        ...
        protected void triggerNow(GCCompletion completion) {
            ...
            if (ret == EventBasedGCInDbBehavior.SUCCESS) {
                ...
                completion.fail(org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_GC_10005, "on purpose"))
            } else if (ret == EventBasedGCInDbBehavior.CANCEL) {
                completion.cancel()
            } else {
                assert false: "unknown behavior $ret"
    ...
    void testEventBasedGCFailure() {
        ...
        def gc = new EventBasedGC1()
        gc.testLogic = { GCCompletion completion ->
            completion.fail(org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_GC_10006, "testEventBasedGCFailure"))
            latch.countDown()
        }
    ...
    void testTwoEventsTriggeredGC() {
        ...
        gc.testLogic = { GCCompletion completion ->
            count ++
            if (count == 1) {
                completion.fail(org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_GC_10007, "testTwoEventsTriggeredGC"))
            } else {
                completion.success()
            }
            latch.countDown()
    ...
    void testEventBasedGCContext() {
        ...
        gc.testLogic = { GCCompletion completion ->
            if (count == 0) {
                ...
                completion.fail(org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_GC_10009, "mock failure"))
                count++
                return
            }
