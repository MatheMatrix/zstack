...
class ZbsVolumeEncryptionCase extends SubCase {
    ...
    void testEncryptedEmptyVolumeCreatorReusesImageKey() {
        ...
        withMaterialFactoryStubs(secretHelper, keyManager, keyBackend) {
            bean(ZbsVolumeEncryptionExtension.class).createEncryptedEmptyVolume(backend, spec,
                    new ReturnValueCompletion<VolumeStats>(null) {
                        @Override
                        void success(VolumeStats stats) {
                            outcome.stats = stats
                        }
        ...
        assert outcome.stats == null
        assert deletedPaths == [installPath]
        assert secretHelper.requests.isEmpty()
        assert secretHelper.sealRequests == [[hostUuid: host.uuid, resourceUuid: imageUuid, dekBase64: "image-dek"]]
        assert keyBackend.volumeRequests.isEmpty()
        assert keyBackend.imageRequests == [imageUuid]
        assert keyManager.requests.size() == 1
        assert keyManager.requests[0].resourceType == ImageVO.simpleName
        assert keyManager.requests[0].resourceUuid == imageUuid
        assert keyManager.requests[0].keyProviderUuid == keyProviderUuid
        assert keyManager.requests[0].purpose == "prepare-zbs-encryption-material"
    }
