...
class RegexElaborationCase extends SubCase {
    ...
    void testElaboration1() {
        def err = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10005, "Fn::Join must be array and contain 2 params, array[0] must be String, array[1] must be array!") as ErrorCode
    ...
    void testElaboration2() {
        def err = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10006, "Param [%s] has no value or default value found!", "ImageUuid") as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration3() {
        def err = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10007, "no host having cpu[%s], memory[%s bytes] found", 4, 8589934592) as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration4() {
        def err = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10008, "no Connected hosts found in the [%s] candidate hosts having the hypervisor type [%s]", 4, "KVM") as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration5() {
        def err = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10009, "no Connected hosts found in the [%s] candidate hosts", 2) as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration7() {
        ...
        def errCode1 = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10011, "operation error, because:%s", ".*can not find vg .* and create vg with forceWipw=.*") as ErrorCode
        def errCode2 = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10012, "operation error, because:%s", ".*can not find vg .* and create vg with forceWipw=.*") as ErrorCode

        causes.add(errCode1)
        causes.add(errCode2)
        errorCodes.setCauses(causes)

        ErrorCode result = org.zstack.core.Platform.err(ORG_ZSTACK_TEST_INTEGRATION_CORE_10013, PrimaryStorageErrors.ATTACH_ERROR, errorCodes, errorCodes.getDetails())
        ...
        errCode1 = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10014, "operation error, because:.*can not find vg .* and create vg with forceWipw=.*") as ErrorCode
        errCode2 = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10015, "operation error, because:.*can not find vg .* and create vg with forceWipw=.*") as ErrorCode
        ...
        result = org.zstack.core.Platform.err(ORG_ZSTACK_TEST_INTEGRATION_CORE_10016, PrimaryStorageErrors.ATTACH_ERROR, errorCodes, errorCodes.getDetails())
    ...
    void testElaboration8() {
        ErrorCode errorCode = org.zstack.core.Platform.operr(ORG_ZSTACK_TEST_INTEGRATION_CORE_10017, "operation error, because:%s", "failed to execute bash")
        ErrorCode result = org.zstack.core.Platform.err(ORG_ZSTACK_TEST_INTEGRATION_CORE_10018, VmErrors.START_ERROR, errorCode, errorCode.getDetails())
