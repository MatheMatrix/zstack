...
class RegexElaborationCase extends SubCase {
    ...
    void testElaboration1() {
        def err = Platform.operr("CASE_OF_ERROR_CODE",  "Fn::Join must be array and contain 2 params, array[0] must be String, array[1] must be array!") as ErrorCode
    ...
    void testElaboration2() {
        def err = Platform.operr("CASE_OF_ERROR_CODE",  "Param [%s] has no value or default value found!", "ImageUuid") as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration3() {
        def err = Platform.operr("CASE_OF_ERROR_CODE",  "no host having cpu[%s], memory[%s bytes] found", 4, 8589934592) as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration4() {
        def err = Platform.operr("CASE_OF_ERROR_CODE",  "no Connected hosts found in the [%s] candidate hosts having the hypervisor type [%s]", 4, "KVM") as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration5() {
        def err = Platform.operr("CASE_OF_ERROR_CODE",  "no Connected hosts found in the [%s] candidate hosts", 2) as ErrorCode
        assert err.messages != null
        assert err.messages.method == ElaborationSearchMethod.regex
    ...
    void testElaboration7() {
        ...
        def errCode1 = Platform.operr("CASE_OF_ERROR_CODE",  "operation error, because:%s", ".*can not find vg .* and create vg with forceWipw=.*") as ErrorCode
        def errCode2 = Platform.operr("CASE_OF_ERROR_CODE",  "operation error, because:%s", ".*can not find vg .* and create vg with forceWipw=.*") as ErrorCode

        causes.add(errCode1)
        causes.add(errCode2)
        errorCodes.setCauses(causes)

        ErrorCode result = err("CASE_OF_ERROR_CODE",  PrimaryStorageErrors.ATTACH_ERROR, errorCodes, errorCodes.getDetails())
        ...
        errCode1 = Platform.operr("CASE_OF_ERROR_CODE",  "operation error, because:.*can not find vg .* and create vg with forceWipw=.*") as ErrorCode
        errCode2 = Platform.operr("CASE_OF_ERROR_CODE",  "operation error, because:.*can not find vg .* and create vg with forceWipw=.*") as ErrorCode
        ...
        result = err("CASE_OF_ERROR_CODE",  PrimaryStorageErrors.ATTACH_ERROR, errorCodes, errorCodes.getDetails())
    ...
    void testElaboration8() {
        ErrorCode errorCode = Platform.operr("CASE_OF_ERROR_CODE",  "operation error, because:%s", "failed to execute bash")
        ErrorCode result = err("CASE_OF_ERROR_CODE",  VmErrors.START_ERROR, errorCode, errorCode.getDetails())
