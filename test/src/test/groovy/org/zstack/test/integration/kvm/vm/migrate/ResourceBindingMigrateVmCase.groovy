...
class ResourceBindingMigrateVmCase extends SubCase {
    ...
    /**
     * Check if error message contains resource binding related keywords.
     * Due to pagination mechanism, the actual error may be wrapped in cause chain or causes list.
     */
    boolean containsResourceBindingError(def error) {
        if (error == null) {
            return false
        }

        def keywords = ["designated host", "bound resource"]

        // Check details field
        if (error.details != null) {
            for (String keyword : keywords) {
                if (error.details.contains(keyword)) {
                    return true
                }
            }
        }

        // Check globalErrorCode field (pagination may put error info here)
        if (error.globalErrorCode != null) {
            for (String keyword : keywords) {
                if (error.globalErrorCode.contains(keyword)) {
                    return true
                }
            }
        }

        // Recursively check single cause
        if (error.cause != null) {
            if (containsResourceBindingError(error.cause)) {
                return true
            }
        }

        // Check causes list (ErrorCodeList has a causes field)
        if (error.causes != null) {
            for (def causeError : error.causes) {
                if (containsResourceBindingError(causeError)) {
                    return true
                }
            }
        }
    ...
    void resetVmToHost1Cluster1() {
        ...
        if (vmVO.hostUuid != host1.uuid) {
            ...
            // Temporarily allow cross-cluster on BOTH clusters to ensure migration works
            updateResourceConfig {
                name = "vm.ha.across.clusters"
                category = "vm"
    ...
    void enableAllHosts() {
        ...
        // Wait for all hosts to be fully connected
        retryInSecs {
            [env.inventoryByName("host1-cluster1") as HostInventory,
             env.inventoryByName("host2-cluster1") as HostInventory,
             env.inventoryByName("host1-cluster2") as HostInventory].each { host ->
                HostVO vo = dbFindByUuid(host.uuid, HostVO.class)
                assert vo.state == HostState.Enabled
                assert vo.status == HostStatus.Connected
            }
        }
    }
    ...
    void testCase4_HaAcrossFalse_Soft_CrossCluster_CoreFix() {
        ...
        // Debug output to see actual error structure
        logger.debug("Error structure: details=${result.error.details}, " +
                "globalErrorCode=${result.error.globalErrorCode}, " +
                "cause=${result.error.cause}, " +
                "causes=${result.error.causes}")
        if (result.error.cause != null) {
            logger.debug("Cause details: details=${result.error.cause.details}, " +
                    "globalErrorCode=${result.error.cause.globalErrorCode}")
        }

        // Check error message in details or cause chain (pagination may wrap the original error)
        assert containsResourceBindingError(result.error) :
                "Expected error about designated host or bound resource, but got: ${result.error.details}"
    }
