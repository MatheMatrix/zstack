 ...
 * PodVO sum).
 *
 * <p>What this protects against: a future refactor that adds an O(roles²) iteration, a
 ...
 * because (a) it pins absolute orchestration cost so a future regression with a 100x slowdown
...
public class PhysicalServerCapacityUpdaterOrchestrationOverheadTest {
    ...
    public void bench_1000_hosts_sequential_recalculate() throws Exception {
        ...
        // available = total - consumed - max(floor, total*pct/100)
        // cpu: 64 - 16 - max(4, 64*5/100=3) = 64 - 16 - 4 = 44
        // mem: 256GiB - 64GiB - 0(reserved) - max(4GiB, 256GiB*10/100=25.6GiB) = 256-64-25.6 = 166.4GiB
        long expectedAvailableCpu = 44L;
        long expectedAvailableMemory = 166L * 1024L * 1024L * 1024L
                + (long) (0.4 * 1024 * 1024 * 1024); // 25.6 GiB ≈ 27487790694 bytes
        // Compute the precise expected memory the same way the production code does to avoid
        // floating-point drift in the assertion.
        long memBuffer = Math.max(4L * 1024 * 1024 * 1024, TOTAL_MEMORY * 10 / 100);
        expectedAvailableMemory = TOTAL_MEMORY - PER_ROLE_USED_MEMORY - 0 - memBuffer;
