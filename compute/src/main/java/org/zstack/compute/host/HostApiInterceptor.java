...
public class HostApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APIDeleteHostNetworkServiceTypeMsg msg) {
        if (Q.New(HostNetworkLabelVO.class).eq(HostNetworkLabelVO_.uuid, msg.getUuid())
            ...
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_HOST_10103, "system host network service type[%s] cannot be deleted", msg.getUuid()));
        }
    }
    ...
    private void validate(APIUpdateHostNetworkServiceTypeMsg msg) {
        if (Q.New(HostNetworkLabelVO.class).eq(HostNetworkLabelVO_.uuid, msg.getUuid())
                .eq(HostNetworkLabelVO_.system, Boolean.TRUE).isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_HOST_10104, "system host network service type[%s] cannot be updated", msg.getUuid()));
        }
    }
    ...
    private void validate(APICreateHostNetworkServiceTypeMsg msg) {
        if (Q.New(HostNetworkLabelVO.class).eq(HostNetworkLabelVO_.serviceType, msg.getServiceType()).isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_HOST_10105, "there has been a host network service type[%s]", msg.getServiceType()));
        }
    }
    ...
    private void validate(APIGetHostWebSshUrlMsg msg) {
        ...
        if (!ret.isReturnCode(0)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_HOST_10106, "webssh server is not running."));
        }
    }
    ...
    private void validate(APIUpdateHostMsg msg) {
        if (msg.getManagementIp() != null) {
            ...
            if (q.isExists()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_HOST_10107, "there has been a host having managementIp[%s]", msg.getManagementIp()));
            }
        }
    }
    ...
    private void validate(APIAddHostMsg msg) {
        if (!NetworkUtils.isIpv4Address(msg.getManagementIp()) && !NetworkUtils.isHostname(msg.getManagementIp())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_HOST_10108, "managementIp[%s] is neither an IPv4 address nor a valid hostname", msg.getManagementIp()));
        }
    }
    ...
    private void validate(APIChangeHostStateMsg msg){
        ...
        if (hostStatus != HostStatus.Connected && msg.getStateEvent().equals(HostStateEvent.maintain.toString())){
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_HOST_10109, "can not maintain host[uuid:%s, status:%s]which is not Connected", msg.getHostUuid(), hostStatus));
        }
    }
}
