...
public abstract class HostBase extends AbstractHost {
    ...
    protected void checkStatus() {
        if (HostStatus.Connected != self.getStatus()) {
            ErrorCode cause = err(ORG_ZSTACK_COMPUTE_HOST_10111, HostErrors.HOST_IS_DISCONNECTED, "host[uuid:%s, name:%s] is in status[%s], cannot perform required operation", self.getUuid(), self.getName(), self.getStatus());
            throw new OperationFailureException(err(ORG_ZSTACK_COMPUTE_HOST_10112, HostErrors.OPERATION_FAILURE_GC_ELIGIBLE, cause, "unable to do the operation because the host is in status of Disconnected"));
        }
    }

    protected void checkState() {
        if (HostState.PreMaintenance == self.getState() || HostState.Maintenance == self.getState()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_HOST_10113, "host[uuid:%s, name:%s] is in state[%s], cannot perform required operation", self.getUuid(), self.getName(), self.getState()));
        }
    }
    ...
    private void handle(APIUpdateHostIpmiMsg msg) {
        ...
        if (null == ipmi.getIpmiAddress()) {
            event.setSuccess(false);
            event.setError(operr(ORG_ZSTACK_COMPUTE_HOST_10114, "host[%s] does not have ipmi device or ipmi does not have address." +
                            ...
                            public void done(ErrorCodeList errorCodeList) {
                                if (!vmFailedToMigrate.isEmpty()) {
                                    if (HostMaintenancePolicyManager.HostMaintenancePolicy.JustMigrate.equals(hostMaintenancePolicyMgr.getHostMaintenancePolicy(self.getUuid()))) {
                                        trigger.fail(operr(ORG_ZSTACK_COMPUTE_HOST_10115, "failed to migrate vm[uuids:%s] on host[uuid:%s, name:%s, ip:%s], will try stopping it.",
    ...
    private void handle(final APIReconnectHostMsg msg) {
        ...
        bus.send(rmsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!reply.isSuccess()) {
                    evt.setError(err(ORG_ZSTACK_COMPUTE_HOST_10116, HostErrors.UNABLE_TO_RECONNECT_HOST, reply.getError(), reply.getError().getDetails()));
    ...
    private void deleteHostByApiMessage(APIDeleteHostMsg msg) {
        ...
        chain.done(new FlowDoneHandler(msg) {
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                evt.setError(err(ORG_ZSTACK_COMPUTE_HOST_10117, SysErrors.DELETE_RESOURCE_ERROR, errCode, errCode.getDetails()));
                bus.publish(evt);
            }
        }).start();
    }
    ...
    protected void handle(final APIChangeHostStateMsg msg) {
        ...
        try {
            ...
            evt.setError(err(ORG_ZSTACK_COMPUTE_HOST_10118, SysErrors.CHANGE_RESOURCE_STATE_ERROR, he.getMessage()));
            bus.publish(evt);
            return;
        }
    ...
    private void doPingHost(final PingHostMsg msg, ReturnValueCompletion<PingHostReply> completion) {
        ...
        if (self.getStatus() == HostStatus.Connecting) {
            completion.fail(operr(ORG_ZSTACK_COMPUTE_HOST_10119, "host is connecting, ping failed"));
            return;
        }
                ...
                bus.send(connectMsg, new CloudBusCallBack(msg, chain, completion) {
                    ...
                    public void run(MessageReply reply) {
                        ...
                        if (reply.isSuccess()) {
                            ...
                            r.setError(err(ORG_ZSTACK_COMPUTE_HOST_10120, HostErrors.UNABLE_TO_RECONNECT_HOST, reply.getError(), reply.getError().getDetails()));
        ...
        thdf.chainSubmit(new ChainTask(msg) {
            ...
            private void reestablishConnection() {
                try {
                    ...
                    throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_HOST_10121, e.getMessage()));
                }
            }
        ...
        if (msg.isJustChangeState()) {
            ...
            maintenanceHook(msg, new Completion(msg) {
                ...
                public void fail(ErrorCode errorCode) {
                    ...
                    completion.fail(err(ORG_ZSTACK_COMPUTE_HOST_10122, HostErrors.UNABLE_TO_ENTER_MAINTENANCE_MODE, errorCode, errorCode.getDetails()));
                }
            });
        } else {
