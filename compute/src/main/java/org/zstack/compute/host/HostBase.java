...
import org.zstack.header.host.HostMaintenancePolicyExtensionPoint.ExtensionHostMaintenancePolicy;
...
public abstract class HostBase extends AbstractHost {
    ...
    protected void maintenanceHook(ChangeHostStateMsg changeHostStateMsg, final Completion completion) {
        ...
        Map<ExtensionHostMaintenancePolicy, Set<String>> policyVmMap = map(
                e(ExtensionHostMaintenancePolicy.MigrateVm, new HashSet<>(operateVmUuids)),
                e(ExtensionHostMaintenancePolicy.StopVm, new HashSet<>()));
        for (HostMaintenancePolicyExtensionPoint ext : pluginRgty.getExtensionList(HostMaintenancePolicyExtensionPoint.class)) {
            Map<String, ExtensionHostMaintenancePolicy> vmUuidPolicyMap = ext.getHostMaintenanceVmOperationPolicy(getSelfInventory());
        ...
        policyVmMap.get(ExtensionHostMaintenancePolicy.MigrateVm).removeAll(policyVmMap.get(ExtensionHostMaintenancePolicy.StopVm));
            ...
            public void setup() {
                flow(new NoRollbackFlow() {
                    ...
                    public void run(final FlowTrigger trigger, Map data) {
                        List<String> vmUuids = new ArrayList<>(policyVmMap.get(ExtensionHostMaintenancePolicy.MigrateVm));
                        if (vmUuids.isEmpty()) {
                            trigger.next();
                            return;
                        }
                            ...
                            public void done(ErrorCodeList errorCodeList) {
                                if (!vmFailedToMigrate.isEmpty()) {
                                    ...
                                    logger.warn(String.format("failed to migrate vm[uuids:%s] on host[uuid:%s, name:%s, ip:%s], will try stopping it.",
                                            vmFailedToMigrate.keySet(), self.getUuid(), self.getName(), self.getManagementIp()));
                                    policyVmMap.get(ExtensionHostMaintenancePolicy.StopVm).addAll(vmFailedToMigrate.keySet());
                                    policyVmMap.get(ExtensionHostMaintenancePolicy.MigrateVm).removeAll(vmFailedToMigrate.keySet());
                                }
                                trigger.next();
                            }
                        });
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        if (policyVmMap.get(ExtensionHostMaintenancePolicy.StopVm).isEmpty()) {
                            trigger.next();
                            return;
                        }

                        List<String> vmUuids = Q.New(VmInstanceVO.class).select(VmInstanceVO_.uuid)
                                .in(VmInstanceVO_.uuid, policyVmMap.get(ExtensionHostMaintenancePolicy.StopVm)).listValues();
