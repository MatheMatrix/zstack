...
public class HostManagerImpl extends AbstractService implements HostManager, ManagementNodeChangeListener,
    ...
    private void handleApiMessage(APIMessage msg) {
        ...
        } else if (msg instanceof APIUnzipFileMsg) {
             handle((APIUnzipFileMsg) msg);
    ...
    private void handle(final APIUnzipFileMsg msg) {
        APIUnzipFileEvent event = new APIUnzipFileEvent(msg.getId());
        if (CoreGlobalProperty.UNIT_TEST_ON) {
            bus.publish(event);
            return;
        }

        UnzipFileMsg umsg = new UnzipFileMsg();
        umsg.setHostUuid(msg.getUuid());
        umsg.setFilePath(msg.getFilePath());
        umsg.setUnzipFilePath(msg.getUnzipFilePath());
