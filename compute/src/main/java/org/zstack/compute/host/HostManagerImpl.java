...
public class HostManagerImpl extends AbstractService implements HostManager, ManagementNodeChangeListener,
    ...
    private void handleApiMessage(APIMessage msg) {
        ...
        }  else if (msg instanceof APIUploadFileToHostMsg) {
             handle((APIUploadFileToHostMsg) msg);
    ...
    private void handle(final APIUploadFileToHostMsg msg) {
        APIUploadFileToHostEvent event = new APIUploadFileToHostEvent(msg.getId());
        if (CoreGlobalProperty.UNIT_TEST_ON) {
            bus.publish(event);
            return;
        }
        ...
        bus.send(umsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply r) {
                if (!r.isSuccess()) {
                    ...
                    UploadFileToHostReply reply = (UploadFileToHostReply) r;
