...
public class HostManagerImpl extends AbstractService implements HostManager, ManagementNodeChangeListener,
    ...
    private void passThrough(HostMessage msg) {
        ...
        if (vo == null) {
            ErrorCode err = Platform.err(ORG_ZSTACK_COMPUTE_HOST_10011, SysErrors.RESOURCE_NOT_FOUND, "cannot find host[uuid:%s], it may have been deleted", msg.getHostUuid());
    ...
    private void doAddHost(final AddHostMessage msg, ReturnValueCompletion<HostInventory > completion) {
        if (Q.New(HostVO.class).eq(HostVO_.managementIp, msg.getManagementIp()).isExists()) {
            completion.fail(argerr(ORG_ZSTACK_COMPUTE_HOST_10012, "there has been a host having managementIp[%s]", msg.getManagementIp()));
            return;
        }

        final ClusterVO cluster = findClusterByUuid(msg.getClusterUuid());
        if (cluster == null) {
            completion.fail(argerr(ORG_ZSTACK_COMPUTE_HOST_10013, "cluster[uuid:%s] is not existing", msg.getClusterUuid()));
            return;
        }
        ...
        chain.then(new NoRollbackFlow() {
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                if (arch == null) {
                    trigger.fail(operr(ORG_ZSTACK_COMPUTE_HOST_10014, "after connecting, host[name:%s, ip:%s] returns a null architecture", vo.getName(), vo.getManagementIp()));
                    return;
                }
                ...
                if (!arch.equals(cluster.getArchitecture())) {
                    trigger.fail(operr(ORG_ZSTACK_COMPUTE_HOST_10015, "cluster[uuid:%s]'s architecture is %s, not match the host[name:%s, ip:%s] architecture %s",
            ...
            protected void run(Map tokens, Object data) {
                ...
                if (d.getNewStatus() == PrimaryStorageHostStatus.Disconnected &&
                    ...
                    new HostBase.HostDisconnectedCanonicalEvent(d.getHostUuid(),
                            operr(ORG_ZSTACK_COMPUTE_HOST_10016, "primary storage[uuid:%s] becomes disconnected, the host has no connected primary storage attached",
