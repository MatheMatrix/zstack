    ...
    private void handle(final APIMountBlockDeviceMsg msg) {
        ...
        try {
            ...
            if (!blkidResult.getStdout().isEmpty() && !msg.isForce()) {
                throw new OperationFailureException(operr("device %s already contains a %s filesystem. to overwrite it, set the 'force' parameter to true. warning: this will destroy all existing data on the device!",
                        msg.getPath()));
            }

            // Format the device
            String mkfsCmd = buildMkfsCommd(msg.getFilesystemType(), msg.getPath());
            long timeout = (msg.getMessageDeadline() - new Date().getTime()) / 1000 - 30;
            executeSshCommand(ssh, String.format("timeout %d %s", timeout, mkfsCmd));
            ...
            // Check if device is already mounted (using UUID)
            SshResult uuidMountedCheck = ssh.command(String.format("findmnt -n -o TARGET -S UUID=%s", uuid)).run();
            ssh.reset();
            if (!uuidMountedCheck.getStdout().isEmpty()) {
                throw new OperationFailureException(operr("device UUID %s is already mounted at %s",
                        uuid, uuidMountedCheck.getStdout().split("\\s+")[0]));
            }
            ...
            ssh.reset();
            if (!mountPointCheck.getStdout().isEmpty()) {
                throw new OperationFailureException(operr("mount point %s is already in use by device %s",
            ...
            // Check if UUID entry already exists in /etc/fstab
            String fstabEntry = String.format("UUID=%s %s %s defaults 0 0", uuid, msg.getMountPoint(), msg.getFilesystemType());
            SshResult fstabCheck = ssh.command(String.format("grep -q -F '%s' /etc/fstab", fstabEntry)).run();
            ssh.reset();
            if (!fstabCheck.getStdout().isEmpty()) {
                bus.publish(event);
                return;
            }
