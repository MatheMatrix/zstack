...
public class UploadFileTracker {
    ...
    public void runTrackTask(String hostUuid, String installPath) {
        runTrackTask(apiId, hostUuid, installPath);
    }
        ...
        thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask() {
            ...
            public boolean run() {
                ...
                if (reply.getDownloadSize() == 0 && overMaxIdleTime(createdTime)) {
                    markFailure(operr("upload file session expired"));
                    return true;
                }
                ...
                if (downloadingImageSuspendedTooLong && reply.isSupportSuspend()) {
                    markFailure(err(HostErrors.UPLOAD_FILE_INTERRUPTED, reply.getError(), "uploading has been inactive more than %d sec", maxIdleSecond));
                    return true;
                }
    ...
    private GetFileDownloadProgressReply getImageDownloadProgress(String apiId, String hostUuid) {
