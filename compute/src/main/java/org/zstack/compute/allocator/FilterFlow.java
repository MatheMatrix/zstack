    ...
    public void allocate() {
        ...
        for (HostAllocatorFilterExtensionPoint filter : pluginRgty.getExtensionList(HostAllocatorFilterExtensionPoint.class)) {
            ...
            if (candidates.isEmpty()) {
                fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10030, "after filtering, HostAllocatorFilterExtensionPoint[%s] returns zero candidate host, it means: %s", filter.getClass().getSimpleName(), filter.filterErrorReason()));
            }
        }
