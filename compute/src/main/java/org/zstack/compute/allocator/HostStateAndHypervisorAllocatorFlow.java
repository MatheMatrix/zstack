    ...
    public void allocate() {
        ...
        if (ret.isEmpty()) {
            if (CollectionUtils.isEmpty(candidates)) {
                fail(operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10000, "no host having state=Enabled status=Connected hypervisorType=%s found",
            ...
            if (isNoConnectedHost()) {
                error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10001, "no Connected hosts found in the [%s] candidate hosts", candidates.size());
            } else if (isNoEnabledHost()) {
                error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10002, "no Enabled hosts found in the [%s] candidate hosts", candidates.size());
                ...
                if (spec.getHypervisorType() != null) {
                    error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10004, "no host having state=Enabled status=Connected hypervisorType=%s found", spec.getHypervisorType());
                } else {
                    error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10005, "no host having state=Enabled status=Connected found");
                }
            }
