...
public class HostAllocatorChain implements HostAllocatorTrigger, HostAllocatorStrategy {
    ...
    private void done() {
        ...
        if (result.isEmpty()) {
            if (isDryRun) {
                dryRunCompletion.fail(err(ORG_ZSTACK_COMPUTE_ALLOCATOR_10011, HostAllocatorError.NO_AVAILABLE_HOST,
                        "host allocation flow doesn't indicate any details"));
            } else {
                completion.fail(err(ORG_ZSTACK_COMPUTE_ALLOCATOR_10012, HostAllocatorError.NO_AVAILABLE_HOST,
                        "host allocation flow doesn't indicate any details"));
            }
            return;
        }
    ...
    private void runFlow(AbstractHostAllocatorFlow flow) {
        try {
            ...
            completion.fail(inerr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10013, t.toString()));
        }
    }
    ...
    public void fail(ErrorCode errorCode) {
        ...
        if (seriesErrorWhenPagination.isEmpty()) {
            ...
            this.errorCode = err(ORG_ZSTACK_COMPUTE_ALLOCATOR_10014, HostAllocatorError.NO_AVAILABLE_HOST, seriesErrorWhenPagination.iterator().next(), "unable to allocate hosts; due to pagination is enabled, " +
