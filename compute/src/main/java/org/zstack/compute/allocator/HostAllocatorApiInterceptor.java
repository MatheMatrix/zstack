...
public class HostAllocatorApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APIGetCandidateBackupStorageForCreatingImageMsg msg) {
        if (msg.getVolumeSnapshotUuid() == null && msg.getVolumeUuid() == null) {
            ...
            ORG_ZSTACK_COMPUTE_ALLOCATOR_10018,         "either volumeUuid or volumeSnapshotUuid must be set"
            ));
        }
    }
    ...
    private void validate(APIGetCpuMemoryCapacityMsg msg) {
        ...
        if (!pass && !msg.isAll()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10019, "zoneUuids, clusterUuids, hostUuids must at least have one be none-empty list, or all is set to true"));
        }
