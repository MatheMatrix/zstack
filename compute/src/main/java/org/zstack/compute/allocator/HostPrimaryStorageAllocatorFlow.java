    ...
    public void allocate() {
        ...
        if (candidates.isEmpty()) {
            if (spec.getVmOperation().equals(VmOperation.NewCreate.toString())) {
                fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10024, "cannot find available primary storage[state: %s, status: %s, available capacity %s bytes]." +
                ...
                fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10025, "cannot find available primary storage[state: %s or %s, status: %s]." +
                        " Check the state/status of primary storage and make sure they have been attached to clusters",
