...
public class ResourceBindingAllocatorFlow extends AbstractHostAllocatorFlow {
    ...
    public void allocate() {
        ...
        String currentHostUuid = spec.getVmInstance().getHostUuid();

        Boolean allowAcrossClusters = rcf.getResourceConfigValue(VmGlobalConfig.VM_HA_ACROSS_CLUSTERS, vmUuid, Boolean.class);

        // Skip binding check if current cluster is null (new VM or start VM..)
        if (StringUtils.isBlank(currentClusterUuid) || StringUtils.isBlank(currentHostUuid)) {
            next(candidates);
            return;
        }
        ...
        List<HostVO> hostsInCurrentCluster = candidates.stream()
                ...
                .filter(h -> !h.getUuid().equals(currentHostUuid))
