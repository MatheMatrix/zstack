        ...
        if (!instanceOfferingExtensions.isEmpty()) {
            ...
            if (!tvos.isEmpty()) {
                ...
                for (InstanceOfferingTagAllocatorExtensionPoint extp : instanceOfferingExtensions) {
                    ...
                    if (tmp.isEmpty()) {
                        fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10019, "InstanceOfferingTagAllocatorExtensionPoint[%s] return zero candidate host", extp.getClass().getName()));
                        return;
                    } else {
        ...
        if (!diskOfferingExtensions.isEmpty() && spec.getDiskOfferings() != null && !spec.getDiskOfferings().isEmpty()) {
            ...
            if (!tvos.isEmpty()) {
                ...
                for (DiskOfferingTagAllocatorExtensionPoint extp : diskOfferingExtensions) {
                    ...
                    if (tmp.isEmpty()) {
                        fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10020, "DiskOfferingTagAllocatorExtensionPoint[%s] return zero candidate host", extp.getClass().getName()));
                        return;
                    } else {
