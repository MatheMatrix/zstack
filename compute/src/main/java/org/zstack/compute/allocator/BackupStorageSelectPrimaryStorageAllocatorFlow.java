    ...
    public void allocate() {
        ...
        if (psUuids == null) {
            ...
            if (possiblePrimaryStorageTypes == null) {
                ...
                ORG_ZSTACK_COMPUTE_ALLOCATOR_10000,         "the image[uuid:%s] is on the backup storage[uuid:%s, type:%s] that doesn't have metrics defined" +
            ...
            if (result.isEmpty()) {
                ...
                ORG_ZSTACK_COMPUTE_ALLOCATOR_10001,         "The image[uuid:%s, name:%s] is on the backup storage[uuid:%s, type:%s] that requires to work with primary storage[types:%s]," +
            ...
            if (result.isEmpty()) {
                throw new OperationFailureException(operr(
                ORG_ZSTACK_COMPUTE_ALLOCATOR_10002,         "The image[uuid:%s] is on the backup storage[uuid:%s, type:%s] that requires to work with primary storage[uuids:%s]," +
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10003, "the backup storage[uuid:%s, type:%s] requires bound" +
