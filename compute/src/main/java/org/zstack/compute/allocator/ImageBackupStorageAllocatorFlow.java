...
public class ImageBackupStorageAllocatorFlow extends AbstractHostAllocatorFlow {
    ...
    public void allocate() {
        ...
        if (bsUuids.isEmpty()) {
            ...
            ORG_ZSTACK_COMPUTE_ALLOCATOR_10010,         "the image[uuid:%s, name:%s] is deleted on all backup storage", spec.getImage().getUuid(), spec.getImage().getName()
            ));
        }
        ...
        if (candidates.isEmpty()) {
            fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10012, "no host found in zones[uuids:%s] that attaches to backup storage where image[%s] is on", zoneUuids, spec.getImage().getUuid()));
        } else {
            next(candidates);
        }
    }
