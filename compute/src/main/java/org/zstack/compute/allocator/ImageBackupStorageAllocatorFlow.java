...
public class ImageBackupStorageAllocatorFlow extends AbstractHostAllocatorFlow {
    ...
    public void allocate() {
        ...
        if (bsUuids.isEmpty()) {
            ...
            ORG_ZSTACK_COMPUTE_ALLOCATOR_10032,         "the image[uuid:%s, name:%s] is deleted on all backup storage", spec.getImage().getUuid(), spec.getImage().getName()
            ));
        }
        ...
        if (bsUuids.isEmpty()) {
            ...
            throw new OperationFailureException(err(ORG_ZSTACK_COMPUTE_ALLOCATOR_10033, HostAllocatorError.NO_AVAILABLE_HOST,
        ...
        if (candidates.isEmpty()) {
            fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10034, "no host found in zones[uuids:%s] that attaches to backup storage where image[%s] is on", zoneUuids, spec.getImage().getUuid()));
        } else {
            next(candidates);
        }
    }
