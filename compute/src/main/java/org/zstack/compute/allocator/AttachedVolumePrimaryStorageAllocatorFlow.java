...
public class AttachedVolumePrimaryStorageAllocatorFlow extends AbstractHostAllocatorFlow {
    ...
    public void allocate() {
        ...
        if (vm.getRootVolume() == null || !VolumeStatus.Ready.toString().equals(vm.getRootVolume().getStatus())) {
            fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10016, "cannot find root volume of vm[uuid:%s]", vm.getUuid()));
        }
        ...
        if (candidates.isEmpty()) {
            fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10017, "no host found in clusters which have attached to all primary storage %s where vm[uuid:%s]'s volumes locate",
