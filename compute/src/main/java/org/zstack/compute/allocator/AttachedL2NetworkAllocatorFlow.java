...
public class AttachedL2NetworkAllocatorFlow extends AbstractHostAllocatorFlow {
    ...
    public void allocate() {
        ...
        if (candidates.isEmpty()) {
            fail(Platform.operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10021, "no host found in clusters that has attached to L2Networks which have L3Networks%s", spec.getL3NetworkUuids()));
        } else {
            next(candidates);
        }
    }
