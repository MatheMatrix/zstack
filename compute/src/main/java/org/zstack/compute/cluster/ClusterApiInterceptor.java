...
public class ClusterApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APICreateClusterMsg msg) {
        if ((msg.getType() != null && msg.getType().equals("baremetal") && !msg.getHypervisorType().equals("baremetal")) ||
            ...
            ORG_ZSTACK_COMPUTE_CLUSTER_10003,         "if cluster type is baremetal, then hypervisorType must be baremetal too, or vice versa"
            ));
        }
    }
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (type != null && !type.equals("KVM")) {
                    throw new ApiMessageInterceptionException(Platform.argerr(
                    ORG_ZSTACK_COMPUTE_CLUSTER_10004,         "only kvm hosts' operating system can be updated, for now"
                    ));
                }
                ...
                if (msg.getHostUuid() != null &&
                        ...
                        .isExists()) {
                    throw new ApiMessageInterceptionException(Platform.argerr(
                    ORG_ZSTACK_COMPUTE_CLUSTER_10005,         "The host[uuid: %s] is not part of the cluster[uuid: %s]. Please verify the host uuid and ensure it belongs to this cluster.",
                            msg.getHostUuid(),
                            msg.getUuid()
                    ));
                }
                ...
                if (premaintain != 0) {
                    throw new ApiMessageInterceptionException(Platform.argerr(
                    ORG_ZSTACK_COMPUTE_CLUSTER_10006,         "there are hosts in cluster[uuid:%s] in the PreMaintenance state, cannot update cluster os right now",
                            msg.getUuid()
                    ));
                }
                ...
                if (notConnected != 0) {
                    throw new ApiMessageInterceptionException(Platform.argerr(
                    ORG_ZSTACK_COMPUTE_CLUSTER_10007,         "not all hosts in cluster[uuid:%s] are in the Connected status, cannot update cluster os right now",
                            msg.getUuid()
                    ));
                }
            }
