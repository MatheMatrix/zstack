        ...
        List<VmNicSpec> firstL3s = VmNicSpec.getFirstL3NetworkInventoryOfSpec(spec.getL3Networks())
                ...
                .peek(v -> {
                    if (!Q.New(NormalIpRangeVO.class)
                            ...
                            .isExists()) {
                        throw new OperationFailureException(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10022, "there is no available ipRange on L3 network [%s]", v.getL3Invs().get(0).getUuid()));
                    }
                })
