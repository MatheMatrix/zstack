...
public class MacOperator {
    ...
    private boolean isMulticastMac(String mac) {
        if (!pattern.matcher(mac.toLowerCase()).matches()){
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10000, "This is not a valid MAC address [%s]", mac));
        }
    ...
    public void validateAvailableMac(String mac) {
        ...
        if (!matcher.matches()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10001, "Not a valid MAC address [%s]", mac));
        }
        if ("00:00:00:00:00:00".equals(lowercaseMac) || "ff:ff:ff:ff:ff:ff".equals(lowercaseMac)) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10002, "Disallowed address"));
        }
        if (isMulticastMac(lowercaseMac)){
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10003, "Expected unicast mac address, found multicast MAC address [%s]", mac));
        }
    }
