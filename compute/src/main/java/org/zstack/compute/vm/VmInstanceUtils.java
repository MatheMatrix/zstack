...
public class VmInstanceUtils {
    ...
    public static UpdateVmInstanceSpec convertToSpec(UpdateVmInstanceMsg message, VmInstanceVO vm) {
        ...
        if (vm.getHostUuid() != null) {
            ...
            throw new OperationFailureException(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10015, "failed to find host of vm[uuid=%s]", vmUuid));
        }
    ...
    public static UpdateVmInstanceSpec convertToSpec(APIChangeInstanceOfferingMsg message,
        ...
        if (vm.getHostUuid() != null) {
            ...
            throw new OperationFailureException(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10016, "failed to find host of vm[uuid=%s]", vmUuid));
        }
    ...
    public static void validateInstanceSettings(NewVmInstanceMessage2 msg) {
        ...
        if (instanceOfferingUuid == null) {
            if (msg.getCpuNum() == null || msg.getMemorySize() == null) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10017, "Missing CPU/memory settings"));
            }

            if (msg.getCpuNum() <= 0 || msg.getMemorySize() <= 0) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10018, "Unexpected CPU/memory settings"));
            }

            if (msg.getReservedMemorySize() != null) {
                if (msg.getReservedMemorySize() > msg.getMemorySize()) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10019, "reserved memory[%s] is greater than memory size[%s]", msg.getReservedMemorySize(), msg.getMemorySize()));
                }
            } else {
        ...
        if (ivo.getState() == InstanceOfferingState.Disabled) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10020, "instance offering[uuid:%s] is Disabled, can't create vm from it", instanceOfferingUuid));
        }
