    ...
    public ErrorCode preStartNewCreatedVm(VmInstanceInventory inv) {
        for (VmInstanceStartNewCreatedVmExtensionPoint ext : startNewCreatedVmExtensions) {
            try {
                ...
                if (err != null) {
                    return operr(ORG_ZSTACK_COMPUTE_VM_10237, "VmInstanceStartNewCreatedVmExtensionPoint[%s] refuses to create vm[uuid:%s] because %s",
    ...
    public ErrorCode preRebootVm(final VmInstanceInventory inv) {
        for (VmInstanceRebootExtensionPoint ext : rebootVmExtensions) {
            try {
                ...
                if (err != null) {
                    return operr(ORG_ZSTACK_COMPUTE_VM_10238, "VmInstanceRebootExtensionPoint[%s] refuses to reboot vm[uuid:%s] because %s", ext.getClass().getName(),
                            inv.getUuid(), err);
                }
            } catch (Exception e) {
                logger.warn(String.format("Unhandled exception while calling %s", ext.getClass().getName()), e);
    ...
    public ErrorCode preDestroyVm(VmInstanceInventory inv) {
        for (VmInstanceDestroyExtensionPoint ext : destroyVmExtensions) {
            try {
                ...
                if (err != null) {
                    return operr(ORG_ZSTACK_COMPUTE_VM_10239, "VmInstanceDestroyVmExtensionPoint[%s] refuses to destroy vm[uuid:%s] because %s", ext.getClass().getName(), inv.getUuid(), err);
                }
            } catch (Exception e) {
                logger.warn(String.format("Unhandled exception while calling %s", ext.getClass().getName()), e);
            }
    ...
    public ErrorCode preStartVm(VmInstanceInventory inv) {
        for (VmInstanceStartExtensionPoint ext : startVmExtensions) {
            try {
                ...
                if (err != null) {
                    return operr(ORG_ZSTACK_COMPUTE_VM_10240, "VmInstanceStartExtensionPoint[%s] refuses to start vm[uuid:%s] because %s", ext.getClass().getName(),
                            inv.getUuid(), err);
                }
            } catch (Exception e) {
