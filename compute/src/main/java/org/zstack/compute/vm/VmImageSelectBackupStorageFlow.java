...
public class VmImageSelectBackupStorageFlow extends NoRollbackFlow {
    ...
    private String findBackupStorage(VmInstanceSpec spec, String imageUuid) {
        ...
        if (spec.getVmInventory().getZoneUuid() != null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10028, "cannot find the image[uuid:%s] in any connected backup storage attached to the zone[uuid:%s]. check below:\n" +
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10029, "cannot find the image[uuid:%s] in any connected backup storage. check below:\n" +
                                    "1. if the backup storage is attached to the zone where the VM[name: %s, uuid:%s] is in\n" +
                                    "2. if the backup storage is in connected status, if not, try reconnecting it",
    ...
    private String findIsoBsUuidInTheZone(final String isoImageUuid, final String zoneUuid) {
        ...
        if (ret.isEmpty()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10030, "no backup storage attached to the zone[uuid:%s] contains the ISO[uuid:%s]",
