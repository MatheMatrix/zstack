...
public class VmInstanceApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APITakeVmConsoleScreenshotMsg msg) {
        ...
        if (!vm.getState().equals(VmInstanceState.Running)) {
            ...
            ORG_ZSTACK_COMPUTE_VM_10040,         "can not take vm console screenshot for vm[uuid:%s] which is not Running", msg.getVmInstanceUuid()));
        }
    }
    ...
    private void validate(APIGetInterdependentL3NetworksBackupStoragesMsg msg) {
        if (msg.getL3NetworkUuids() == null && msg.getBackupStorageUuid() == null) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10041, "either l3NetworkUuids or backupStorageUuid must be set"));
        }
    }
    ...
    private void validate(APIChangeVmNicNetworkMsg msg) {
        ...
        long count  = SQL.New("select count(distinct l2.vSwitchType) from L2NetworkVO l2, L3NetworkVO l3 where l2.uuid = l3.l2NetworkUuid" +
                ...
                .find();
        if (count > 1) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10042, "could not change to L3 network, the l2 of l3[uuid:%s, %s] use different vswitch",
        ...
        if (!networkServices.isEmpty()) {
            ...
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10043, error, msg.getVmNicUuid()));
        }
        ...
        if (!VmInstanceState.Stopped.equals(state) && !VmInstanceState.Running.equals(state)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10044, "unable to change to L3 network. The vm[uuid: %s] is not Running or Stopped; the current state is %s",
        ...
        if (l3NetworkVO.enableIpAddressAllocation() && l3NetworkVO.getIpRanges().isEmpty()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10045, "unable to change to L3 network. The L3 network[uuid:%s] doesn't has have ip range",
        ...
        if(l2Uuids.size() > 1) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10046, "unable to change to L3 network. The L3 network[uuid:%s] are belonged to different l2 networks [uuids:%s]",
        ...
        if (clusterUuids.isEmpty()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10047, "unable to change to L3 network. The L3 network[uuid:%s] are belonged to l2 networks [uuids:%s] that have not been attached to any cluster",
                    newAddedL3Uuids, l2Uuids));
        }
        ...
        if (attachedL3Uuids != null && !attachedL3Uuids.isEmpty()) {
            if (!VmGlobalConfig.MULTI_VNIC_SUPPORT.value(Boolean.class)
                ...
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10048, "unable to change to L3 network. The L3 network[uuid:%s] is already attached to the vm[uuid: %s]",
            ...
            if (attachedNonGuestL3Uuids != null && !attachedNonGuestL3Uuids.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10049, "unable to change to a non-guest L3 network. The L3 network[uuid:%s] is already attached to the vm[uuid: %s]",
                        attachedL3Uuids, msg.getVmInstanceUuid()));
            }
        }
        ...
        for (String l3Uuid : newAddedL3Uuids) {
            ...
            if (l3Vo.getState() == L3NetworkState.Disabled) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10050, "unable to change to L3 network. The L3 network[uuid:%s] is disabled", l3Uuid));
            }
            if (VmInstanceConstant.USER_VM_TYPE.equals(type) && l3Vo.isSystem()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10051, "unable to change to L3 network. The L3 network[uuid:%s] is a system network and vm is a user vm",
                        l3Uuid));
            }
        }
        ...
        if (msg.getRequiredIpMap() != null) {
            ...
            if (!found) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10052, "the static IP[%s] is not in any IP range of the L3 network[uuid:%s]", msg.getStaticIp(), msg.getDestL3NetworkUuid()));
            }
            ...
            if (uq.isExists()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10053, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", msg.getStaticIp(), msg.getDestL3NetworkUuid()));
            }
        }

        for (Map.Entry<String, List<String>> e : staticIps.entrySet()) {
            if (!newAddedL3Uuids.contains(e.getKey())) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10054, "static ip l3 uuid[%s] is not included in nic l3 [%s]", e.getKey(), newAddedL3Uuids));
            }
            ...
            for (String staticIp : ips) {
                ...
                if (!found) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10055, "the static IP[%s] is not in any IP range of the L3 network[uuid:%s]", staticIp, l3Uuid));
                }
                ...
                uq.add(UsedIpVO_.ip, Op.EQ, msg.getStaticIp());
                if (uq.isExists()) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10056, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", staticIp, l3Uuid));
                }
            }
        }
        ...
        if (nicIpAddressInfo != null) {
            if (!nicIpAddressInfo.ipv4Address.isEmpty() && Q.New(UsedIpVO.class).eq(UsedIpVO_.ip, nicIpAddressInfo.ipv4Address).eq(UsedIpVO_.l3NetworkUuid, msg.getDestL3NetworkUuid()).isExists()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10057, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", nicIpAddressInfo.ipv4Address, msg.getDestL3NetworkUuid()));
            }
            if (!nicIpAddressInfo.ipv6Address.isEmpty() && Q.New(UsedIpVO.class).eq(UsedIpVO_.ip, IPv6NetworkUtils.getIpv6AddressCanonicalString(nicIpAddressInfo.ipv6Address)).eq(UsedIpVO_.l3NetworkUuid, msg.getDestL3NetworkUuid()).isExists()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10058, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", nicIpAddressInfo.ipv6Address, msg.getDestL3NetworkUuid()));
            }
        }
    }
    ...
    private void validate(APIGetCandidateZonesClustersHostsForCreatingVmMsg msg) {
        ...
        if (instanceOfferingUuid == null) {
            if (msg.getCpuNum() == null || msg.getMemorySize() == null) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10059, "Missing CPU/memory settings"));
            }
        }
        ...
        if (image != null && image.getMediaType() == ImageMediaType.ISO) {
            if (msg.getRootDiskOfferingUuid() == null) {
                if (msg.getRootDiskSize() == null || msg.getRootDiskSize() <= 0) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10060, "the image[name:%s, uuid:%s] is an ISO, rootDiskSize must be set",
    ...
    private void validate(final APICreateVmCdRomMsg msg) {
        ...
        if (!vo.getState().equals(VmInstanceState.Stopped)) {
            ...
            ORG_ZSTACK_COMPUTE_VM_10061,         "Can not create CD-ROM for vm[uuid:%s] which is in state[%s] ", msg.getVmInstanceUuid(), vo.getState().toString()));
        }
    }
    ...
    private void validate(final APIUpdateVmNicDriverMsg msg) {
        ...
        if (vo.getPlatform().equals(ImagePlatform.Other.toString())) {
            throw new ApiMessageInterceptionException(argerr(
            ORG_ZSTACK_COMPUTE_VM_10062,         "Current platform %s not support update nic driver yet", vo.getPlatform()));
        }
    ...
    private void validate(final APIGetCandidatePrimaryStoragesForCreatingVmMsg msg) {
        ...
        if (ImageMediaType.ISO == mediaType) {
            if (msg.getRootDiskOfferingUuid() == null) {
                if (msg.getRootDiskSize() == null || msg.getRootDiskSize() <= 0) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10064, "rootDiskSize is needed when image media type is ISO"));
                }
            }
        }
    }
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (vo.getState().equals(VmInstanceState.Running) && vo.getHostUuid().equals(msg.getHostUuid())) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10065,         "the vm[uuid:%s] is already on host[uuid:%s]", msg.getVmInstanceUuid(), msg.getHostUuid()
                    ));
                }

                if (vo.getState() == VmInstanceState.Paused && VmSystemTags.VM_STATE_PAUSED_AFTER_MIGRATE.hasTag(msg.getVmInstanceUuid())) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10066,         "the vm[uuid:%s] is still paused after the last migration, please resume it before migrate.", msg.getVmInstanceUuid()));
                }
            }
        }.execute();
    }
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (!numa && !VmInstanceState.Stopped.equals(vo.getState())) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10067,         "the VM cannot do online cpu/memory update because of disabling Instance Offering Online Modification. Please stop the VM then do the cpu/memory update again"
                    ));
                }

                if (!VmInstanceState.Stopped.equals(vo.getState()) && !VmInstanceState.Running.equals(vo.getState())) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10068, "The state of vm[uuid:%s] is %s. Only these state[%s] is allowed to update cpu or memory.",
                ...
                if (instanceOfferingVO.getCpuNum() < vo.getCpuNum() || instanceOfferingVO.getMemorySize() < vo.getMemorySize()) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10069,         "can't decrease capacity when vm[uuid:%s] is running", vo.getUuid()
                    ));
                }
            }
        }.execute();
            ...
            protected void scripts() {
                ...
                if (msg.getReservedMemorySize() != null) {
                    ...
                    if (msg.getReservedMemorySize() > memorySize) {
                        throw new ApiMessageInterceptionException(argerr(
                        ORG_ZSTACK_COMPUTE_VM_10070,         "reservedMemorySize[%s] is greater than memorySize[%s]", msg.getReservedMemorySize(), memorySize
                        ));
                    }
                }
                ...
                if (uniqueVmName && Q.New(VmInstanceVO.class).eq(VmInstanceVO_.name, msg.getName()).notEq(VmInstanceVO_.uuid, msg.getUuid()).isExists()) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10071, "could not create vm, a vm with the name [%s] already exists",
                            msg.getName()));
                }
                ...
                if (!numa && !VmInstanceState.Stopped.equals(vmState)) {
                    if (cpuSum != null && cpuSum != vo.getCpuNum()) {
                        throw new ApiMessageInterceptionException(argerr(
                        ORG_ZSTACK_COMPUTE_VM_10072,         "the VM cannot do cpu hot plug because of disabling cpu hot plug. Please stop the VM then do the cpu hot plug again"
                        ));
                    }

                    if (memorySize != null && memorySize != vo.getMemorySize()) {
                        throw new ApiMessageInterceptionException(argerr(
                        ORG_ZSTACK_COMPUTE_VM_10073,         "the VM cannot do memory hot plug because of disabling memory hot plug. Please stop the VM then do the memory hot plug again"
                        ));
                    }
                }

                if (!VmInstanceState.Stopped.equals(vo.getState()) && !VmInstanceState.Running.equals(vo.getState())) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10074, "The state of vm[uuid:%s] is %s. Only these state[%s] is allowed to update cpu or memory.",
                            vo.getUuid(), vo.getState(),
                            StringUtils.join(list(VmInstanceState.Running, VmInstanceState.Stopped), ",")));
                }
                ...
                if (msg.getCpuNum() != null && msg.getCpuNum() < vo.getCpuNum()) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10075,         "can't decrease cpu of vm[uuid:%s] when it is running", vo.getUuid()
                    ));
                }

                if (msg.getMemorySize() != null && msg.getMemorySize() < vo.getMemorySize()) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10076,         "can't decrease memory size of vm[uuid:%s] when it is running", vo.getUuid()
                    ));
                }
            }
        }.execute();
    ...
    private void validateStaticIPv4(VmNicVO vmNicVO, L3NetworkVO l3NetworkVO, String ip) {
        if (!NetworkUtils.isIpv4Address(ip)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10077, "%s is not a valid IPv4 address", ip));
        }
        ...
        for (UsedIpVO ipVo : vmNicVO.getUsedIps()) {
            ...
            if (ipVo.getL3NetworkUuid().equals(l3NetworkVO.getUuid())) {
                if (ipVo.getIp().equals(ip)) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10078, "ip address [%s] already set to vmNic [uuid:%s]",
                            ip, vmNicVO.getUuid()));
                }
                if (!l3NetworkVO.enableIpAddressAllocation()) {
                    continue;
                ...
                if (!NetworkUtils.isIpv4InCidr(ip, rangeVO.getNetworkCidr())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10079, "ip address [%s] is not in ip range [%s]",
    ...
    private void validateStaticIPv6(VmNicVO vmNicVO, L3NetworkVO l3NetworkVO, String ip) {
        if (!IPv6NetworkUtils.isIpv6Address(ip)) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10080, "%s is not a valid IPv6 address", ip));
        }
        ...
        for (UsedIpVO ipVo : vmNicVO.getUsedIps()) {
            ...
            if (ipVo.getL3NetworkUuid().equals(l3NetworkVO.getUuid())) {
                if (ip.equals(ipVo.getIp())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10081, "ip address [%s] already set to vmNic [uuid:%s]",
                            ip, vmNicVO.getUuid()));
                }
                if (!l3NetworkVO.enableIpAddressAllocation()) {
                    continue;
                ...
                if (!IPv6NetworkUtils.isIpv6InRange(ip, rangeVO.getStartIp(), rangeVO.getEndIp())) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10082, "ip address [%s] is not in ip range [startIp %s, endIp %s]",
    ...
    private void validate(APISetVmStaticIpMsg msg) {
        ...
        if (msg.getIp() == null && msg.getIp6() == null) {
            if(l3NetworkVO.enableIpAddressAllocation()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10083, "could not set ip address, due to no ip address is specified"));
            }
        }
        ...
        for (VmNicVO nic : vmNics) {
            ...
            if (msg.getIp() != null) {
                ...
                if (NetworkUtils.isIpv4Address(ip)) {
                    ...
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10084, "static ip [%s] format error", msg.getIp()));
                }
            }
            if (msg.getIp6() != null) {
        ...
        if (msg.getIp() != null && !l3NetworkVO.enableIpAddressAllocation()) {
            ...
            if (msg.getNetmask() == null) {
                if (ipv4Ranges.isEmpty()) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10085, "ipv4 address need a netmask"));
                } else {
            ...
            if (Q.New(UsedIpVO.class).eq(UsedIpVO_.ip, msg.getIp()).eq(UsedIpVO_.l3NetworkUuid, msg.getL3NetworkUuid()).isExists()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10086, "ip address [%s] already set to vmNic", msg.getIp()));
            }
        }
        if (msg.getIp6() != null && !l3NetworkVO.enableIpAddressAllocation()) {
            ...
            if (Q.New(UsedIpVO.class).eq(UsedIpVO_.ip, msg.getIp6()).eq(UsedIpVO_.l3NetworkUuid, msg.getL3NetworkUuid()).isExists()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10088, "ip address [%s] already set to vmNic", msg.getIp6()));
            }
        }
        if (!l3Found) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10089, "the VM[uuid:%s] has no nic on the L3 network[uuid:%s]", msg.getVmInstanceUuid(),
    ...
    private void validate(APIDeleteVmStaticIpMsg msg) {
        ...
        if (!q.isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10090, "the VM[uuid:%s] has no nic on the L3 network[uuid:%s]", msg.getVmInstanceUuid(),
                            msg.getL3NetworkUuid()));
        }
        ...
        if (msg.getStaticIp() != null) {
            if (!Q.New(UsedIpVO.class).eq(UsedIpVO_.l3NetworkUuid, msg.getL3NetworkUuid())
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10091, "could not delete static ip [%s] for vm [uuid:%s] " +
        ...
        if (msg.getBootOrder() != null) {
            for (String o : msg.getBootOrder()) {
                try {
                    ...
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10092, "invalid boot device[%s] in boot order%s", o, msg.getBootOrder()));
                }
            }
        }
    }
    ...
    private void validate(APISetVmBootVolumeMsg msg) {
        ...
        if (volume.isShareable()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10093, "boot volume cannot be shareable."));
        }

        if (!msg.getVmInstanceUuid().equals(volume.getVmInstanceUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10094, "volume[uuid:%s] must be attached to vm[uuid:%s]",
                    msg.getVolumeUuid(), msg.getVmInstanceUuid()));
        }

        if (isVmHasMemorySnapshotGroup(msg.getVmInstanceUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10095, "the vm %s with memory snapshots do not support setting boot volume", msg.getVmInstanceUuid()));
        }
    }
    ...
    private void validate(APIAttachIsoToVmInstanceMsg msg) {
        ...
        if (isoUuids.contains(msg.getIsoUuid())) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10096, "VM[uuid:%s] already has an ISO[uuid:%s] attached", msg.getVmInstanceUuid(), msg.getIsoUuid()));
        }
        ...
        if (type != ImageConstant.ImageMediaType.ISO) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10097, "Unsupported Image Media Type: [%s] ", type));
        }
    ...
    private void validateCdRomUuid(APIAttachIsoToVmInstanceMsg msg) {
        ...
        if (cdRomUuid != null) {
            ...
            if (cdRomVO == null) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10098, "The cdRom[uuid:%s] does not exist", cdRomUuid));
            }

            if (StringUtils.isNotEmpty(cdRomVO.getIsoUuid())){
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10099, "VM[uuid:%s] cdRom[uuid:%s] has mounted the ISO", msg.getVmInstanceUuid(), cdRomUuid));
            }
    ...
    private void validate(APIDetachIsoFromVmInstanceMsg msg) {
        ...
        if (isoUuids.size() > 1 && msg.getIsoUuid() == null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10100, "VM[uuid:%s] has multiple ISOs attached, specify the isoUuid when detaching", msg.getVmInstanceUuid()));
        }
    ...
    private void validate(APICreateVmNicMsg msg) {
        ...
        if (l3VO.getState() == L3NetworkState.Disabled) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10101, "unable to attach a L3 network. The L3 network[uuid:%s] is disabled", msg.getL3NetworkUuid()));
        }
        ...
        if (msg.getIp() != null) {
            ...
            if (!found) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10102, "the static IP[%s] is not in any IP range of the L3 network[uuid:%s]", msg.getIp(), msg.getL3NetworkUuid()));
            }
            ...
            if (uq.isExists()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10103, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", msg.getIp(), msg.getL3NetworkUuid()));
            }
        }
    }
    ...
    private void validate(APIAttachL3NetworkToVmMsg msg) {
        ...
        if (!VmInstanceState.Running.equals(state) && !VmInstanceState.Stopped.equals(state)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10104, "unable to attach a L3 network. The vm[uuid: %s] is not Running or Stopped; the current state is %s",
                            msg.getVmInstanceUuid(), state));
        }
        ...
        l2Uuids = l2Uuids.stream().distinct().collect(Collectors.toList());
        if(l2Uuids.size() > 1) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10106, "unable to attach a L3 network. The L3 network[uuid:%s] are belonged to different l2 networks [uuids:%s]",
                    newAddedL3Uuids, l2Uuids));
        }
        ...
        if (!l2NetworkVOS.isEmpty()) {
            List<String> clusterUuids = Q.New(L2NetworkClusterRefVO.class).eq(L2NetworkClusterRefVO_.l2NetworkUuid, l2Uuids.get(0))
                    .select(L2NetworkClusterRefVO_.clusterUuid).listValues();
            if (clusterUuids.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10107, "unable to attach a L3 network. The L3 network[uuid:%s] are belonged to l2 networks [uuids:%s] that have not been attached to any cluster",
                        newAddedL3Uuids, l2Uuids));
            }
        }
        ...
        if (attachedL3Uuids != null && !attachedL3Uuids.isEmpty()) {
            if (!VmGlobalConfig.MULTI_VNIC_SUPPORT.value(Boolean.class)
                    || !VmInstanceConstant.USER_VM_TYPE.equals(type)) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10108, "unable to attach a L3 network. The L3 network[uuid:%s] is already attached to the vm[uuid: %s]",
                        attachedL3Uuids, msg.getVmInstanceUuid()));
            }
            ...
            List<String> attachedNonGuestL3Uuids = Q.New(L3NetworkVO.class).select(L3NetworkVO_.uuid).
                    notEq(L3NetworkVO_.category, L3NetworkCategory.Private).in(L3NetworkVO_.uuid, attachedL3Uuids).listValues();
            if (attachedNonGuestL3Uuids != null && !attachedNonGuestL3Uuids.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10109, "unable to attach a non-guest L3 network. The L3 network[uuid:%s] is already attached to the vm[uuid: %s]",
                        attachedL3Uuids, msg.getVmInstanceUuid()));
            }
        }
        ...
        for (String l3Uuid : newAddedL3Uuids) {
            L3NetworkVO l3Vo = dbf.findByUuid(l3Uuid, L3NetworkVO.class);
            if (l3Vo.getState() == L3NetworkState.Disabled) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10110, "unable to attach a L3 network. The L3 network[uuid:%s] is disabled", l3Uuid));
            }
            if (VmInstanceConstant.USER_VM_TYPE.equals(type) && l3Vo.isSystem()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10111, "unable to attach a L3 network. The L3 network[uuid:%s] is a system network and vm is a user vm",
                        l3Uuid));
            }
        }
        ...
        if (msg.getStaticIp() != null) {
            ...
            if (!found) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10112, "the static IP[%s] is not in any IP range of the L3 network[uuid:%s]", msg.getStaticIp(), msg.getL3NetworkUuid()));
            }
            ...
            uq.add(UsedIpVO_.ip, Op.EQ, msg.getStaticIp());
            if (uq.isExists()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10113, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", msg.getStaticIp(), msg.getL3NetworkUuid()));
            }
        }

        for (Map.Entry<String, List<String>> e : staticIps.entrySet()) {
            if (!newAddedL3Uuids.contains(e.getKey())) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10114, "static ip l3 uuid[%s] is not included in nic l3 [%s]", e.getKey(), newAddedL3Uuids));
            }
            ...
            for (String staticIp : ips) {
                ...
                if (!found) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10115, "the static IP[%s] is not in any IP range of the L3 network[uuid:%s]", staticIp, l3Uuid));
                }
                ...
                if (uq.isExists()) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10116, "the static IP[%s] has been occupied on the L3 network[uuid:%s]", staticIp, l3Uuid));
                }
            }
        }
        ...
        if (!StringUtils.isEmpty(msg.getVmNicParams())) {
            ...
            try {
                ...
                throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10117, "invalid json format, causes: %s", e.getMessage()));
            }
    ...
    private void validate(APIAttachVmNicToVmMsg msg) {
        ...
        if (!VmInstanceState.Running.equals(state) && !VmInstanceState.Stopped.equals(state)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10118, "unable to attach the nic. The vm[uuid: %s] is not Running or Stopped; the current state is %s",
                    msg.getVmInstanceUuid(), state));
        }
        ...
        if (vmNicVO.getVmInstanceUuid() != null) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10119, "unable to attach the nic. The nic has been attached with vm[uuid: %s]", vmNicVO.getVmInstanceUuid()));
        }
        ...
        if (exist) {
            if (!VmGlobalConfig.MULTI_VNIC_SUPPORT.value(Boolean.class)
                    || !VmInstanceConstant.USER_VM_TYPE.equals(type)) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10120, "unable to attach the nic. Its L3 network[uuid:%s] is already attached to the vm[uuid: %s]",
                        vmNicVO.getL3NetworkUuid(), msg.getVmInstanceUuid()));
            }

            if (!L3NetworkCategory.Private.equals(l3NetworkVO.getCategory())) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10121, "unable to attach the nic with a non-guest L3 network. Its L3 network[uuid:%s] is already attached to the vm[uuid: %s]",
                        vmNicVO.getL3NetworkUuid(), msg.getVmInstanceUuid()));
            }
        }
        ...
        if (l3state == L3NetworkState.Disabled) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10122, "unable to attach the nic. Its L3 network[uuid:%s] is disabled", l3NetworkVO.getUuid()));
        }
        if (VmInstanceConstant.USER_VM_TYPE.equals(type) && system) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10123, "unable to attach the nic. Its L3 network[uuid:%s] is a system network and vm is a user vm",
                    l3NetworkVO.getUuid()));
        }
        ...
        List<String> clusterUuids = Q.New(L2NetworkClusterRefVO.class).eq(L2NetworkClusterRefVO_.l2NetworkUuid, l3NetworkVO.getL2NetworkUuid())
                                     .select(L2NetworkClusterRefVO_.clusterUuid).listValues();
        if (clusterUuids.isEmpty()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10124, "unable to attach the nic. Its l2 network [uuid:%s] that have not been attached to any cluster",
    ...
    private void validate(APIChangeVmNicStateMsg msg) {
        ...
        if (msg.getState().equals(VmNicState.enable.toString()) && !msg.getState().equals(nicVO.getState().toString())) {
            ...
            if (mo.checkDuplicateMac(nicVO.getHypervisorType(), nicVO.getL3NetworkUuid(),
                    nicVO.getMac())) {
                throw new ApiMessageInterceptionException(Platform.argerr(ORG_ZSTACK_COMPUTE_VM_10125, "Duplicate mac address [%s]", nicVO.getMac()));
            }
        }

        if (!nicVO.getType().equals(VmInstanceConstant.VIRTUAL_NIC_TYPE)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10126, "could not update nic[uuid: %s] state, due to nic type[%s] not support",
    ...
    private void validate(APIDetachL3NetworkFromVmMsg msg) {
        ...
        if (!VmInstanceState.Running.equals(state) && !VmInstanceState.Stopped.equals(state)) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10127, "unable to detach a L3 network. The vm[uuid: %s] is not Running or Stopped; the current state is %s",
                    vmUuid, state));
        }
    ...
    private void validate(APIGetVmAttachableDataVolumeMsg msg) {
        ...
        if (state != VmInstanceState.Stopped && state != VmInstanceState.Running) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10128, "vm[uuid:%s] can only attach volume when state is Running or Stopped, current state is %s", msg.getVmInstanceUuid(), state));
        }
    }
        ...
        if (imgFormat == ImageMediaType.ISO) {
            if (msg.getRootDiskOfferingUuid() == null) {
                if (msg.getRootDiskSize() == null) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10129, "image mediaType is ISO but missing root disk settings"));
                }

                if (msg.getRootDiskSize() <= 0) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10130, "Unexpected root disk settings"));
                }
            }
        }
    }
    ...
    private void validatePsWhetherSameCluster(APICreateVmInstanceMsg msg) {
        ...
        if (clusterUuidsForRootVolume.isEmpty()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10131, "the primary storage[%s] of the root volume and the primary storage[%s] of the data volume are not in the same cluster", msg.getPrimaryStorageUuidForRootVolume(), primaryStorageUuidForDataVolume));
        }
    }
    ...
    private void validateZoneOrClusterOrHostOrL3Exist(NewVmInstanceMessage2 msg) {
        if (CollectionUtils.isEmpty(msg.getL3NetworkUuids()) && StringUtils.isEmpty(msg.getZoneUuid())
            ...
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10132, "could not create vm, because at least one of field (l3NetworkUuids,zoneUuid,clusterUuid,hostUuid) should be set"));
        }
    }
    ...
    private void validateDataDiskSizes(APICreateVmInstanceMsg msg) throws ApiMessageInterceptionException {
        ...
        msg.getDataDiskSizes().forEach(dataDiskSize -> {
            if (dataDiskSize <= 0) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10133, "Unexpected data disk settings. dataDiskSizes need to be greater than 0"));
            }
        });
    }
    ...
    private void validate(APICreateVmInstanceMsg msg) {
        ...
        if (CollectionUtils.isNotEmpty(msg.getDiskAOs())) {
            ...
            if (rootDiskAO == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10134, "missing root disk"));
            }
        ...
        if (image == null) {
            ...
            if (!err.isEmpty()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10135, String.format("when imageUuid is null, %s", err)));
            }
        } else {
            ImageState imgState = image.getState();
            if (imgState == ImageState.Disabled) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10136, "image[uuid:%s] is Disabled, can't create vm from it", msg.getImageUuid()));
            }

            ImageStatus imgStatus = image.getStatus();
            if (imgStatus != ImageStatus.Ready) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10137, "image[uuid:%s] is not ready yet, can't create vm from it", msg.getImageUuid()));
            }

            ImageMediaType imgFormat = image.getMediaType();
            if (imgFormat != ImageMediaType.RootVolumeTemplate && imgFormat != ImageMediaType.ISO) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10138, "image[uuid:%s] is of mediaType: %s, only RootVolumeTemplate and ISO can be used to create vm", msg.getImageUuid(), imgFormat));
            }

            boolean isSystemImage = image.isSystem();
            if (isSystemImage && (msg.getType() == null || VmInstanceConstant.USER_VM_TYPE.equals(msg.getType()))) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10139, "image[uuid:%s] is system image, can't be used to create user vm", msg.getImageUuid()));
            }

            if (msg.getPlatform() == null && image.getPlatform() == null) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10140, "at least one of field platform in msg or image[uuid:%s] should be set", msg.getImageUuid()));
            }

            if (msg.getGuestOsType() == null && image.getGuestOsType() == null) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10141, "at least one of field guestOsType in msg or image[uuid:%s] should be set", msg.getImageUuid()));
            }

            if (msg.getArchitecture() == null && image.getArchitecture() == null) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10142, "at least one of field architecture in msg or image[uuid:%s] should be set", msg.getImageUuid()));
            }
        ...
        if (!allDiskOfferingUuids.isEmpty()) {
            ...
            if (!diskUuids.isEmpty()) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10143, "disk offerings[uuids:%s] are Disabled, can not create vm from it", diskUuids));
            }
        }
    ...
    public void checkMutualExclusion(APICreateVmInstanceMsg.DiskAO diskAO) {
        ...
        if (count > 1) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10144, "Cannot set the following properties at the same time: %s", errorMsg));
        }
        ...
        if (count == 0) {
            ...
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10145, "Need to set one of the following properties, and can only be one of them: %s", properties));
        }
    }
    ...
    private void validate(APICreateVmInstanceFromVolumeMsg msg) {
        ...
        if (volume.isShareable()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10146, "cannot create vm instance from a shareable volume."));
        }

        if (volume.isAttached()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10147, "could not create vm instance from a attached volume."));
        }

        if (volume.getStatus() != VolumeStatus.Ready || volume.getState() != VolumeState.Enabled) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10148, "volume[uuid:%s] could not satisfy conditions[state:Enabled status:Ready]", msg.getVolumeUuid()));
        }
    }
    ...
    private void validate(NewVmInstanceMessage2 msg) {
        ...
        if (uniqueVmName && Q.New(VmInstanceVO.class).eq(VmInstanceVO_.name, msg.getName()).isExists()) {
            throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10149, "could not create vm, a vm with the name [%s] already exists",
                    msg.getName()));
        }
        ...
        if (null != msg.getSystemTags()) {
            ...
            if (duplicateMac.isPresent()){
                throw new ApiMessageInterceptionException(operr(
                ORG_ZSTACK_COMPUTE_VM_10150,         "Not allowed same mac [%s]", duplicateMac.get()));
            }
        }
        ...
        if (msg.getVmNicParams() != null && !msg.getVmNicParams().isEmpty()) {
            ...
            if (msg.getL3NetworkUuids() == null || msg.getL3NetworkUuids().isEmpty()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10151, "l3NetworkUuids and vmNicInventories mustn't both be empty or both be set"));
            }
            ...
            try {
                ...
                throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10152, "invalid json format, causes: %s", e.getMessage()));
            }
        ...
        if (!CollectionUtils.isEmpty(msg.getL3NetworkUuids())) {
            ...
            if (duplicateElements.size() > 0) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10153, "Can't add same uuid in the l3Network,uuid: %s", duplicateElements.get(0)));
            }
            ...
            for (Tuple t : l3ts) {
                ...
                if (state != L3NetworkState.Enabled) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10154, "l3Network[uuid:%s] is Disabled, can not create vm on it", l3Uuid));
                }
                if (system && (msg.getType() == null || VmInstanceConstant.USER_VM_TYPE.equals(msg.getType()))) {
                    throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10155, "l3Network[uuid:%s] is system network, can not create user vm on it", l3Uuid));
                }
            }
        }
        ...
        if (msg.getZoneUuid() != null) {
            ...
            if (zoneState == ZoneState.Disabled) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10156, "zone[uuid:%s] is specified but it's Disabled, can not create vm from it", msg.getZoneUuid()));
            }
        }
        ...
        if (msg.getClusterUuid() != null) {
            ...
            if (clusterState == ClusterState.Disabled) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10157, "cluster[uuid:%s] is specified but it's Disabled, can not create vm from it", msg.getClusterUuid()));
            }
        }
        ...
        if (msg.getHostUuid() != null) {
            ...
            if (hostState == HostState.Disabled) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10158, "host[uuid:%s] is specified but it's Disabled, can not create vm from it", msg.getHostUuid()));
            }

            HostStatus connectionState = t.get(1, HostStatus.class);
            if (connectionState != HostStatus.Connected) {
                throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10159, "host[uuid:%s] is specified but it's connection status is %s, can not create vm from it", msg.getHostUuid(), connectionState));
            }
        }
        ...
        if (VmInstanceConstant.USER_VM_TYPE.equals(msg.getType())) {
            if (msg.getDefaultL3NetworkUuid() == null && (!CollectionUtils.isEmpty(msg.getL3NetworkUuids()) && msg.getL3NetworkUuids().size() != 1)) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10160, "there are more than one L3 network specified in l3NetworkUuids, but defaultL3NetworkUuid is null"));
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10161, "defaultL3NetworkUuid[uuid:%s] is not in l3NetworkUuids%s", msg.getDefaultL3NetworkUuid(), msg.getL3NetworkUuids()));
            }
        }
    ...
    private void validateCdRomsTag(NewVmInstanceMessage msg) {
        ...
        for (String cdRomIsoUuid : cdRoms) {
            ...
            if (imageVO == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10162, "The image[uuid=%s] does not exist", cdRomIsoUuid));
            }
        }

        if (cdRoms.size() != new HashSet<>(cdRoms).size()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10163, "Do not allow to mount duplicate ISO"));
        }
    }
    ...
    private void validate(APISetVmConsolePasswordMsg msg) {
        ...
        if (pwd.startsWith("password")){
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10164, "The console password cannot start with 'password' which may trigger a VNC security issue"));
        }
    }

    private void validate(APIAttachL3NetworkToVmNicMsg msg) {
        throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10165, "can not call this api because it's Deprecated"));
    }
    ...
    private void validate(APISetVmInstanceDefaultCdRomMsg msg) {
        ...
        if (vmCdRomVO.getDeviceId() == 0) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10166, "The CdRom[%s] Already the default", vmCdRomVO.getUuid()));
        }
    }
    ...
    private void validate(APIFstrimVmMsg msg) {
        ...
        if (state != VmInstanceState.Running) {
            throw new ApiMessageInterceptionException(operr(
            ORG_ZSTACK_COMPUTE_VM_10167,         "vm[uuid:%s] can only fstrim when state is Running, current state is %s", msg.getUuid(), state));
        }
