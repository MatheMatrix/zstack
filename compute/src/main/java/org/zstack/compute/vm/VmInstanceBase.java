...
public class VmInstanceBase extends AbstractVmInstance {
    ...
    protected VmInstanceVO refreshVO(boolean noException) {
        ...
        if (self == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10163, "vm[uuid:%s, name:%s] has been deleted", vo.getUuid(), vo.getName()));
        }
        ...
        bus.send(gmsg, new CloudBusCallBack((NeedReplyMessage) msg) {
            ...
            public void run(MessageReply r) {
                ...
                if (!hasTaskVols.isEmpty()) {
                    bus.replyErrorByMessageType((Message) msg, operr(ORG_ZSTACK_COMPUTE_VM_10164, Strings.join(hasTaskVols, ';')));
                    return;
                }
    ...
    private void executeCrashStrategy(final ExecuteCrashStrategyMsg msg, final SyncTaskChain chain) {
        executeCrashStrategy(msg, new Completion(chain) {
            ...
            public void fail(ErrorCode errorCode) {
                ExecuteCrashStrategyReply reply = new ExecuteCrashStrategyReply();
                reply.setError(err(ORG_ZSTACK_COMPUTE_VM_10165, VmErrors.REBOOT_ERROR, errorCode, errorCode.getDetails()));
                bus.reply(msg, reply);
                chain.next();
            }
        });
    ...
    protected void executeCrashStrategy(final Message fmsg, final Completion completion) {
        ...
        bus.send(cmsg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                if (!reply.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10166, "failed to check state of the vm[uuid:%s] on the host[uuid:%s], %s", vmInv.getUuid(), vmInv.getHostUuid(), reply.getError()));
                    return;
                }
                ...
                if (state == null) {
                    completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10167, "got an unrecognized state of the vm[uuid:%s] on the host[uuid:%s]", vmInv.getUuid(), vmInv.getHostUuid()));
                    return;
                }
    ...
    private void changeVmIp(final String l3Uuid, final Map<Integer, String> staticIpMap, final Completion completion) {
        ...
        if (targetNic == null) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10168, "the vm[uuid:%s] has no nic on the L3 network[uuid:%s]", self.getUuid(), l3Uuid));
        }
    ...
    private void vmStateChangeOnHost(final VmStateChangedOnHostMsg msg, final NoErrorCompletion completion) {
        ...
        if (refreshVO(true) == null) {
            ...
            reply.setError(operr(ORG_ZSTACK_COMPUTE_VM_10169, "the vm has been deleted"));
            bus.reply(msg, reply);
            completion.done();
            return;
        }
    ...
    private void rebootVm(final RebootVmInstanceMsg msg, final SyncTaskChain chain) {
        rebootVm(msg, new Completion(chain) {
            ...
            public void fail(ErrorCode errorCode) {
                RebootVmInstanceReply reply = new RebootVmInstanceReply();
                reply.setError(err(ORG_ZSTACK_COMPUTE_VM_10170, VmErrors.REBOOT_ERROR, errorCode, errorCode.getDetails()));
                bus.reply(msg, reply);
                chain.next();
            }
        });
    ...
    private void stopVm(final StopVmInstanceMsg msg, final SyncTaskChain chain) {
        stopVm(msg, new Completion(chain) {
            ...
            public void fail(ErrorCode errorCode) {
                StopVmInstanceReply reply = new StopVmInstanceReply();
                reply.setError(err(ORG_ZSTACK_COMPUTE_VM_10171, VmErrors.STOP_ERROR, errorCode, errorCode.getDetails()));
                bus.reply(msg, reply);
                chain.next();
            }
        });
        ...
        try {
            ...
            bus.send(cmsg, new CloudBusCallBack(chain) {
                private void fail(ErrorCode errorCode) {
                    reply.setError(operr(ORG_ZSTACK_COMPUTE_VM_10172, errorCode, "failed to create template from root volume[uuid:%s] on primary storage[uuid:%s]",
    ...
    private void handle(final AttachNicToVmMsg msg) {
        ...
        bus.send(amsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!reply.isSuccess()) {
                    r.setError(err(ORG_ZSTACK_COMPUTE_VM_10173, VmErrors.ATTACH_NETWORK_ERROR, r.getError(), r.getError().getDetails()));
                }
                bus.reply(msg, r);
            }
        });
    ...
    private void handle(APIGetVmDeviceAddressMsg msg) {
        ...
        if (self.getHostUuid() == null || self.getState() != VmInstanceState.Running) {
            reply.setError(operr(ORG_ZSTACK_COMPUTE_VM_10174, "VM[uuid:%s] state is not Running.", msg.getUuid()));
            bus.reply(msg, reply);
            return;
        }
    ...
    private void checkIfIsoAttachable(String isoUuid) {
        ...
        throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10175, "the ISO[uuid:%s] is on backup storage that is not compatible of the primary storage[uuid:%s]" +
        ...
        bus.send(innerMsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply innerReply) {
                if (!innerReply.isSuccess()) {
                    completion.fail(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10176, innerReply.getError(),
                ...
                completion.fail(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10177, list,
        ...
        chain.then(new NoRollbackFlow() {
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                if (msg.getName() != null) {
                    ...
                    if (unique && exists) {
                        trigger.fail(operr(ORG_ZSTACK_COMPUTE_VM_10178, "could not create vm, a vm with the name [%s] already exists", msg.getName()));
                        return;
                    }
        ...
        chain.then(new NoRollbackFlow() {
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                bus.send(innerMsg, new CloudBusCallBack(innerMsg) {
                    ...
                    public void run(MessageReply innerReply) {
                        if (!innerReply.isSuccess()) {
                            trigger.fail(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10179, innerReply.getError(),
                                    "failed to update vm[uuid=%s] on hypervisor", self.getUuid()));
                            return;
                        }
                        final UpdateVmOnHypervisorReply casedReply = innerReply.castReply();
                        ...
                        final ErrorCodeList list = new ErrorCodeList();
                        list.getCauses().addAll(casedReply.getIgnoredErrors());
                        trigger.fail(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10180, list,
                                "failed to update vm[uuid=%s] on hypervisor: The modification of some properties failed",
                                self.getUuid()));
                    }
                });
    ...
    private void updateVmIsoFirstOrder(List<String> systemTags) {
        ...
        if (!isoList.contains(isoUuid)) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10181, "ISO[uuid:%s] is not attached to VM[uuid:%s]", isoUuid, self.getUuid()));
        }
    ...
    private void detachVolume(final DetachDataVolumeFromVmMsg msg, final Completion completion) {
        ...
        if (allowed != null) {
            completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10182, "Detaching volume is not allowed when VM[uuid=%s] is in state[%s]",
        ...
        chain.done(new FlowDoneHandler(completion) {
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10183, errCode, "Failed to detach volume[uuid=%s] of VM[uuid=%s]",
    ...
    protected void attachDataVolume(final AttachDataVolumeToVmMsg msg, final NoErrorCompletion completion) {
        ...
        chain.done(new FlowDoneHandler(msg, completion) {
            ...
            public void handle(final ErrorCode errCode, Map data) {
                ...
                reply.setError(err(ORG_ZSTACK_COMPUTE_VM_10184, VmErrors.ATTACH_VOLUME_ERROR, errCode, errCode.getDetails()));
                bus.reply(msg, reply);
                completion.done();
            }
        }).start();
            ...
            for (VmNicSpec nicSpec : struct.getL3NetworkUuids()) {
                ...
                for (L3NetworkInventory inv : nicSpec.l3Invs) {
                    ...
                    if (l3 == null) {
                        ...
                        ORG_ZSTACK_COMPUTE_VM_10185,         "Unable to find L3Network[uuid:%s] to start the current vm, it may have been deleted, " +
    ...
    private List<CdRomSpec> buildVmCdRomSpecsForNewCreated(VmInstanceSpec vmSpec) {
        ...
        if (cdRomSpecs.size() > max) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10186, "One vm cannot create %s CDROMs, vm can only add %s CDROMs", cdRomSpecs.size(), max));
        }
    ...
    protected void instantiateVmFromNewCreate(InstantiateVmFromNewCreatedStruct struct, Completion completion) {
        ...
        chain.done(new FlowDoneHandler(completion) {
            ...
            public void handle(final ErrorCode errCode, Map data) {
                ...
                completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10187, errCode, errCode.getDetails()));
            }
        }).start();
    }
    ...
    protected void startVm(final StartVmInstanceMsg msg, final SyncTaskChain taskChain) {
        startVm(msg, new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                StartVmInstanceReply reply = new StartVmInstanceReply();
                reply.setError(err(ORG_ZSTACK_COMPUTE_VM_10188, VmErrors.START_ERROR, errorCode, errorCode.getDetails()));
                bus.reply(msg, reply);
                taskChain.next();
            }
        });
    ...
    protected void startVm(final APIStartVmInstanceMsg msg, final SyncTaskChain taskChain) {
        startVm(msg, new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                APIStartVmInstanceEvent evt = new APIStartVmInstanceEvent(msg.getId());
                evt.setError(err(ORG_ZSTACK_COMPUTE_VM_10189, VmErrors.START_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
                taskChain.next();
            }
        });
    ...
    private void destroyVm(APIDestroyVmInstanceMsg msg, final Completion completion) {
        ...
        chain.done(new FlowDoneHandler(msg) {
            ...
            @Override
            public void handle(ErrorCode errCode, Map data) {
                completion.fail(err(ORG_ZSTACK_COMPUTE_VM_10190, SysErrors.DELETE_RESOURCE_ERROR, errCode, errCode.getDetails()));
            }
        }).start();
    }
    ...
    protected void buildImageSpec(VmInstanceSpec spec) {
        ...
        if (imvo != null) {
            ...
            if (resultList.isEmpty()) {
                ...
                if (resultList.isEmpty()) {
                    throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10191, "no way to get image size of %s, report exception.", spec.getVmInventory().getImageUuid()));
                }
            }
    ...
    protected void rebootVm(final APIRebootVmInstanceMsg msg, final SyncTaskChain taskChain) {
        rebootVm(msg, new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                APIRebootVmInstanceEvent evt = new APIRebootVmInstanceEvent(msg.getId());
                evt.setError(err(ORG_ZSTACK_COMPUTE_VM_10192, VmErrors.REBOOT_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
                taskChain.next();
            }
        });
    ...
    protected void handle(final APIRebootVmInstanceMsg msg) {
        ...
        bus.send(rmsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply reply) {
                if (!reply.isSuccess()) {
                    evt.setError(err(ORG_ZSTACK_COMPUTE_VM_10193, VmErrors.REBOOT_ERROR, reply.getError(), reply.getError().getDetails()));
                } else {
                    refreshVO();
                    evt.setInventory(getSelfInventory());
                }
    ...
    protected void stopVm(final APIStopVmInstanceMsg msg, final SyncTaskChain taskChain) {
        stopVm(msg, new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                APIStopVmInstanceEvent evt = new APIStopVmInstanceEvent(msg.getId());
                evt.setError(err(ORG_ZSTACK_COMPUTE_VM_10194, VmErrors.STOP_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
                taskChain.next();
            }
        });
    ...
    protected void pauseVm(final APIPauseVmInstanceMsg msg, final SyncTaskChain taskChain) {
        pauseVm(msg, new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                APIPauseVmInstanceEvent evt = new APIPauseVmInstanceEvent(msg.getId());
                evt.setError(err(ORG_ZSTACK_COMPUTE_VM_10195, VmErrors.SUSPEND_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
                taskChain.next();
            }
        });
    ...
    protected void resumeVm(final APIResumeVmInstanceMsg msg, final SyncTaskChain taskChain) {
        resumeVm(msg, new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                APIResumeVmInstanceEvent evt = new APIResumeVmInstanceEvent(msg.getId());
                evt.setError(err(ORG_ZSTACK_COMPUTE_VM_10196, VmErrors.RESUME_ERROR, errorCode, errorCode.getDetails()));
                bus.publish(evt);
                taskChain.next();
            }
        });
    ...
    protected void checkStateAndStartVmInstance(final CheckAndStartVmInstanceMsg msg, final SyncTaskChain taskChain) {
        checkStateAndStartVmInstance(new Completion(taskChain) {
            ...
            public void fail(ErrorCode errorCode) {
                CheckAndStartVmInstanceReply reply = new CheckAndStartVmInstanceReply();
                reply.setError(err(ORG_ZSTACK_COMPUTE_VM_10197, VmErrors.START_ERROR, errorCode, errorCode.getDetails()));
                bus.reply(msg, reply);
                taskChain.next();
            }
        });
    ...
    private void doCreateVmCdRom(CreateVmCdRomMsg msg, ReturnValueCompletion<VmCdRomInventory> completion) {
        ...
        int max = VmGlobalConfig.MAXIMUM_CD_ROM_NUM.value(Integer.class);
        if (max <= vmCdRomNum) {
            completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10198, "VM[uuid:%s] can only add %s CDROMs", msg.getVmInstanceUuid(), max));
            return;
        }
        ...
        if (msg.getIsoUuid() != null) {
            boolean targetIsoUsed = Q.New(VmCdRomVO.class)
                    ...
                    .isExists();
            if (targetIsoUsed) {
                completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10199, "VM[uuid:%s] already has an ISO[uuid:%s] attached", msg.getVmInstanceUuid(), msg.getIsoUuid()));
                return;
            }
        }
        ...
        if (targetDeviceId >= max) {
            completion.fail(operr(ORG_ZSTACK_COMPUTE_VM_10200, "VM[uuid:%s] can only add %s CDROMs", msg.getVmInstanceUuid(), max));
            return;
        }
    ...
    private void updateVmPriority(APIUpdateVmPriorityMsg msg, Completion completion) {
        ...
        bus.send(smsg, new CloudBusCallBack(msg) {
            ...
            public void run(MessageReply reply) {
                if (!reply.isSuccess()) {
                    ErrorCode err = operr(ORG_ZSTACK_COMPUTE_VM_10201, "update vm[%s] priority to [%s] failed,because %s",
    ...
    private void handle(ReimageVmInstanceMsg msg) {
        ...
        // check vm stopped
        {
            if (self.getState() != VmInstanceState.Stopped) {
                ...
                ORG_ZSTACK_COMPUTE_VM_10202,         VmErrors.RE_IMAGE_VM_NOT_IN_STOPPED_STATE,
        ...
        // check image cache to ensure image type is not ISO
        {
            ...
            if (imageMediaType == null) {
                throw new OperationFailureException(err(
                ORG_ZSTACK_COMPUTE_VM_10203,         VmErrors.RE_IMAGE_CANNOT_FIND_IMAGE_CACHE,
                        "unable to reset volume[uuid:%s] to origin image[uuid:%s]," +
            ...
            if (imageMediaType.toString().equals("ISO")) {
                throw new OperationFailureException(err(
                ORG_ZSTACK_COMPUTE_VM_10204,         VmErrors.RE_IMAGE_IMAGE_MEDIA_TYPE_SHOULD_NOT_BE_ISO,
                        "unable to reset volume[uuid:%s] to origin image[uuid:%s]," +
