 ...
 * 通过 {@link VmMetadataUpdateInterceptor} 自动触发标脏。
 ...
 *   <li>健康巡检兜底：周期全量比对 DB vs 存储元数据</li>
 * </ol>
 */
        ...
        for (String vmUuid : vmUuids) {
            ...
            if (dbf.isExist(vmUuid, VmInstanceVO.class)) {
                logger.debug(String.format("[MetadataCascade] volume cascade cleanup affected "
                        + "vm[uuid:%s], marking dirty for metadata update", vmUuid));
                // 级联删除 Volume 属于存储结构变更（STORAGE → OP type 2）
