...
public class VmInstanceManagerImpl extends AbstractService implements
    ...
    private void handle(APIGetSpiceCertificatesMsg msg) {
        ...
        if (StringUtils.isNotEmpty(certificateStr)) {
            ...
            reply.setError(operr(ORG_ZSTACK_COMPUTE_VM_10227, "Spice certificate does not exist, Please check if spice tls is enabled"));
        }
        bus.reply(msg, reply);
    }
    ...
    private List<L3NetworkInventory> getInterdependentL3NetworksByImageUuid(APIGetInterdependentL3NetworksImagesMsg msg, String accountUuid) {
        ...
        if (bss.isEmpty()) {
            throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10228, "the image[uuid:%s] is not on any backup storage that has been attached to the zone[uuid:%s]",
    ...
    private List<L3NetworkInventory> getInterdependentL3NetworksByBackupStorageUuids(List<BackupStorageVO> bss, String zoneUuid, String accountUuid, boolean raiseException) {
        ...
        if (l3s.isEmpty()) {
            if (psUuids.isEmpty()) {
                if (raiseException) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10229, "no primary storage accessible to the backup storage[uuid:%s, type:%s] is found",
            ...
            if (clusterNum == null || clusterNum == 0) {
                if (raiseException) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10230, "the primary storages[uuids:%s] has not attached any cluster on the zone[uuid:%s]",
                            psUuids, zoneUuid));
                }
            ...
            if (l2Num == null || l2Num == 0) {
                if (raiseException) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10231, "no l2Networks found in clusters that have attached to primary storages[uuids:%s]",
                            psUuids));
                }
    ...
    private void handle(APIGetCandidateZonesClustersHostsForCreatingVmMsg msg) {
        ...
        if (image.getBackupStorageRefs().size() == 1) {
            ...
            if (msg.getZoneUuid() == null) {
                throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10232, "zoneUuid must be set because the image[name:%s, uuid:%s] is on multiple backup storage",
    ...
    private List<ErrorCode> extEmitterHandleSystemTag(final CreateVmInstanceMsg msg, final APICreateMessage cmsg, VmInstanceVO finalVo) {
        ...
        if (msg == null) {
            result.add(operr(ORG_ZSTACK_COMPUTE_VM_10233, "CreateVmInstanceMsg cannot be null"));
            return result;
    ...
    private List<ErrorCode> extEmitterHandleSshKeyPair(final CreateVmInstanceMsg msg, final APICreateMessage cmsg, VmInstanceVO finalVo) {
        List<ErrorCode> result = Collections.emptyList();
        if (msg == null) {
            result.add(operr(ORG_ZSTACK_COMPUTE_VM_10234, "CreateVmInstanceMsg cannot be null"));
            return result;
            ...
            public void setup() {
                ...
                flow(new Flow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        try {
                            ...
                            errorCodes.add(operr(ORG_ZSTACK_COMPUTE_VM_10235, "instantiate system tag for vm failed because %s", e.getMessage()));
                        }
                        ...
                        if (!errorCodes.isEmpty()) {
                            trigger.fail(operr(ORG_ZSTACK_COMPUTE_VM_10236, "handle system tag fail when creating vm because [%s]",
                ...
                flow(new Flow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (!errorCodes.isEmpty()) {
                            trigger.fail(operr(ORG_ZSTACK_COMPUTE_VM_10237, "handle sshkeypair fail when creating vm because [%s]",
            ...
            public void beforeUpdateExtensionPoint(GlobalConfig oldConfig, String newValue) {
                ...
                for (Tuple tuple: tuples) {
                    if (tuple.get(2, Long.class) > 1) {
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10238, "unable to enable this function. There are multi nics of L3 network[uuid:%s] in the vm[uuid: %s]",
    ...
    private void installHostnameValidator() {
        class HostNameValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            private void validateHostname(String tag, String hostname) {
                ...
                if (!domainValidator.isValid(hostname)) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10239, "hostname[%s] specified in system tag[%s] is not a valid domain name", hostname, tag));
                }
            }
            ...
            public void validateSystemTagInCreateMessage(APICreateMessage cmsg) {
                ...
                for (String sysTag : msg.getSystemTags()) {
                    if (VmSystemTags.HOSTNAME.isMatch(sysTag)) {
                        if (++hostnameCount > 1) {
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10240, "only one hostname system tag is allowed, but %s got", hostnameCount));
                        }
            ...
            private void validateHostNameOnDefaultL3Network(String tag, String hostname, String l3Uuid) {
                ...
                if (!vos.isEmpty()) {
                    ...
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10241, "conflict hostname in system tag[%s];" +
            ...
            private void validateBootOrder(String systemTag) {
                ...
                for (String o : order.split(",")) {
                    try {
                        ...
                        throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10242, "invalid boot device[%s] in boot order[%s]", o, order));
                    }
                }
            }
        }
        ...
        VmHardwareSystemTags.CPU_SOCKETS.installValidator((resourceUuid, resourceType, systemTag) -> {
            ...
            try {
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10243, "cpuSockets must be an integer"));
            }
        });
        ...
        VmHardwareSystemTags.CPU_CORES.installValidator((resourceUuid, resourceType, systemTag) -> {
            ...
            try {
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10244, "cpuCores must be an integer"));
            }
        });
        ...
        VmHardwareSystemTags.CPU_THREADS.installValidator((resourceUuid, resourceType, systemTag) -> {
            ...
            try {
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10245, "cpuThreads must be an integer"));
            }
        });
    }
        ...
        class UserDataValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            ...
            private void check(String resourceUuid, Class resourceType) {
                ...
                if (existUserdataTagCount > 0) {
                    ...
                    ORG_ZSTACK_COMPUTE_VM_10246,         "Already have one userdata systemTag for vm[uuid: %s].",
                            resourceUuid));
                }
            }
                ...
                for (String sysTag : msg.getSystemTags()) {
                    if (VmSystemTags.USERDATA.isMatch(sysTag)) {
                        if (userdataTagCount > 0) {
                            throw new OperationFailureException(argerr(
                            ORG_ZSTACK_COMPUTE_VM_10247,         "Shouldn't be more than one userdata systemTag for one vm."));
                        }
                        userdataTagCount++;
        ...
        class BootModeValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            ...
            public void validateSystemTagInCreateMessage(APICreateMessage msg) {
                ...
                for (String systemTag : msg.getSystemTags()) {
                    if (VmSystemTags.BOOT_MODE.isMatch(systemTag)) {
                        if (++bootModeCount > 1) {
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10248, "only one bootMode system tag is allowed, but %d got", bootModeCount));
                        }
            ...
            private void validateBootMode(String systemTag, String bootMode) {
                ...
                if (!valid) {
                    ...
                    ORG_ZSTACK_COMPUTE_VM_10249,         "[%s] specified in system tag [%s] is not a valid boot mode", bootMode, systemTag)
                    );
                }
            }
        }
        ...
        class CleanTrafficValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            ...
            private void validateVmType(String vmUuid, String vmType) {
                ...
                if (!VmInstanceConstant.USER_VM_TYPE.equals(vmType)) {
                    throw new ApiMessageInterceptionException(argerr(
                    ORG_ZSTACK_COMPUTE_VM_10250,         "clean traffic is not supported for vm type [%s]", vmType)
                    );
                }
            }
        }
        ...
        class MachineTypeValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
            ...
            private void validateMachineType(String systemTag) {
                ...
                if (VmMachineType.get(type) == null) {
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10251, "vm machine type requires [q35, pc, virt], but get [%s]", type));
                }
            }
        }
        ...
        VmSystemTags.SECURITY_ELEMENT_ENABLE.installValidator(new SystemTagValidator() {
            ...
            public void validateSystemTag(String resourceUuid, Class resourceType, String systemTag) {
                ...
                if (VmSystemTags.SECURITY_ELEMENT_ENABLE.isMatch(systemTag)) {
                    ...
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10252, "invalid securityElementEnable[%s], %s is not securityElementEnable tag", systemTag, SecurityElementEnableTokenByTag));
                }
                if (!isBoolean(SecurityElementEnableTokenByTag)) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10253, "invalid securityElementEnable[%s], %s is not boolean class", systemTag, SecurityElementEnableTokenByTag));
                }
            }
        ...
        VmSystemTags.USB_REDIRECT.installValidator(new SystemTagValidator() {
            ...
            public void validateSystemTag(String resourceUuid, Class resourceType, String systemTag) {
                ...
                if (VmSystemTags.USB_REDIRECT.isMatch(systemTag)) {
                    ...
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10254, "invalid usbRedirect[%s], %s is not usbRedirect tag", systemTag, usbRedirectTokenByTag));
                }
                if (!isBoolean(usbRedirectTokenByTag)) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_COMPUTE_VM_10255, "invalid usbRedirect[%s], %s is not boolean class", systemTag, usbRedirectTokenByTag));
                }
            }
            private boolean isBoolean(String param) {
                return "true".equalsIgnoreCase(param) || "false".equalsIgnoreCase(param);
                ...
                .addMessageRequiredQuotaHandler(VmQuotaConstant.VM_RUNNING_CPU_NUM, (msg) -> {
                    ...
                    if (imgType == ImageConstant.ImageMediaType.RootVolumeTemplate) {
                        ...
                        if (msg.getRootDiskOfferingUuid() != null) {
                            ...
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10256, "rootDiskOfferingUuid cannot be null when image mediaType is ISO"));
                        }
                    } else {
                        if (msg.getRootDiskOfferingUuid() != null) {
                            diskOfferingUuids.add(msg.getRootDiskOfferingUuid());
                            ...
                            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10257, "rootDiskOfferingUuid cannot be null when create vm without image"));
                        }
                    }
    ...
    private void validateAPIChangeResourceOwnerMsg(APIChangeResourceOwnerMsg msg) {
        ...
        if (vq.isExists()) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10258, "the resource[uuid:%s] is a ROOT volume, you cannot change its owner, instead," +
