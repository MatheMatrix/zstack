    ...
    public void run(final FlowTrigger trigger, final Map data) {
        ...
        if (spec.getImageSpec().relyOnImageCache()) {
            ...
            if (!CollectionUtils.isEmpty(requirdPsUuids) && Collections.disjoint(requirdPsUuids, cachedPsUuids)) {
                trigger.fail(operr(ORG_ZSTACK_COMPUTE_VM_10026, "creation rely on image cache[uuid:%s, locate ps uuids: [%s]], cannot create other places.", imageUuid, cachedPsUuids));
                return;
        ...
        if (!CollectionUtils.isEmpty(spec.getCandidatePrimaryStorageUuidsForRootVolume())) {
            ...
            if (filterPsUuids.isEmpty()) {
                trigger.fail(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10027, String.format("none of the specified primary storages%s are available",  spec.getCandidatePrimaryStorageUuidsForRootVolume())));
                return;
            }
        ...
        if (needCreateDataVolume(spec) && !CollectionUtils.isEmpty(spec.getCandidatePrimaryStorageUuidsForDataVolume())) {
            ...
            if (filterPsUuids.isEmpty()) {
                trigger.fail(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10028, String.format("none of the specified primary storages%s are available",  spec.getCandidatePrimaryStorageUuidsForDataVolume())));
                return;
            }
