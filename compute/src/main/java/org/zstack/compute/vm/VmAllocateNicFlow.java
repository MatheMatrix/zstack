...
public class VmAllocateNicFlow implements Flow {
    ...
    public void run(final FlowTrigger trigger, final Map data) {
        ...
        new While<>(VmNicSpec.getFirstL3NetworkInventoryOfSpec(spec.getL3Networks())).each((nicSpec, wcomp) -> {
            ...
            if (type == null) {
                errs.add(Platform.operr(ORG_ZSTACK_COMPUTE_VM_10011, "there is no available nicType on L3 network [%s]", nw.getUuid()));
                wcomp.allDone();
            }
            ...
            if (mo.checkDuplicateMac(nic.getHypervisorType(), nic.getL3NetworkUuid(), nic.getMac())) {
                trigger.fail(operr(ORG_ZSTACK_COMPUTE_VM_10012, "Duplicate mac address [%s]", nic.getMac()));
                return;
            }
