...
public class IsoOperator {
    ...
    static void checkAttachIsoToVm(String vmUuid, String isoUuid) {
        ...
        if (isoList.contains(isoUuid)) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10022, "VM[uuid:%s] has attached ISO[uuid:%s]", vmUuid, isoUuid));
        }
        ...
        long emptyCdRomNum = Q.New(VmCdRomVO.class)
                ...
                .count();
        if (emptyCdRomNum == 0) {
            throw new OperationFailureException(operr(ORG_ZSTACK_COMPUTE_VM_10023, "All vm[uuid:%s] CD-ROMs have mounted ISO", vmUuid));
        }
    }
