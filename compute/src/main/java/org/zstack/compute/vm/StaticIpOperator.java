...
public class StaticIpOperator implements SystemTagCreateMessageValidator, SystemTagValidator {
        ...
        for (String sysTag : systemTags) {
            if(VmSystemTags.STATIC_IP.isMatch(sysTag)) {
                ...
                if (NetworkUtils.isIpv4Address(ip)) {
                    ...
                    throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10227, "the static IP[%s] format error", ip));
                }
            }
        }
    ...
    public void checkIpAvailability(String l3Uuid, String ip) {
        ...
        if (!r.isSuccess()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10228, r.getError().getDetails()));
        }

        CheckIpAvailabilityReply cr = r.castReply();
        if (!cr.isAvailable()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_COMPUTE_VM_10229, "IP[%s] is not available on the L3 network[uuid:%s] because: %s", ip, l3Uuid, cr.getReason()));
        }
    }
            ...
            if (!StringUtils.isEmpty(nicIp.ipv4Address)) {
                ...
                if (ipRangeVO == null) {
                    if (StringUtils.isEmpty(nicIp.ipv4Netmask)) {
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10230, "netmask must be set"));
                    }
                } else {
                    if (StringUtils.isEmpty(nicIp.ipv4Netmask)) {
                        ...
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10231, "netmask error, expect: %s, got: %s",
                    ...
                    if (StringUtils.isEmpty(nicIp.ipv4Gateway)) {
                        ...
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10232, "gateway error, expect: %s, got: %s",
            ...
            if (!StringUtils.isEmpty(nicIp.ipv6Address)) {
                ...
                if (ipRangeVO == null) {
                    if (StringUtils.isEmpty(nicIp.ipv6Prefix)) {
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10233, "ipv6 prefix length must be set"));
                    }
                } else {
                    if (StringUtils.isEmpty(nicIp.ipv6Prefix)) {
                        ...
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10234, "ipv6 prefix length error, expect: %s, got: %s",
                    ...
                    if (StringUtils.isEmpty(nicIp.ipv6Gateway)) {
                        ...
                        throw new ApiMessageInterceptionException(operr(ORG_ZSTACK_COMPUTE_VM_10235, "gateway error, expect: %s, got: %s",
