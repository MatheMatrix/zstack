...
public class AccountInterceptor implements GlobalApiMessageInterceptor {
    ...
    private void validate(APICheckPasswordMessage msg) {
        ...
        if (session == null) {
            throw new ApiMessageInterceptionException(err(ORG_ZSTACK_IDENTITY_10071, IdentityErrors.INVALID_SESSION,
                    "Session expired"));
        }
        ...
        boolean correct = Q.New(AccountVO.class).eq(AccountVO_.uuid, accountUuid)
                ...
                .isExists();
        if (!correct) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10072, "wrong password"));
        }
    }
}
