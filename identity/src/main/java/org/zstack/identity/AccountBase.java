...
public class AccountBase extends AbstractAccount {
    ...
    private void handle(APIUpdateUserGroupMsg msg) {
        ...
        if (!AccountConstant.INITIAL_SYSTEM_ADMIN_UUID.equals(msg.getAccountUuid()) &&
            ...
            throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10006, "the user group[uuid:%s] does not belong to the account[uuid:%s]", group.getUuid(), msg.getAccountUuid()));
        }
    ...
    private void handle(APIShareResourceMsg msg) {
        ...
        for (String ruuid : msg.getResourceUuids()) {
            if (!uuidType.containsKey(ruuid)) {
                throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10007, "the account[uuid: %s] doesn't have a resource[uuid: %s]", self.getUuid(), ruuid));
            }
        }
    ...
    private void handle(APIUpdateUserMsg msg) {
        ...
        if (!AccountConstant.INITIAL_SYSTEM_ADMIN_UUID.equals(msg.getAccountUuid()) && !user.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10008, "the user[uuid:%s] does not belong to the" +
                    " account[uuid:%s]", user.getUuid(), msg.getAccountUuid()));
        }

        if (msg.getOldPassword() != null && !msg.getOldPassword().equals(user.getPassword())){
            throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10009, "old password is not equal to the original password, cannot update the password of user[uuid:%s]", user.getUuid()));
        }
