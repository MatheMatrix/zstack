...
public class AccountManagerImpl extends AbstractService implements AccountManager, SoftDeleteEntityExtensionPoint,
    ...
    public AccountResourceRefInventory changeResourceOwner(String resourceUuid, String newOwnerUuid) {
        ...
        if (refs.isEmpty()) {
            throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10020, "cannot find the resource[uuid:%s]; wrong resourceUuid or the resource is admin resource",
                            resourceUuid));
        }
    ...
    private void passThrough(AccountMessage msg) {
        ...
        if (vo == null) {
            bus.replyErrorByMessageType((Message) msg, err(ORG_ZSTACK_IDENTITY_10021, SysErrors.RESOURCE_NOT_FOUND, "unable to find account[uuid=%s]", msg.getAccountUuid()));
            return;
        }
        ...
        chain.then(new NoRollbackFlow() {
            ...
            public void run(FlowTrigger trigger, Map data) {
                ...
                if (resourceTargetOwnerAccountUuid.equals(resourceOriginalOwnerAccountUuid)) {
                    trigger.fail(err(ORG_ZSTACK_IDENTITY_10022, IdentityErrors.QUOTA_INVALID_OP,
    ...
    private void handle(APICheckApiPermissionMsg msg) {
        if (msg.getUserUuid() != null) {
            ...
            if (!isAdmin && !isMine) {
                ...
                ORG_ZSTACK_IDENTITY_10023,         "the user specified by the userUuid[%s] does not belong to the current account, and the" +
        ...
        for (String apiName : msg.getApiNames()) {
            try {
                ...
                throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10024, "%s is not an API", apiName));
            } catch (Exception e) {
    ...
    private void handle(APILogInByUserMsg msg) {
        ...
        if (msg.getAccountUuid() != null) {
            ...
            if (accountUuid == null) {
                reply.setError(err(ORG_ZSTACK_IDENTITY_10025, IdentityErrors.AUTHENTICATION_ERROR, "wrong account or username or password"));
                bus.reply(msg, reply);
                return;
            }
        }
        ...
        if (user == null) {
            reply.setError(err(ORG_ZSTACK_IDENTITY_10026, IdentityErrors.AUTHENTICATION_ERROR,
    ...
    public List<String> getResourceUuidsCanAccessByAccount(String accountUuid, Class resourceType) {
        ...
        if (types.isEmpty()) {
            throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10027, "cannot find the account[uuid:%s]", accountUuid));
        }
    ...
    class Auth {
        ...
        private void accountFieldCheck() throws IllegalAccessException {
            ...
            for (Tuple t : ts) {
                ...
                if (!session.getAccountUuid().equals(resourceOwnerAccountUuid)) {
                    throw new ApiMessageInterceptionException(err(ORG_ZSTACK_IDENTITY_10028, IdentityErrors.PERMISSION_DENIED,
        ...
        private void useDecision(Decision d, boolean userPolicy) {
            ...
            if (d.effect == PolicyStatementEffect.Allow) {
                ...
                throw new ApiMessageInterceptionException(err(ORG_ZSTACK_IDENTITY_10029, IdentityErrors.PERMISSION_DENIED,
        ...
        private void sessionCheck() {
            ...
            if (curr.after(session.getExpiredDate())) {
                ...
                throw new ApiMessageInterceptionException(err(ORG_ZSTACK_IDENTITY_10037, IdentityErrors.INVALID_SESSION, "Session expired"));
            }
    ...
    private void validate(APILogInByUserMsg msg) {
        if (msg.getAccountName() == null && msg.getAccountUuid() == null) {
            ...
            ORG_ZSTACK_IDENTITY_10038,         "accountName and accountUuid cannot both be null, you must specify at least one"
            ));
        }
    }
    ...
    private void validate(APICreateUserGroupMsg msg) {
        ...
        if (q.isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10039, "unable to create a group. A group called %s is already under the account[uuid:%s]",
    ...
    private void validate(APICreateUserMsg msg) {
        ...
        if (q.isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10040, "unable to create a user. A user called %s is already under the account[uuid:%s]",
                            msg.getName(), msg.getAccountUuid()));
        }
    }
    ...
    private void validate(APICreateAccountMsg msg) {
        ...
        if (q.isExists()) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10041, "unable to create an account. An account already called %s", msg.getName()));
        }
    }
    ...
    private void validate(APIDeleteAccountMsg msg) {
        if (new QuotaUtil().isAdminAccount(msg.getUuid())) {
            if (msg.getAccountUuid().equals(msg.getSession().getAccountUuid())) {
                throw new ApiMessageInterceptionException(argerr(
                ORG_ZSTACK_IDENTITY_10042,         "account cannot delete itself"
                ));
            }

            if (msg.getAccountUuid().equals(AccountConstant.INITIAL_SYSTEM_ADMIN_UUID)) {
                throw new ApiMessageInterceptionException(argerr(
                ORG_ZSTACK_IDENTITY_10043,         "cannot delete builtin admin account."
                ));
            }
        }
        if(!new QuotaUtil().isAdminAccount(msg.getSession().getAccountUuid())){
            throw new ApiMessageInterceptionException(argerr(
            ORG_ZSTACK_IDENTITY_10044,         "Only admin can delete account."
            ));
        }
    }
    ...
    private void validate(APIUpdateUserMsg msg) {
        if (msg.getUuid() == null && msg.getSession().isAccountSession()) {
            throw new ApiMessageInterceptionException(argerr(
            ORG_ZSTACK_IDENTITY_10045,         "the current session is an account session. You need to specify the field 'uuid' of the user" +
                            " you want to update"
            ));
        }
        ...
        if (msg.getUuid() != null && !msg.getSession().getUserUuid().equals(msg.getUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10046, "your are login as a user, you cannot another user[uuid:%s]", msg.getUuid()));
        }
    ...
    private void validate(APIRevokeResourceSharingMsg msg) {
        if (!msg.isAll() && (msg.getAccountUuids() == null || msg.getAccountUuids().isEmpty())) {
            throw new ApiMessageInterceptionException(argerr(
            ORG_ZSTACK_IDENTITY_10047,         "all is set to false, accountUuids cannot be null or empty"
            ));
        }
    }
    ...
    private void validate(APIShareResourceMsg msg) {
        if (!msg.isToPublic() && (msg.getAccountUuids() == null || msg.getAccountUuids().isEmpty())) {
            throw new ApiMessageInterceptionException(argerr(
            ORG_ZSTACK_IDENTITY_10048,         "toPublic is set to false, accountUuids cannot be null or empty"
            ));
        }
    }
    ...
    private void validate(APIDetachPolicyFromUserMsg msg) {
        ...
        if (!policy.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10049, "policy[name: %s, uuid: %s] doesn't belong to the account[uuid: %s]",
                            policy.getName(), policy.getUuid(), msg.getSession().getAccountUuid()));
        }
        if (!user.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10050, "user[name: %s, uuid: %s] doesn't belong to the account[uuid: %s]",
    ...
    private void validate(APIDetachPolicyFromUserGroupMsg msg) {
        ...
        if (!group.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10052, "group[name: %s, uuid: %s] doesn't belong to the account[uuid: %s]",
    ...
    private void validate(APIAttachPolicyToUserMsg msg) {
        ...
        if (!user.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10054, "user[name: %s, uuid: %s] doesn't belong to the account[uuid: %s]",
                            user.getName(), user.getUuid(), msg.getSession().getAccountUuid()));
        }
    }
    ...
    private void validate(APIAttachPolicyToUserGroupMsg msg) {
        ...
        if (!group.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10056, "group[name: %s, uuid: %s] doesn't belong to the account[uuid: %s]",
                            group.getName(), group.getUuid(), msg.getSession().getAccountUuid()));
        }
    }
    ...
    private void validate(APIAddUserToGroupMsg msg) {
        ...
        UserGroupVO group = dbf.findByUuid(msg.getGroupUuid(), UserGroupVO.class);
        if (!user.getAccountUuid().equals(msg.getAccountUuid())) {
            throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10057, "user[name: %s, uuid: %s] doesn't belong to the account[uuid: %s]",
                            user.getName(), user.getUuid(), msg.getSession().getAccountUuid()));
        }
    ...
    private void validate(APICreatePolicyMsg msg) {
        ...
        for (PolicyStatement s : msg.getStatements()) {
            if (s.getEffect() == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10059, "a statement must have effect field. Invalid statement[%s]", JSONObjectUtil.toJsonString(s)));
            }
            if (s.getActions() == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10060, "a statement must have action field. Invalid statement[%s]", JSONObjectUtil.toJsonString(s)));
            }
            if (s.getActions().isEmpty()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10061, "a statement must have a non-empty action field. Invalid statement[%s]",
            ...
            if (s.getActions() != null) {
                s.getActions().forEach(as -> {
                    if (PolicyUtils.isAdminOnlyAction(as)) {
                        throw new OperationFailureException(err(ORG_ZSTACK_IDENTITY_10062, IdentityErrors.PERMISSION_DENIED, "normal accounts can't create admin-only action polices[%s]", as));
                    }
                });
            }
        }
    ...
    private void validate(APIUpdateAccountMsg msg) {
        ...
        if (msg.getName() != null) {
            ...
            q.add(AccountVO_.name, Op.EQ, msg.getName());
            if (q.isExists()) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_IDENTITY_10063, "unable to update name. An account already called %s", msg.getName()));
            }
        }
        ...
        if (msg.getOldPassword() != null && !msg.getOldPassword().equals(account.getPassword())) {
            throw new OperationFailureException(operr(ORG_ZSTACK_IDENTITY_10064, "old password is not equal to the original password, cannot update the password of account[uuid: %s]", msg.getUuid()));
        }
        ...
        if (a.getType() == AccountType.SystemAdmin) {
            if (msg.getName() != null && (msg.getUuid() == null || msg.getUuid().equals(AccountConstant.INITIAL_SYSTEM_ADMIN_UUID))) {
                throw new OperationFailureException(operr(
                ORG_ZSTACK_IDENTITY_10065,         "the name of admin account cannot be updated"
                ));
            }

            if (msg.getPassword() != null && (!AccountConstant.isAdminPermission(msg.getSession()))) {
                throw new OperationFailureException(operr(ORG_ZSTACK_IDENTITY_10066, "only admin account can update it's password"));
            }
        ...
        if (!account.getUuid().equals(a.getUuid())) {
            throw new OperationFailureException(operr(ORG_ZSTACK_IDENTITY_10067, "account[uuid: %s, name: %s] is a normal account, it cannot reset the password of another account[uuid: %s]",
    ...
    private void validate(APIUpdateQuotaMsg msg) {
        QuotaVO quota = Q.New(QuotaVO.class)
                ...
                .find();
        if (quota == null) {
            throw new OperationFailureException(argerr(ORG_ZSTACK_IDENTITY_10068, "cannot find Quota[name: %s] for the account[uuid: %s]", msg.getName(), msg.getIdentityUuid()));
        }
        ...
        if (checkers.isEmpty()) {
            throw new ApiMessageInterceptionException(
                    argerr(ORG_ZSTACK_IDENTITY_10069, "can not find quota update checker for quota[uuid:%s, type:%s]", quota.getIdentityUuid(), quota.getIdentityType()));
        }
        ...
        for (QuotaUpdateChecker checker : checkers) {
            ...
            if (errorCode != null) {
                throw new ApiMessageInterceptionException(
                        operr(ORG_ZSTACK_IDENTITY_10070, errorCode, "cannot update Quota[name: %s] for the account[uuid: %s]", msg.getName(), msg.getIdentityUuid()));
            }
        }
