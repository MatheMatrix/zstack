    ...
    private void validate(IpRangeInventory ipr) {
        ...
        if (ipr.getIpRangeType() == IpRangeType.Normal) {
            ...
            if (l3IpRanges.isEmpty()) {
                ...
                if (gatewayUsed) {
                    throw new ApiMessageInterceptionException(argerr(
                            "gateway address[%s] is already used by a VM NIC, cannot add IP range with this gateway",
                            ipr.getGateway()));
                }
                ...
                if (networkAddressUsed) {
                    throw new ApiMessageInterceptionException(argerr(
                            "network address[%s] is already used by a VM NIC, cannot add IP range containing this address",
                            networkAddress));
                }
                ...
                if (broadcastAddressUsed) {
                    throw new ApiMessageInterceptionException(argerr(
                            "broadcast address[%s] is already used by a VM NIC, cannot add IP range containing this address",
                            broadcastAddress));
                }
            }
