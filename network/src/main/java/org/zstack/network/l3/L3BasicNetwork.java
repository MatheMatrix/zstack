            ...
            public void setup() {
                flow(new Flow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        vo = dbf.persist(vo);
                        data.put(L3NetworkConstant.L3Param.L3_HOSTROUTE_VO, vo);
                        data.put(L3NetworkConstant.L3Param.L3_HOSTROUTE_SUCCESS, true);
                        trigger.next();
                    }

                    @Override
                    public void rollback(FlowRollback trigger, Map data) {
                        boolean flag = (boolean) data.get(L3NetworkConstant.L3Param.L3_HOSTROUTE_SUCCESS);
                        if (flag) {
                            L3NetworkHostRouteVO vo = (L3NetworkHostRouteVO) data.get(L3NetworkConstant.L3Param.L3_HOSTROUTE_VO);
                            dbf.remove(vo);
                        }
                        trigger.rollback();
                    }
