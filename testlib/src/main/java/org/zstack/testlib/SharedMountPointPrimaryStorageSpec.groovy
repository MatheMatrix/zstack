...
class SharedMountPointPrimaryStorageSpec extends PrimaryStorageSpec {
    ...
    class Simulators implements Simulator {
        ...
        void registerSimulators(EnvSpec xspec) {
            ...
            simulator(KvmBackend.GET_BACKING_FILE_PATH) { HttpEntity<String> e, EnvSpec spec ->
                return new KvmBackend.GetBackingFileRsp()
            }

            VFS.vfsHook(KvmBackend.GET_BACKING_FILE_PATH, xspec) { KvmBackend.GetBackingFileRsp rsp, HttpEntity<String> e, EnvSpec spec ->
                def cmd = JSONObjectUtil.toObject(e.body, KvmBackend.GetBackingFileCmd.class)
