...
public class SshYumRepoChecker implements AnsibleChecker, ManagementServerIpAwareChecker {
    @Autowired
    ...
    public boolean needDeploy() {
        ...
        try {
            ...
            if (StringUtils.isBlank(managementServerIp)) {
                logger.warn(String.format("management server IP is not set for repo checker on target[%s]", targetIp));
                return true;
            }
            if (!repoOutputUsesEndpoint(ret.getStdout(), managementServerIp, restf.getPort())) {
                logger.debug(String.format("yum repo endpoint on target[%s] does not match management node[%s:%s]",
                        targetIp, managementServerIp, restf.getPort()));
                return true;
            }
        } catch (Exception e) {
