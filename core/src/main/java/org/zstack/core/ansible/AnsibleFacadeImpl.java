    ...
    void init() {
        ...
        try {
            ...
            if (!invFile.exists() && !invFile.createNewFile()) {
                throw new OperationFailureException(operr(ORG_ZSTACK_CORE_ANSIBLE_10004, "fail to create new File[%s]", invFile));
            }
            ...
            private void run(Completion completion) {
                ...
                try {
                    ...
                    if (output.contains("skipping: no hosts matched")) {
                        throw new OperationFailureException(operr(ORG_ZSTACK_CORE_ANSIBLE_10005, output));
                    }

                } catch (ShellException se) {
                    String errMsg = hidePassword(se.getMessage());
                    logger.warn(errMsg, se);
                    throw new OperationFailureException(operr(ORG_ZSTACK_CORE_ANSIBLE_10006, errMsg));
                }
