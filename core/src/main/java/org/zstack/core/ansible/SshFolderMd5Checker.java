...
public class SshFolderMd5Checker implements AnsibleChecker {
    ...
    public boolean needDeploy() {
        ...
        if (!srcRes.isReturnCode(0)) {
            throw new OperationFailureException(operr(ORG_ZSTACK_CORE_ANSIBLE_10001, "cannot check md5sum of files in the folder[%s].\nstdout:%s\nstderr:%s", srcFolder,
        ...
        if (dstRes.getReturnCode() == 101) {
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_CORE_ANSIBLE_10002, "cannot check md5sum of files in the folder[%s] on the host[ip:%s].\nstdout:%s\nstderr:%s",
