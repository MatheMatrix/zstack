...
public abstract class ReturnValueCoalesceQueue<T, R, V> extends AbstractCoalesceQueue<T, R, V> {
    ...
    @Override
    protected final AbstractCompletion createBatchCompletion(String syncSignature, List<PendingRequest> requests) {
        return new ReturnValueCompletion<R>(null) {
            @Override
            public void success(R batchResult) {
                handleSuccess(syncSignature, requests, batchResult);
            }

            @Override
            public void fail(ErrorCode errorCode) {
                handleFailure(syncSignature, requests, errorCode);
            }
        };
    }
}
