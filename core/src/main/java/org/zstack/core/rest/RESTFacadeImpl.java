...
public class RESTFacadeImpl implements RESTFacade {
    ...
    public ResponseEntity<String> syncRawJson(String url, HttpEntity<String> req, HttpMethod method, TimeUnit unit, long timeout) {
        ...
        try {
            ...
            throw new OperationFailureException(operr(ORG_ZSTACK_CORE_REST_10006, "failed to %s to %s, status code: %s, response body: %s", method.toString().toLowerCase(), url, e.getStatusCode(), e.getResponseBodyAsString()));
        } catch (ResourceAccessException e) {
            throw new OperationFailureException(operr(ORG_ZSTACK_CORE_REST_10007, "failed to %s to %s, IO Error: %s", method.toString().toLowerCase(), url, e.getMessage()));
        }
        ...
        thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask(completion) {
            ...
            public boolean run() {
                try {
                    ...
                    if (now > expired) {
                        completion.fail(operr(ORG_ZSTACK_CORE_REST_10009, "unable to echo %s in %sms", url, finalTimeout));
                        return true;
                    }
                }
