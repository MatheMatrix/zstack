...
public class CloudBusImpl2 implements CloudBus, CloudBusIN, ManagementNodeChangeListener, CloudBusMXBean {
                            ...
                            try {
                                ...
                                if (msg instanceof NeedReplyMessage) {
                                    ...
                                    if (e == null) {
                                        ...
                                        reply.setError(err(ORG_ZSTACK_CORE_CLOUDBUS_10015, SysErrors.UNDELIVERABLE_ERROR,
            ...
            throwableSafe(new Runnable() {
                ...
                private void handleNoRouteLetter(Message msg) {
                    ...
                    if (msg instanceof APIIsReadyToGoMsg) {
                        ...
                        reply.setError(err(ORG_ZSTACK_CORE_CLOUDBUS_10016, SysErrors.NOT_READY_ERROR, "management node[uuid:%s] is no ready", Platform.getManagementServerId()));
                        reply(msg, reply);
                        return;
                    }
                    ...
                    if (msg instanceof MessageReply) {
                        replyErrorByMessageType(msg, err(ORG_ZSTACK_CORE_CLOUDBUS_10017, SysErrors.NO_ROUTE_ERROR, "No route found for the reply[%s], the service[id:%s] waiting for this reply may have been quit. %s",
                                msg.getClass().getName(), msg.getServiceId(), wire.dumpMessage(msg)));
                    } else {
                        replyErrorByMessageType(msg, err(ORG_ZSTACK_CORE_CLOUDBUS_10018, SysErrors.NO_ROUTE_ERROR, "No route found for the message[%s], the service[id:%s] may not be running. Checking Spring xml to make sure you have loaded it. Message dump:\n %s",
                                msg.getClass().getName(), msg.getServiceId(), wire.dumpMessage(msg)));
                    }
                }
                ...
                private void handleDeadLetter(Message msg) {
                    if (msg instanceof MessageReply || msg instanceof APIEvent) {
                        ...
                        ErrorCode err = err(ORG_ZSTACK_CORE_CLOUDBUS_10019, SysErrors.NO_ROUTE_ERROR, "the message becomes a dead letter; the possible reason is the service[%s] it sends to has been dead", msg.getServiceId());
    ...
    private class Wire implements GsonTypeCoder<Message> {
        ...
        private void tryBestToReplyError(byte[] binary, String errMsg) {
            ...
            try {
                ...
                replyErrorByMessageType(msgInstance, argerr(ORG_ZSTACK_CORE_CLOUDBUS_10020, "message is not in corrected JSON mediaType, %s", errMsg));
            } catch (Exception e) {
            ...
            if (metaData instanceof LockMessageMetaData) {
                ...
                MessageTrackerEnvelope e = new MessageTrackerEnvelope() {
                    ...
                    private void replyError() {
                        ...
                        ErrorCode err = err(ORG_ZSTACK_CORE_CLOUDBUS_10021, SysErrors.MANAGEMENT_NODE_UNAVAILABLE_ERROR,
    ...
    private MessageReply createTimeoutReply(NeedReplyMessage m) {
        ...
        r.setError(touterr(ORG_ZSTACK_CORE_CLOUDBUS_10022, m.toErrorString()));
        return r;
    }
                                ...
                                public Void call() throws Exception {
                                    ...
                                    try {
                                        ...
                                        if (t instanceof OperationFailureException) {
                                            ...
                                            replyErrorByMessageType(msg, inerr(ORG_ZSTACK_CORE_CLOUDBUS_10023, t.getMessage()));
                                        }
                                    }
    ...
    public void dealWithUnknownMessage(Message msg) {
        ...
        if (msg instanceof APISyncCallMessage) {
            ...
            reply.setError(err(ORG_ZSTACK_CORE_CLOUDBUS_10024, SysErrors.UNKNOWN_MESSAGE_ERROR, details));
            reply.setSuccess(false);
            this.reply(msg, reply);
        } else if (msg instanceof APIMessage) {
            APIEvent evt = new APIEvent(msg.getId());
            evt.setError(err(ORG_ZSTACK_CORE_CLOUDBUS_10025, SysErrors.UNKNOWN_MESSAGE_ERROR, details));
            this.publish(evt);
        } else if (msg instanceof NeedReplyMessage) {
            MessageReply reply = new MessageReply();
            reply.setError(err(ORG_ZSTACK_CORE_CLOUDBUS_10026, SysErrors.UNKNOWN_MESSAGE_ERROR, details));
            reply.setSuccess(false);
            this.reply(msg, reply);
        }
    ...
    private void replyErrorIfMessageNeedReply(Message msg, String errStr) {
        if (msg instanceof NeedReplyMessage) {
            ErrorCode err = inerr(ORG_ZSTACK_CORE_CLOUDBUS_10027, errStr);
    ...
    private void replyErrorToApiMessage(APIMessage msg, String err) {
        replyErrorToApiMessage(msg, inerr(ORG_ZSTACK_CORE_CLOUDBUS_10028, err));
    }
