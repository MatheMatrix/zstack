        ...
        return new ASTVisitor<String, ASTNode.QueryTarget>() {
            ...
            public String visit(ASTNode.QueryTarget queryTarget) {
                if (StringUtils.isBlank(queryTarget.getEntity())) {
                    throw new OperationFailureException(Platform.operr(ORG_ZSTACK_ZQL_AST_VISITORS_10000, "entity is empty, cannot get columnName"));
                }
                if (CollectionUtils.isEmpty(queryTarget.getFields())) {
                    throw new OperationFailureException(Platform.operr(ORG_ZSTACK_ZQL_AST_VISITORS_10001, "fieldList is empty, cannot get columnName"));
                }
