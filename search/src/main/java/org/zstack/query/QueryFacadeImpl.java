...
public class QueryFacadeImpl extends AbstractService implements QueryFacade, GlobalApiMessageInterceptor, MarshalReplyMessageExtensionPoint {
        ...
        try {
            ...
            if (setter == null) {
                ...
                if (setter == null) {
                    ...
                    ORG_ZSTACK_QUERY_10000,         "query reply[%s] has no method setInventories()", replyClass.getName()
                    ));
                }
            ...
            throw new OperationFailureException(inerr(ORG_ZSTACK_QUERY_10001, e.getMessage()));
        }
    }
    ...
    private void handle(APIQueryMessage msg) {
        ...
        if (at == null) {
            ...
            if (at == null) {
                throw new OperationFailureException(inerr(
                ORG_ZSTACK_QUERY_10002,         "message[%s] is not annotated by @AutoQuery", msg.getClass()
                ));
            }
    ...
    private QueryBelongFilter validateFilterNameAndGetExp(String filterName) {
        if (filterName.split(":").length < 2) {
            throw new OperationFailureException(argerr(ORG_ZSTACK_QUERY_10003, "filterName must be formatted as [filterType:condition(s)]"));
        }
    ...
    public APIMessage intercept(APIMessage msg) throws ApiMessageInterceptionException {
        ...
        for (QueryCondition cond : qmsg.getConditions()) {
            try {
                ...
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_QUERY_10004, e.getMessage()));
            }

            if (!QueryOp.NOT_NULL.equals(cond.getOp()) && !QueryOp.IS_NULL.equals(cond.getOp()) && cond.getValue() == null) {
                throw new ApiMessageInterceptionException(argerr(ORG_ZSTACK_QUERY_10005, "'value' of query condition %s cannot be null",
