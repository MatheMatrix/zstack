        ...
        String toJpql() {
            ...
            if (!skipInventoryCheck) {
                if (inventoryField == null || inventoryField.isAnnotationPresent(APINoSee.class)) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_QUERY_10006, "condition name[%s] is invalid, no such field on inventory class[%s]",
                                    attr, inventoryClass.getName()));
                }

                if (inventoryField.isAnnotationPresent(Unqueryable.class)) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_QUERY_10007, "condition name[%s] is invalid, field[%s] of inventory[%s] is annotated as @Unqueryable field",
            ...
            if (at == null) {
                ...
                if (metaField == null) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_QUERY_10008, "entity meta class[%s] has no field[%s]",
        ...
        private void validateFields() {
            ...
            for (String f : msg.getFields()) {
                if (!info.premitiveFieldNames.contains(f)) {
                    throw new OperationFailureException(argerr(ORG_ZSTACK_QUERY_10009, "field[%s] is not a primitive of the inventory %s; you cannot specify it in the parameter 'fields';" +
