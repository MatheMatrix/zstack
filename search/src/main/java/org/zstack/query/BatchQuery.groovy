...
class BatchQuery {
    ...
    private Map syncApiCall(String apiname, String jstr) {
        ...
        if (msgClz == null) {
            throw new OperationFailureException(Platform.argerr(ORG_ZSTACK_QUERY_10006, "no query API found for %s", apiname))
        }
    ...
    private Map doQuery(String qstr) {
        ...
        if (words.isEmpty()) {
            throw new OperationFailureException(Platform.argerr(ORG_ZSTACK_QUERY_10007, "invalid query string: %s", qstr))
        }

        String api = words[0].toLowerCase()
        Class msgClz = queryMessageClass[api]
        if (msgClz == null) {
            throw new OperationFailureException(Platform.argerr(ORG_ZSTACK_QUERY_10008, "no query API found for %s", words[0]))
        }
        ...
        if (words.size() > 1) {
            words[1..words.size() - 1].each { String word ->
                if (word.startsWith("fields=")) {
                    ...
                    if (OP == null) {
                        throw new OperationFailureException(Platform.argerr(ORG_ZSTACK_QUERY_10009, "invalid query string[%s], word[%s] doesn't have a valid operator", qstr, word))
                    }
                    ...
                    if (OP == QueryOp.IS_NULL.toString() || OP == QueryOp.NOT_NULL.toString()) {
                        ...
                        if (ks.size() != 2) {
                            throw new OperationFailureException(Platform.argerr(ORG_ZSTACK_QUERY_10010, "invalid query string[%s], word[%s] doesn't has key-value pair", qstr, word))
                        }
                        cond.name = ks[0]
                        cond.op = OP
                        cond.value = ks[1]
    ...
    Map<String, Object> query(APIBatchQueryMsg msg) {
        try {
            ...
            try {
                ...
                sandbox.unregister()
                throw new OperationFailureException(Platform.operr(ORG_ZSTACK_QUERY_10011, "${errorLine(msg.script, t)}"))
            } finally {
                sandbox.unregister()
