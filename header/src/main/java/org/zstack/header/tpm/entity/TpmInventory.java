...
public class TpmInventory implements Serializable {
    ...
    public static TpmInventory __example__() {
        ...
        tpm.setUuid(DocUtils.uuidForAPIDoc());
        tpm.setVmInstanceUuid(DocUtils.uuidForAPIDoc());
        tpm.setName("TPM-for-VM-" + tpm.getVmInstanceUuid());
        tpm.setCreateDate(new Timestamp(DocUtils.date));
        tpm.setLastOpDate(new Timestamp(DocUtils.date));
        return tpm;
    }
}
