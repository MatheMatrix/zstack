...
public class VolumeTree {
    ...
    public class VolumeSnapshotDeletionContext {
        VolumeSnapshotVO targetSnapshot;
        ...
        List<VolumeTree.VolumeSnapshotLeaf> snapshotChainWhichStatusIsDeletedAndWithSingleChildren = new ArrayList<>();
        List<VolumeTree.VolumeSnapshotLeaf> snapshotChainWhichStatusIsDeletedAndWithSingleParent = new ArrayList<>();
        ...
        public void process() {
            ...
            online = (vmInstanceState == VmInstanceState.Running || vmInstanceState == VmInstanceState.Paused)
                    && aliveChainUuids.contains(dstSnapshotLeaf.getUuid());
        ...
        private void handleLastReadySnapshot() {
            ...
            if (isCurrent) {
                resolveDirection(getSnapshotLeaf(volume.getUuid()));

                if (direction.equals(DeleteVolumeSnapshotDirection.Commit.toString())) {
        ...
        private void handleSnapshotWithParent() {
            ...
            if (Objects.equals(direction, DeleteVolumeSnapshotDirection.Commit.toString())) {
                srcSnapshotLeaf = childrenLeaf;
        ...
        private VolumeTree.VolumeSnapshotLeaf getChildrenLeafLeaf(VolumeTree.VolumeSnapshotLeaf leaf) {
            fillSnapshotChainWhichStatusIsDeletedAndWithSingleChildren(leaf);
        ...
        private boolean shouldUseCommit(VolumeTree.VolumeSnapshotLeaf leaf) {
            boolean notVolume = !Objects.equals(leaf.getUuid(), volume.getUuid());
            boolean inAliveChain = aliveChainUuids.contains(leaf.getUuid());
        ...
        private void resolveDirection(VolumeTree.VolumeSnapshotLeaf leaf) {
            if (Objects.equals(direction, DeleteVolumeSnapshotDirection.Pull.toString())) {
                if (shouldUseCommit(leaf)) {
                    errorString = String.format("The snapshot[uuid:%s] will be deleted by block %s, but the direction is %s, " +
                            "change the direction to pull.", targetSnapshotLeaf.getUuid(), direction, DeleteVolumeSnapshotDirection.Pull.toString());
                    return;
                }
            }
            ...
            if (Objects.equals(direction, DeleteVolumeSnapshotScope.Auto.toString())) {
                if (shouldUseCommit(leaf)) {
                    direction = DeleteVolumeSnapshotDirection.Commit.toString();
                    return;
                }
                direction = DeleteVolumeSnapshotDirection.Pull.toString();
            }
