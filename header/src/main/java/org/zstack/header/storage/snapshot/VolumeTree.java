...
public class VolumeTree {
    ...
    public VolumeSnapshotLeaf findSnapshot(String snapshotUuid) {
    ...
    private List<VolumeSnapshotInventory> getAliveChainSnapshotInventory() {
        ...
        //Collections.reverse(latestLeaf.getAncestors());
    ...
    // 获取当前快照节点的所有祖先节点以及当前节点。不包含快照节点子节点
    ...
    public List<String> getSnapshotChainSnapshotUuids(String snapshotUuid) {
        if (getSnapshotChainSnapshotInventory(snapshotUuid).isEmpty()) {
            return new ArrayList<>();
        }
        return getSnapshotChainSnapshotInventory(snapshotUuid).stream().map(VolumeSnapshotInventory::getUuid).collect(Collectors.toList());
    }

    public List<String> getSnapshotChainSnapshotInstallPaths(String snapshotUuid) {
        if (getSnapshotChainSnapshotInventory(snapshotUuid).isEmpty()) {
            return new ArrayList<>();
        }
        return getSnapshotChainSnapshotInventory(snapshotUuid).stream().map(VolumeSnapshotInventory::getPrimaryStorageInstallPath).collect(Collectors.toList());
    }
