...
public class VolumeTree {
    ...
    public class VolumeSnapshotDeletionContext {
        ...
        List<String> snapshotChainFromSrcToDst = new ArrayList<>();
        ...
        boolean deleteByBlockCommit = false;

        VolumeSnapshotDeletionContext(VolumeSnapshotVO targetSnapshot, VmInstanceState vmInstanceState, boolean isLatest) {
            this.targetSnapshot = targetSnapshot;
            this.vmInstanceState = vmInstanceState;
            this.isLatest = isLatest;
            this.targetSnapshotLeaf = getSnapshotLeaf(targetSnapshot.getUuid());
            this.parentSnapshotLeaf = targetSnapshotLeaf.getParent();
            ...
            List<VolumeSnapshotVO> allReadySnapshots = allSnapshots.stream()
                    .filter(snapshotVO -> snapshotVO.getStatus() == VolumeSnapshotStatus.Ready).collect(Collectors.toList());

            if (allReadySnapshots.size() == 1 && allReadySnapshots.get(0).getUuid().equals(targetSnapshot.getUuid())) {
                allSnapshots.forEach(vo -> snapshotsToDelete.add(VolumeSnapshotInventory.valueOf(vo)));
                if (isCurrent) {
                    // 当前快照树是volume正在所在快照树，使用pull方式将数据合并到volume。
                    // 当前快照树是volume正在所在快照树，使用push方式将数据合并到root。
                ...
                // 当前快照所在快照树仅有一个Ready的快照，直接删除快照树。
                logger.debug(String.format("The snapshot tree[uuid:%s] only contains one ready snapshot[uuid:%s], delete the snapshot tree directly.",
                        targetSnapshot.getTreeUuid(), targetSnapshot.getUuid()));
                return;
            }
            ...
            if (isCurrent && Objects.equals(srcSnapshotLeaf.getUuid(), volume.getUuid()) && dstSnapshotLeaf != null) {
                newLatestSnapshotUuid = dstSnapshotLeaf.getUuid();
            }
        ...
        private void handleSnapshotWithoutParent() {
            ...
            shouldProceedWithBlockCommitDeletion();
            logger.debug(String.format("The snapshot[uuid:%s] will be deleted by %s: %s.",
                    targetSnapshotLeaf.getUuid(), deleteByBlockCommit ? "block commit" : "block pull", srcSnapshotLeaf.getUuid()));

            if (deleteByBlockCommit) {
        ...
        private void handleSnapshotWithParent() {
            ...
            shouldProceedWithBlockCommitDeletion();
            if (deleteByBlockCommit) {
                newParentUuid = dstSnapshotLeaf.getParent() != null ? dstSnapshotLeaf.getParent().getUuid() : null;
                return;
            }
            dstSnapshotLeaf = dstSnapshotLeaf.getParent();
            newParentUuid = dstSnapshotLeaf != null ? dstSnapshotLeaf.getUuid() : null;
        }
        ...
        private void setSnapshotChainFromSrcToDst() {
            ...
            if (dstSnapshotLeaf == null) {
                srcSnapshotLeaf.getAncestors().forEach(ancestor -> snapshotChainFromSrcToDst.add(ancestor.getPrimaryStorageInstallPath()));
                return;
            }
            ...
            for (VolumeSnapshotInventory ancestor : srcSnapshotLeaf.getAncestors()) {
                if (Objects.equals(ancestor.getUuid(), dstSnapshotLeaf.getUuid())) {
                    snapshotChainFromSrcToDst.add(ancestor.getPrimaryStorageInstallPath());
                    break;
                }
                snapshotChainFromSrcToDst.add(ancestor.getPrimaryStorageInstallPath());
            }
        ...
        private void shouldProceedWithBlockCommitDeletion() {
            if (this.isLatest) {
                deleteByBlockCommit = true;
            }

            if (!isCurrent) {
                deleteByBlockCommit = false;
                return;
            }
            ...
            if (Objects.equals(srcSnapshotLeaf.getUuid(), volume.getUuid())) {
                deleteByBlockCommit = false;
                return;
            }

            List<String> aliveChainSnapshotUuids = aliveChainSnapshots.stream()
                    .map(VolumeSnapshotInventory::getUuid).collect(Collectors.toList());
            if (!aliveChainSnapshotUuids.contains(srcSnapshotLeaf.getInventory().getUuid())) {
                deleteByBlockCommit = false;
                return;
            }

            if (vmInstanceState != null && (vmInstanceState == VmInstanceState.Running || vmInstanceState == VmInstanceState.Paused)) {
                deleteByBlockCommit = true;
            }
        }
    ...
    public VolumeSnapshotDeletionContext createDeletionContext(VolumeSnapshotVO snapshotVO, VmInstanceState vmInstanceState, boolean isLatest) {
        return new VolumeSnapshotDeletionContext(snapshotVO, vmInstanceState, isLatest);
    }
