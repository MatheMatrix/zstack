...
public class VolumeTree {
    ...
    public class VolumeSnapshotDeletionContext {
        ...
        VolumeSnapshotDeletionContext(VolumeSnapshotVO targetSnapshot, VmInstanceState vmInstanceState, boolean isLatest) {
            ...
            if (!isLatest && allReadySnapshots.size() == 1 && allReadySnapshots.get(0).getUuid().equals(targetSnapshot.getUuid())) {
        ...
        private void shouldProceedWithBlockCommitDeletion() {
            ...
            if (isCurrent && srcSnapshotLeaf != null && !Objects.equals(srcSnapshotLeaf.getUuid(), volume.getUuid())
                    && aliveChainSnapshotUuids.contains(srcSnapshotLeaf.getInventory().getUuid())
                    && vmInstanceState == VmInstanceState.Running || vmInstanceState == VmInstanceState.Paused) {
                deleteByBlockCommit = true;
            }
        }
