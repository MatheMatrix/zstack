...
public class VolumeTree {
    ...
    public class VolumeSnapshotDeletionContext {
        ...
        VolumeSnapshotDeletionContext(VolumeSnapshotInventory targetSnapshot, VmInstanceState vmState, String direction) {
            ...
            this.online = (vmState == VmInstanceState.Running || vmState == VmInstanceState.Paused)
                    && aliveChainUuids.contains(targetSnapshot.getUuid());

            direction = resolveDirection(targetSnapshot.getUuid(), vmState, direction);
            if (errorString != null) {
                return;
            }
        ...
        private String resolveDirection(String targetSnapshotUuid, VmInstanceState vmState, String direction) {
            if (Objects.equals(direction, DeleteVolumeSnapshotDirection.Pull.toString())) {
                if (shouldUseCommit(targetSnapshotUuid, vmState)) {
            ...
            if (Objects.equals(direction, DeleteVolumeSnapshotScope.Auto.toString())) {
                direction = shouldUseCommit(targetSnapshotUuid, vmState) ?
                        DeleteVolumeSnapshotDirection.Commit.toString() : DeleteVolumeSnapshotDirection.Pull.toString();
            }
            return direction;
        }
