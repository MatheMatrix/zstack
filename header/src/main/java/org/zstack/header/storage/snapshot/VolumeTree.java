...
public class VolumeTree {
    ...
    private static final Logger log = LoggerFactory.getLogger(VolumeTree.class);
    ...
    public class VolumeSnapshotDeletionContext {
        ...
        private void handleCommitDirection(List<VolumeSnapshotLeaf> children) {
            children.forEach(leaf -> {
                if (!aliveChainSnapshotUuids.contains(leaf.getUuid())) {
                    ...
                    List<String> childrenInstallPath = leaf.getChildren().stream().map(child ->
                            child.getInventory().getPrimaryStorageInstallPath()).collect(Collectors.toList());
                    commitMsg.setSrcChildrenInstallPathInDb(childrenInstallPath);
                }
            });
        }
        ...
        private boolean shouldUseCommitStrategy() {
            return isCurrent && !targetSnapshot.isLatest() && online;
        }

        private String resolveDirection(String initialDirection) {
            if (Objects.equals(initialDirection, DeleteVolumeSnapshotDirection.Pull.toString())) {
                if (shouldUseCommitStrategy()) {
                    errorString = String.format("the snapshot[uuid:%s] will be deleted by block 'commit', but the direction is 'pull', " +
                            "change the direction to 'commit' or 'auto'.", targetSnapshot.getUuid());
                    return DeleteVolumeSnapshotDirection.Commit.toString();
                }
            }
