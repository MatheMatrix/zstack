...
public class VolumeTree {
    ...
    public class VolumeSnapshotDeletionContext {
        ...
        boolean online;
        String errorString;
        List<String> aliveChainUuids;
        CommitVolumeSnapshotOnPrimaryStorageMsg commitMsg = new CommitVolumeSnapshotOnPrimaryStorageMsg();
        List<PullVolumeSnapshotOnPrimaryStorageMsg> pullMsgs = new ArrayList<>();

        VolumeSnapshotDeletionContext(VolumeSnapshotInventory targetSnapshot, VmInstanceState vmInstanceState, String direction) {
            if (VolumeSnapshotConstant.STORAGE_SNAPSHOT_TYPE.toString().equals(targetSnapshot.getType())) {
                return;
            }

            this.aliveChainUuids = aliveChain.stream().map(VolumeSnapshotInventory::getUuid).collect(Collectors.toList());
            ...
            this.online = (vmInstanceState == VmInstanceState.Running || vmInstanceState == VmInstanceState.Paused)
                    && aliveChainUuids.contains(targetSnapshot.getUuid());

            direction = resolveDirection(targetSnapshot.getUuid(), vmInstanceState, direction);
            if (errorString != null) {
                return;
            }

            if (Objects.equals(direction, DeleteVolumeSnapshotDirection.Pull.toString())) {
                handleDirectionIsPull(targetSnapshotLeaf.getChildren());
                return;
            }
            handleDirectionIsCommit(targetSnapshotLeaf.getChildren());
        }

        private void handleDirectionIsPull(List<VolumeSnapshotLeaf> children) {
            children.forEach(leaf -> {
                if (aliveChainUuids.contains(leaf.getUuid())) {
                    ...
                    if (online) {
                        commitMsg.setOnline(true);
                    }
                    if (leaf.getChildren().size() > 1) {
                        commitMsg.setSrcChildrenInstallPathInDb(leaf.getChildren().stream()
                                .map(child -> child.getInventory().getPrimaryStorageInstallPath()).collect(Collectors.toList()));
                    }
                    return;
                }
                ...
                pullMsg.setDstSnapshot(leaf.getInventory());

                pullMsgs.add(pullMsg);
            });
        }

        private void handleDirectionIsCommit(List<VolumeSnapshotLeaf> children) {
            children.forEach(leaf -> {
                PullVolumeSnapshotOnPrimaryStorageMsg pullMsg = new PullVolumeSnapshotOnPrimaryStorageMsg();
                pullMsg.setVolume(VolumeInventory.valueOf(volume));
                pullMsg.setSrcSnapshot(targetSnapshot);
                pullMsg.setDstSnapshot(leaf.getInventory());
                if (online && aliveChainUuids.contains(leaf.getUuid())) {
                    pullMsg.setOnline(true);
                }
                pullMsgs.add(pullMsg);
            });
        }

        private boolean shouldUseCommit(String targetSnapshotUuid, VmInstanceState vmInstanceState) {
            boolean notVolume = !Objects.equals(targetSnapshotUuid, volume.getUuid());
            boolean inAliveChain = aliveChainUuids.contains(targetSnapshotUuid);
        ...
        private String resolveDirection(String targetSnapshotUuid, VmInstanceState vmInstanceState, String direction) {
            if (Objects.equals(direction, DeleteVolumeSnapshotDirection.Pull.toString())) {
                if (shouldUseCommit(targetSnapshotUuid, vmInstanceState)) {
                    errorString = String.format("the snapshot[uuid:%s] will be deleted by block %s, but the direction is %s, " +
                            "change the direction to pull.", targetSnapshotUuid, direction, DeleteVolumeSnapshotDirection.Pull);
                    return direction;
                }
            }

            if (Objects.equals(direction, DeleteVolumeSnapshotScope.Auto.toString())) {
                direction = shouldUseCommit(targetSnapshotUuid, vmInstanceState) ? DeleteVolumeSnapshotDirection.Commit.toString() : DeleteVolumeSnapshotDirection.Pull.toString();
            }
            return direction;
        }
    ...
    public VolumeSnapshotDeletionContext createDeletionContext(VolumeSnapshotInventory targetSnapshot, VmInstanceState vmInstanceState, String direction) {
