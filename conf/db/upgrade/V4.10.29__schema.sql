...
-- Poller CAS claim query optimization: WHERE managementNodeUuid IS NULL AND lastClaimTime ... AND nextRetryTime <= NOW()
CREATE INDEX `idxVmMetadataDirtyUnclaimed` ON `VmMetadataDirtyVO` (`managementNodeUuid`, `lastClaimTime`, `nextRetryTime`);

-- Path fingerprint for lightweight drift detection (§8.2.3)
CREATE TABLE IF NOT EXISTS `zstack`.`VmMetadataPathFingerprintVO` (
    `vmInstanceUuid` VARCHAR(32) NOT NULL,
    `pathSnapshot` LONGTEXT,
    ...
    PRIMARY KEY (`vmInstanceUuid`),
    CONSTRAINT `fkVmMetadataPathFingerprintVOVmInstanceEO` FOREIGN KEY (`vmInstanceUuid`)
        REFERENCES `VmInstanceEO` (`uuid`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
